...
static SUPPORT_TYPE           support;
static TRANSACTION_TYPE       type;
static PAYMENT_MODE           payment;
static const TAB_ENTRY_FCT tab_amount = {0x0400,60};
...
void InitDebit(unsigned long *amount,S_MONEY *currency)
{
...
unsigned char WaitCard(unsigned short to)
{
  FILE            *hcam0;
  STATUS           statis;
  unsigned char    Cr;
  ...
  status(hcam0,(unsigned char *)&statis);
  /* CAM is already present */
  if ((statis & CAM_PRESENT)==CAM_PRESENT)
  {
    Cr = CR_ENTRY_OK;
  }
  else
  {
    ...
    do
    {
      ...
      { 
        if (ret==CAM0)
        {
          Cr = CR_ENTRY_OK;break;
        }
        else
        {
          Cr = CR_ENTRY_TIME_OUT;break;
        }
      }
    }
        while (1);
...
unsigned char WaitSwipe(unsigned short to)
{
  FILE            *swipe2;
  unsigned char    Cr;
  int              ret;
...
unsigned char ChoosePayment(void)
{
  ENTRY_BUFFER     buf;
  StructList       Menu;
  unsigned char    Cr;
  ...
  do
  {
    default_menu(&Menu);
    Menu.MyWindow.shortcommand  = _ON_;
    Menu.MyWindow.title         = "PAYMENT";
    Menu.tab[0] = (unsigned char *)"Unknown";
    Menu.tab[1] = (unsigned char *)"Cards";
    Menu.tab[2] = (unsigned char *)"Check";
    Menu.tab[3] = NULL;
    G_List_Entry((void *)&Menu);
    ttestall(ENTRY,0);
    Cr = Get_Entry(&buf);
  ...
  {
    if (Cr==CR_ENTRY_CANCEL)   break;
    if (Cr==CR_ENTRY_TIME_OUT) break;
  }
 }
 while (1);
 return(Cr);
...
unsigned char ChooseType()
{
  ...
  do
  {
   default_menu(&Menu);
   Menu.MyWindow.shortcommand  = _ON_;
   Menu.MyWindow.title         = "TRANSACTION";
   Menu.tab[0] = (unsigned char *)"Debit";
   Menu.tab[1] = (unsigned char *)"Cancel";
   Menu.tab[2] = (unsigned char *)"Credit";
   Menu.tab[3] = (unsigned char *)"Extended Cancel";
   Menu.tab[4] = NULL;
   G_List_Entry((void *)&Menu);
   ttestall(ENTRY,0);
   Cr = Get_Entry(&buf);
...
unsigned char ChooseSupport(void)
{
 ENTRY_BUFFER     buf;
 StructList       Menu;
 unsigned char    Cr;
 do
 {
   default_menu((void *)&Menu);
   Menu.MyWindow.shortcommand  = _ON_;
   Menu.MyWindow.title         = "SUPPORT";
   Menu.tab[0] = (unsigned char *)"Card";
   Menu.tab[1] = (unsigned char *)"Swipe2";
   Menu.tab[2] = (unsigned char *)"Manual";
   Menu.tab[3] = (unsigned char *)"Unknow";
   Menu.tab[4] = NULL;
   G_List_Entry((void *)&Menu);
   ttestall(ENTRY,0);
   Cr = Get_Entry(&buf);
   if (Cr==CR_ENTRY_OK)
   {
     switch (buf.d_entry[0])
     {
       ...
       case 2:
        ...
        strcpy(s_debit.param_in.track2,"B4761739001010010D05110000000F");
       break;
       case 3:
...
int MyTransaction (char *message)
{
  unsigned char    priority;
  unsigned int     retour;
  unsigned char    cr = STOP;
  S_MONEY          currency;
  ...
  {
    cr=GetAmount(0,"USD",1,&amount,message,(TAB_ENTRY_FCT *)&tab_amount);
    if (cr == T_VAL)
    {
      /* Default value */
      support = UNKNOWN_SUPPORT;
      ...
      if (cr==CR_ENTRY_OK)
      {
        ...
        ServiceCall(TYPE_GESTIONNAIRE,100,sizeof(StructDebitEmv),
        &s_debit,&retour);
        /* Close CAM0 if opened */
        if (hcam0==NULL)
        {
        ...
        fopen("PRINTER","w-");
        if (s_debit.param_out.rc_payment == PAY_OK)
        {
          pprintf("Transaction OK\n");
          pprintf("Appli No   : %04X\n",s_debit.param_out.noappli);
          pprintf("HolderNo:%19.19s\n",s_debit.param_out.card_holder_nb);
        }
        else
        {
