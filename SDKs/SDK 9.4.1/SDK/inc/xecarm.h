...
/*! @brief Define a configuration table. */
struct tConfigTable
  {Word  MaxPriority;							/*!< range 0..4095. */
   Word  NbTasks;									/*!< range 0..8191. */
   Word  NbSemaphores;						/*!< range 0..2047. */
   Word  NbMailBoxes;							/*!< range 0..2047. */
   Word  NbMessages;							/*!< range 0..65535. */
   Word  NbDelays;								/*!< range 0..4095. */
   DoubleWord DataSegmentSize;		/*!< size of a data segment. */
   Word  *ClockTaskStackPtr;			/*!< pointer to the end of the stack of the clock task. */
   Word  ClockTaskPriority;				/*!< priority of the clock task. Range 0..MaxPriority. */
   Word  TCBExtSize;							/*!< TCB ext size. */
   char  (*BKGHook) ();						/*!< (*BKGHook) () : BKG hook function */
   DoubleWord BKGHookParam;				/*!< BKG hook parameters */
   char  (*CLKHook)();						/*!< (*CLKHook)() : CLK hook function */
   DoubleWord CLKHookParam;				/*!< CLK hook parameters  */
   Word  XECMaskLevel;						/*!< range 0..7  */
    };
...
/*! @brief This function is used to initialise tasks. 
 * @param TaskNumber     : range 0..NbTasks-1.
 * @param Priority       : range 0..MaxPriority
 * @param StartAddress   : pointer to the fonction which contains the code of the task
 * @param SStackPtr      : pointer to the end of the supervisor stack
 * @param UStackPtr    	 : pointer to the end of the user stack
 * @param SupervisorMode : 1 if supervisor, 0 if user mode task
 * @param HookAddr       : hook of the task,-1 means no hook 
 * @param HookParam      : hook parameter
 * @param pTaskTCB       : output parameter:TCB address
 *
 * @link KSFAM_OS_TASK Back to top @endlink
 */ 
extern tStatus OS_rtc_InitTask(
     Word       TaskNumber,      
     Word       Priority, 
     Word       (*StartAddress)(),
     Word       *SStackPtr,       
 ...
 * @return The state of a task. tStatus : 
 * - cOK 
...
/*! @addtogroup KSFAM_OS_EVENT
	* @{
	 */ 	
extern void OS_rtc_EnterRegion(void);

extern void OS_rtc_LeaveRegion(void);
/*
=======================================
			Event management 
=======================================
...
/*! @brief The calling task is requesting allocation of a unit in the semaphore identified by SemaphoreNumber. 
 *
 * If no unit is available, the calling task is inserted in 
   the task queue associated with semaphore @c SemaphoreNumber and waits until one unit 
   is available. The enqueueing scheme will be based upon tasks priorities : the task is
   inserted immediately before the first task of the task queue which has a lower priority.
 *
 * The unit count associated with the semaphore is decremented. 
 *
 * In order to limit the waiting time, a maximum value TimeOut can be specified. Timeout is 
   specified in terms of number of 10 milliseconds. If the time-out occurs before the granting 
   of the unit, the task is reactivated and will resume with a status cTime-out. A value of 
   zero means that no limit is requested. 
 *
 * It is possible, but not advised to combine calls to P with calls to PWithPrio for the same semaphore.
 * @param SemaphoreNumber : Use GetSemaphoreUser() to get the SemaphoreNumber.
 * @param TimeOut : Maximum waiting time in terms of number of 10 milliseconds. A time-out value of 0 indicates no waiting limit.
 * @return Status :
 * - cOK 
 ...
 * @link KSFAM_OS_SEMAPHORE Back to top @endlink
 */ 
extern tStatus OS_rtc_PWithPrio( Word SemaphoreNumber,
                               tTimeOut TimeOut);
              /* returns status = cOK or cTimeOut or cExceptionOccurred */
...
/*! @brief This primitive releases all the tasks that are waiting for a unit of the specified semaphore. 
 *
 * The state of these tasks is set to ready. When they will be scheduled again their status will be cCleared. 
   The scheduler is invoked by this operation, so the calling task may be preempted.
 * @param SemaphoreNumber : Use GetSemaphoreUser() to get the SemaphoreNumber.
 * @param pNbReleasedTasks : Number of released tasks
 * @return  Status :
 * - cOK 
 * - cBadSemaphoreNumber : Fatal error. The semaphore number specified is not in the range configured by the OS.
 *
 * @link KSFAM_OS_SEMAPHORE Back to top @endlink
 */ 
extern tStatus OS_rtc_ClearSem( Word SemaphoreNumber , Word *pNbReleasedTasks);
...
/*! @brief The send primitive requests the sending of the message @c Message to the mailbox identified by @c MailboxNumber. 
 *
 * If no task is waiting for a message from the mailbox the 'Message' is stored in an 'envelope' and enqueued at the end of the message queue associated with the mailbox.
 *
 * The enqueueing scheme will be based upon message priorities @c MessagePriority. 
   The message is inserted after the messages of higher or equal priority (i.e. before the first message with lower priority in the queue). 
   If no envelope is available, a fatal error (cMessOverFlow) is generated. Should the number of messages in
	 the message queue exceed the local capacity specified by the
	 primitive InitMailbox, the request is refused and an error status word
	 (cFull) is returned to the calling task.
 *
 * In the case where the queue of waiting task is not empty, the
	 message is directly granted to the first task of the queue, which is
	 thus dequeued and activated.
 *
 * It is possible to combine calls to Send with SendWithprio referencing
	 the same mailbox. A message sent without priority will be enqueued
	 as if its priority is null.
 * @param MailBoxNumber   : Use GetMailboxUser() to get the MailBoxNumber.
 * @param Message         : Message to be sent (4 bytes) 
 * @param MessagePriority : Priority of the @c Message to be sent.
 * @return Status :
 * - cOK 
 * - cFull : The local capacity of the mailbox (specified by InitMailBox) has been exceeded. 
 * - cBadMailBoxNumber : The mailbox number specified is not in the range configured by the OS. 
 * - cMessOverFlow : The primitive attempted to use more envelopes than were configured by the OS.
 *
 * @link KSFAM_OS_MAILBOX Back to top @endlink
 */ 
extern tStatus OS_rtc_SendWithPrio( Word MailBoxNumber,
                             DoubleWord Message,
                             Word  MessagePriority);
                             /* range 0..65535 */
               /* returns status = cOK or cFull */
...
/*! @brief This primitive releases all the tasks that are waiting for a message upon the specified mailbox. 
 *
 * The state of those tasks is set to ready. When they will be scheduled again their status will be cCleared. If
   there are messages in the message queue they will be deleted. The
   scheduler is invoked by this operation, so the calling task may be preempted.
 * @param MailboxNumber      : Use GetMailboxUser() to get the MailBoxNumber.
 * @param pNbReleasedTasks   : Number of tasks released by this operation.
 * @param pNbDeletedMessages : Number of message deleted by this operation.
 * @return Status :
 * - cOK 
 * - cBadMailBoxNumber : The mailbox number specified is not in the range configured by the OS.
 *
 * @link KSFAM_OS_MAILBOX Back to top @endlink
 */ 
extern tStatus OS_rtc_ClearMailBox( Word MailboxNumber,
                             Word *pNbReleasedTasks,
                             Word *pNbDeletedMessages);
               /* returns status = cOK */
