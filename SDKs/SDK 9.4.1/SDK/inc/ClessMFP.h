...
/** \addtogroup MIFARELib 
 * @{
   * \defgroup ClessMFP MIFARE Plus card management
   * @{
     *   \details This library implements MIFARE Plus commands to manage MIFARE Plus cards from 
     *   Security Level 0 to Security Level 3.
     *    The cryptographic operations can be done either on the Thunder, either on a MIFARE SAM AV2.
     *    
     ...
     *   @{
     *      \details The MIFARE Plus cryptographic operations can be done by Telium
...
#define MFP_DLL_NOT_LOADED               0xFF //!< The DLL is not loaded
 ...
 * @return Pointer to the new MFP context. NULL if an error occured.
 */
 ...
 * @return Return a pointer to the new MFP context. NULL if an error occured.
 */
 ...
 */
int ClessMFP_SL0WritePerso(T_MFP_CONTEXT* pContext,  unsigned short usBNr, void* pData);
 ...
 * @param[in] pucKey Value of the key (16 bytes).
 * @return @ref ClessMFPErrors.
 * @note When using SL1 Card Authentication Key, \a ucMode shall be MFP_AUTH_N
 */
int ClessMFP_SL1AuthenticateAES(T_MFP_CONTEXT* pContext, unsigned short usKeyNb, unsigned char ucMode, const unsigned char* pucKey);
 ...
 * @param[in] iDivInpLen Length of the \a pDivInp data. 0 if no diversification.
...
int ClessMFP_SL1AuthenticateAESWithSAM(T_MFP_CONTEXT* pContext, unsigned short usKeyNb, unsigned char ucMode,
               unsigned char ucSAMKeyNb, unsigned char ucSAMKeyVersion, int iDivInpLen, unsigned char* pDivInp);
 ...
 * @param[in] pucKey Value of the key (16 bytes).
 * @return @ref ClessMFPErrors.
 */int ClessMFP_SL2AuthenticateAES(T_MFP_CONTEXT* pContext, unsigned short usKeyNb,  unsigned char ucMode, const unsigned char* pucKey);
 ...
 * @param[in] pucKey Value of the MIFARE key (6 bytes).
 * @return @ref ClessMFPErrors.
 */
  ...
  * @param[in] ucSAMKeyVersion Key version of the SAM key used.
  * @param[in] pucDiv Pointer to the diversification block number (1 byte) or NULL if no diversification.
  * @return @ref ClessMFPErrors.
  */
 int ClessMFP_SL12AuthenticateClassicWithSAM(T_MFP_CONTEXT* pContext, unsigned char ucKeyType, unsigned char ucSector,
                unsigned char ucSAMKeyNb, unsigned char ucSAMKeyVersion, unsigned char* pucDiv);
 ...
 * @return @ref ClessMFPErrors.
 */
int ClessMFP_SL3Authenticate(T_MFP_CONTEXT* pContext, unsigned short usKeyNb,  unsigned char ucMode, const unsigned char* pucKey);
 ...
 * @return @ref ClessMFPErrors.
 */
int ClessMFP_SL3Write(T_MFP_CONTEXT* pContext, unsigned char ucCC, unsigned short usBNr, unsigned char ucNbBlock, void* pData);
 ...
 * @param[in,out] pContext Pointer to the MFP context.
 * @param[in] pucKey Value of the Proximity Check key or NULL to use the sessions keys.
 * @return @ref ClessMFPErrors.
 */
int ClessMFP_SL3ProximityCheck(T_MFP_CONTEXT* pContext, const unsigned char* pucKey);
...
int ClessMFP_SL3ProximityCheckWithSAM(T_MFP_CONTEXT* pContext, unsigned char ucSAMKeyNo,
      unsigned char ucSAMKeyVersion, int iDivInpLen, const unsigned char* pDivInp);
...
int ClessMFP_SL3ChangeKeyWithSAM(T_MFP_CONTEXT* pContext, unsigned short usKeyNb,
      unsigned char ucSAMKeyNb, unsigned char ucSAMKeyVersion, int iDivInpLen, const unsigned char* pDivInp);


/**
 * Authentication between PCD and SAM AV2.
 * @param[in,out] pContext Pointer to the MFP context.
 * @param[in] pucKey Value of key (16 bytes).
 ...
 * @return @ref ClessMFPErrors.
 */
int ClessMFP_SAMAuthenticate(T_MFP_CONTEXT* pContext, const unsigned char* pucKey,
