...
static const char APP_NAME[] = "FMG SAMPLE";
...
static int after_reset_(NO_SEGMENT AppliNum, S_TRANSOUT* pParamOut)
{
	...
	//Mount sample Disk : must be done before FMG_Init()
	EntCreateOrMountDisk(DISK_NAME,DISK_SIZE);
	...
	if (ret ==FMG_SUCCESS )
	{
		CUTERMprintLine("First Fmg Init Ok");
	}
	else if(ret == FMG_INIT_OK)
	{
		CUTERMprintLine("Init Fmg Ok");
	}
	else
	{
		CUTERMprintLine("Init Fmg Failed !");
	}
	CUTERMprintLine("\n\n");
	//////////////////////
