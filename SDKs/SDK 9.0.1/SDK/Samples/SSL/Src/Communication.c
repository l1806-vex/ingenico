...
int PerformConnectionSSL( const char *szHostName, unsigned int nPort, const char *szProfile )
{
	char szTemp[100];
	...
	DisplayScreen( "Loading SSL Profile", "" );

	hProfile = SSL_LoadProfile( szProfile );
	if( hProfile != NULL )
	{
		DisplayScreen( "Connecting SSL...", "" );

		if( SSL_New( &hSSL, hProfile ) == 0 )
		{
			if( SSL_Connect( hSSL, szHostName, nPort, __20_SECONDS__ ) == 0 )
			{
				DisplayScreen( "Connected SSL...", "OK !" );
				ttestall( 0, __3_SECONDS__ );

				//
				DisplayScreen( "Disconnecting SSL...", "" );
				SSL_Disconnect( hSSL );
			}
			else
			{
				...
				sprintf( szTemp, "Error = %d", nError );

				DisplayScreen( "Connecting SSL...", szTemp );
				ttestall( 0, __3_SECONDS__ );
			}
