...
//// Global variables ///////////////////////////////////////////
static const char APP_NAME[] = "CARD ENTRY";

static service_desc_t Services[] = {
    { 0, GIVE_YOUR_DOMAIN, (SAP)Main, SERVICES_HIGH_PRIORITY },
    { 0, AFTER_RESET, (SAP)Main, SERVICES_HIGH_PRIORITY },
    { 0, IS_NAME, (SAP)Main, SERVICES_HIGH_PRIORITY },
    { 0, IS_STATE, (SAP)Main, SERVICES_HIGH_PRIORITY },
...
extern void ServiceCardEntry(int size, S_CUST_CARD_Entry *infoscust);
...
int give_your_domain(NO_SEGMENT no,void *p1,S_INITPARAMOUT *param_out)
{
  ...
  etatout.returned_state[etatout.response_number].application_type = TYP_CARTE;
  /* One more */
...
int is_name(NO_SEGMENT no,void *p1,S_ETATOUT *etatseq)
{
  ...
  strcpy((char *)etatout.returned_state[etatout.response_number].appname,"CARD ENTRY");
...
{
  service_desc_t MyServices[NB_MAX_SERVICE];
  int            i;

  for (i=0;Services[i].sap!=(SAP)NULL;i++)
  {
    MyServices[i].appli_id  =      no;
    MyServices[i].serv_id   =      Services[i].serv_id;
    MyServices[i].sap       = (SAP)Services[i].sap;
    MyServices[i].priority  =      Services[i].priority;
  }
  ServiceRegister(i,MyServices);


  MyServices[0].appli_id  = no;
  MyServices[0].serv_id   = CUSTOMIZE_CARD_ENTRY;
  MyServices[0].sap       = (SAP)ServiceCardEntry;
  MyServices[0].priority  = 10;
  ServiceRegister(1,MyServices);
