...
int IsNetworkAvailable( int bGPRS, int *pbResult, int *pError, int *pbLinkPPP )
{
	...
	if( gprs != NULL )
	{
		...
		if( CheckSIM( &info, &bReadySIM, pError ) == __FALSE__ )
		{
			// SIM status unknown.
			return __TRUE__;
		}

		// The SIM status is know.
...
int WaitNetworkScreen( char *szAPN, int bGPRS )
{
	...
	if(( nError == GPRS_OK )&&
	   ( WaitNetworkAvailable( bGPRS, __500_MSECONDS__, __60_SECONDS__, &nError ) == __TRUE__ ))
	{
		return __TRUE__;
	}
...
int PerformConnectionGPRS( char *szAPN, char *szLogin, char *szPassword )
{
	...
	if( WaitNetworkScreen( szAPN, __TRUE__ ) == __TRUE__ )
	{
...
static int PerformTestTCP_LoopHTTP( char *szHostname, int nPort )
{
	...
	while( bContinue == __TRUE__ )
	{
		...
		if( nHandle >= 0 )
		{
			...
			while(( nResult = tcp_send_to_server( nHandle, "GET / HTTP/1.0\r\n\r\n", 18, __INFINITE__ ))
						== ERROR_TIMEOUT )
			{
...
static int PerformTestTCP_LoopEcho( char *szHostname, int nPort )
{
	...
	if( nHandle >= 0 )
	{
		...
		while(( bContinue == __TRUE__ ) && ( nIterations < 1000 ))
		{
			...
			while(( nResult = tcp_send_to_server( nHandle, __FRAME_ECHO, nFrameSize,0 ))
						== ERROR_TIMEOUT )
			{
				ttestall( 0, __100_MSECONDS__ );
			}
...
static int PerformTestTCP_Receive100K( char *szHostname, int nPort )
{
	...
	if( nHandle >= 0 )
	{
		...
		// Send request frame.
		while(( nResult = tcp_send_to_server( nHandle, __FRAME_RECEIVE_100K, nFrameSize,0 ))
						== ERROR_TIMEOUT )
		{
			ttestall( 0, __100_MSECONDS__ );
		}
						  ...
						  int nMode, int bRLP )
{
	   ...
	   ( WaitNetworkAvailable( __FALSE__, __500_MSECONDS__, __20_SECONDS__, &nError ) == __TRUE__ ))
	{
		...
		if( nError == GPRS_OK )
		{
			...
			// Run FLOW tests
			gsm_write( __FRAME_RECEIVE_100K, strlen( __FRAME_RECEIVE_100K ));
