 ...
 *
 *  The key value is DES or TDES ciphered with a ciphering key.
 *
 ...
 *  @param[in] ptKeyValue 		: DES/TDES Ciphered value of the Key to load (its length is 8, 16 or 24 bytes) 
 ...
 *  @param[in] 		secureType 			: Secure part
 *  @param[in] 		ptCipherKeyId 	: Ciphering Key to use 
 *  @param[in] 		iFormat 				: ISO9564 Format : format 0 (ISO9564_F0_DES, ISO9564_F0_TDES),
																format 1 (ISO9564_F1_DES, ISO9564_F1_TDES),
																format 2 (ISO9564_F2_DES, ISO9564_F2_TDES, ISO9564_F2_OTHER),
																format 3 (ISO9564_F3_DES, ISO9564_F3_TDES),
...
/*! @brief This function is used to load the Key Serial Number (KSN) for DUKPT algoritm
 *
...
/*! @brief This function is used to load the Derivation Key (16 bytes) to use later to generate the initial key for DUKPT algoritm
 *
...
/*! @brief This function is used to load the Initial Key (8/16 bytes if DES/TDES Dukpt) for DUKPT algoritm
 *         and to initialise the set of 21 keys and counter. This Initial Key is TDES ciphered.
 *
 ...
 *  @param[in] 		ptDukptId		 	: DUKPT Identifier to use (cAlgoType determines the Dukpt Algorithm to use
											...
											DES Dukpt light memory (TLV_TYPE_DESDUKPTLIGHT) or TDES Dukpt light memory (TLV_TYPE_TDESDUKPTLIGHT) )
 *  @param[in] 		ptCipherKeyId 		: Identifier of the TDES ciphering key 
 *  @param[in] 		ptInitialKeyValue : Ciphered Initial Key value (8 or 16 bytes) 
 *  @return      OK or @link ERR_Schemes Error @endlink
...
/*! @brief This function is used to generate the initial Key (8/16 bytes if DES/TDES Dukpt) for DUKPT algoritm
...
/*! @brief This function is used to get the current status of DES/TDES DUKPT algoritm
 *
 *	If the secure part is a BL1 booster and if the secret area does not exist then an error is returned.
 *
 *	If the secret area has the format C_SEC_NO_TLV or C_SEC_TLV_BL1, the scheme to call is 'SchDukpt' or 'SchDukpTDes' otherwise 'TlvDukpt' or 'TlvDukpTDes'.
...
/*! @brief This function is used to stop and delete the data of DES/TDES DUKPT algoritm
 *
 *	If the secure part is a BL1 booster and if the secret area does not exist then an error is returned.
 *
 *	If the secret area has the format C_SEC_NO_TLV or C_SEC_TLV_BL1, the scheme to call is 'SchDukpt' or 'SchDukpTDes' otherwise 'TlvDukpt' or 'TlvDukpTDes'.
...
/*! @brief This function is used to encrypt the PIN with DES/TDES DUKPT algoritm; The supported PIN block formats are formats 0 and 3.
 *
 *	The PIN code has been entered previously on the secure Pincode part (#C_SEC_PINCODE) and optionally transferred 
...
/*! @brief This function is used to verify and compute the Key checksum value of a DES/TDES LightDukpt
 *     key (Initial key or Derivation Key);
 *
 *   The key checksum value are the 3 most significant bytes result
 *        of DES/TDES ciphering 8 null data bytes with the key value.
 *
 *	If the secure part is a BL1 booster and if the secret area does not exist then an error is returned.
 *
 *	If the secret area has the format C_SEC_NO_TLV or C_SEC_TLV_BL1, the scheme to call is 'SchDukpt' or 'SchDukpTDes' otherwise 'TlvDukpt' or 'TlvDukpTDes'.

 *  Schemes to sign with your application: #NAM_SCH_UTILTLV, #NAM_TLV_DUKPT_LIGHT_DES, #NAM_TLV_DUKPT_LIGHT_TDES
 *  @param[in] 		secureTypeKey 				: Secure part
 *  @param[in] 		ptDukptId 				: Dukpt light Key to compute KCV (cAlgoType determines the Dukpt Algorithm to use
											DES Dukpt light memory (TLV_TYPE_DESDUKPTLIGHT) or TDES Dukpt light memory (TLV_TYPE_TDESDUKPTLIGHT) )
 ...
 *  @param[in] 		ptCipherKeyId 		: Ciphering Key to use (cAlgoType determines the Algorithm to use
											DES (TLV_TYPE_KDES) or TDES (TLV_TYPE_KTDES) )
 ...
 *
 *  Signed Schemes with the Security DLL: "s_PresenPin" for EMV Pin format; "SchSubmiPin" for MONEO Pin format
 ...
 * - The key checksum value are the 3 most significant bytes result of DES/TDES ciphering 8 null 
          data bytes with the key value for key types #TLV_TYPE_KDES, #TLV_TYPE_KTDES.
 * - The key checksum value are the 3 most significant bytes result TDES ciphering the constant
     8 data bytes (0x12, 0x34, 0x56, 0x78 ,0x90, 0x12, 0x34,0x56)  with the key value for key type #TLV_TYPE_KTDES_24.
 *
 ...
 *
 *	The 1st DES key value is the MSB 8 bytes of the TDES key and the 2nd DES key value is the LSB 8 bytes of the TDES key.
 *
 *	This function computes a 8 bytes MAC.
 *
 *	If the secret area has the format C_SEC_NO_TLV, the scheme to call is 'SchCipherPR'.
 ...
 *  Schemes to sign with your application: #NAM_SCH_UTILTLV, #NAM_TLV_CIPHER_DATA, #NAM_SCH_CIPHER_PR,
 *								#NAM_TLV_MAC_TDES
 *  @param[in] 		ptCipherKeyId 		: Ciphering Key to use (cAlgoType must be set to TDES (TLV_TYPE_KTDES) )
 *  @param[in] 		pt_inputData 			: Input Data for MAC calculation 
 *  @param[in] 		uiInputDataLength : length in bytes of Input Data for MAC calculation
 *  @param[in] 		pt_IV  						: pointer to 8 bytes ICV value (or NULL pointer if 8 bytes zero value)
 *	@param[out]   pt_OutputMAC 			: computed MAC buffer (8 bytes)
...
/*! @brief This function is used to get the status and the current transaction SMID of Light DES/TDES DUKPT algorithm 
 *
 *	If the secret area does not exist or if the secret area has the format C_SEC_NO_TLV then an error is returned.
 *
 *  Schemes to sign with your application: #NAM_SCH_UTILTLV, #NAM_TLV_DUKPT_LIGHT_TDES, #NAM_TLV_DUKPT_LIGHT_DES
 *  @param[in] 		secureTypeKey 	: Secure part
 *  @param[in] 		ptDukptId 				: Dukpt light Key Identifier to use (cAlgoType determines the Dukpt Algorithm to use
											DES Dukpt light memory (TLV_TYPE_DESDUKPTLIGHT) or TDES Dukpt light memory (TLV_TYPE_TDESDUKPTLIGHT) )
 ...
 *
 *  Signed Schemes with security DLL: #NAM_SCH_SHA_COMPUTE
 *  @param[in] 		secureType 		: Secure part
...
typedef struct {
	...
	char cType;  															/*!< Secure scheme location : C_MODULE_SCH (in the security component) 
