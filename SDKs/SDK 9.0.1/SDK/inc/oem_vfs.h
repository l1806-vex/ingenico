 ...
 * @par Example:
 * @code
 * static int FsMount(void) 
 * {
 *   struct msdosfs_args    fat_args;

 *   memset(&fat_args, 0, sizeof(fat_args));

 *   fat_args.fspec = "/dev/umsd0a";  // USB mass storage device 0 partition a (USB key)
 *   fat_args.mask = 0666;
 *   fat_args.flags = MSDOSFSMNT_LONGNAME;
 *   fat_args.magic = MSDOSFS_ARGSMAGIC;
 *   return mount(MOUNT_MSDOS, "/USB", MNT_SYNCHRONOUS, &fat_args) ;
 * }
* @endcode 
