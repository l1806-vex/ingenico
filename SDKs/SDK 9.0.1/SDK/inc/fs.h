/*! @addtogroup KSFAM_FFMS
	...
	* @section ex_FFMS Example
 	...
 	* strcpy(ParamCreate.Label, "APPLI1");
 	...
 	* if (ret==FS_NOMEM)
 	* printf ("not enough space : %l \n", &DiskSize);
 	*
 	* // open FTEST for writing testing purpose
 	* sprintf(FilePath_L,"/%s/FTEST", ParamCreate.Label);
 	* fp1 = FS_open(FilePath_L,"a");
 	* if (fp1==NULL)
 	*   printf("FS_open FTEST : ERROR\n");
 	* else
 	*   printf("FS_open FTEST : OK\n");
 	* ret = FS_write("This is the writing test in file FTEST",39,1, fp1);
 	* if (ret!=1)
 	*   printf("FS_write dans FTEST: ERROR\n");
 	* else
 	*   printf("FS_write dans FTEST: OK\n");
 	* FS_close(fp1);
 	*
 	* // searching for FTEST in the volume
 	* fpdir = FS_opendir(FilePath_L);
 	* if (fpdir==NULL)
 	*   printf("FS_opendir : ERROR\n");
 	* else
 	*   printf("FS_opendir : OK\n");
 	* if(FS_readdir(fpdir, &info)!=FS_OK)
 	*   printf("FS_readdir : NO FILE\n");
 	* else
 	*   printf("get info from %s : DiskSize %ld\t",
 	*   info->FileName,
 	*   info->FileSize);
 	* FS_closedir(fpdir);
 	*
 	* // open FTEST for reading checking
 	* fp2 = FS_open(FilePath_L,"r");
 	* if (fp2==NULL)
 	*   printf("FS_open FTEST: ERROR\n");
 	* else
 	*   printf("FS_open FTEST: OK\n");
 	* ret = FS_read(buffer,39,1,fp2);
 	* if (ret!=1)
 	*   printf("FS_read FTEST: ERROR\n");
 	* else
 	*   printf("file contains : %s\n",buffer);
 	* FS_close(fp2);
 	*
 	* // display volume free space
 	* printf("Free space: %ld bytes\n",FS_dskfree(FilePath_L));
 	* // desactivate the volume
 ...
 *
 * - r+ create or open an existing file in updating mode (read and write). File cursor is set to 0.
 *   With r+ mode writing is allowed everywhere, even at the end of the file (append mode)
 *   r+ mode is allowed for FLASH disk and RAM disk.
 *
 * - r# mode is similar to r+ mode, but writing into the file is made asynchronously.
 *   When calling FS_write, data are not written immediately: if a power down occurs, data are not guaranteed 
 *   Whith r# FS_flush and FS_close can be used to flush data into memory.
 * 
...
//  @GSIM_A:1:ONE_ADDRESS
// \endif
unsigned long   FS_length (S_FS_FILE *File);
