...
#define USBCOM_FIOCTL_SET_DTR            0x8020		/*!< Set DTR signal - Only for compatibility : please use \ref COM_FIOCTL_SET_DTR */
#define USBCOM_FIOCTL_CLEAR_DTR          0x8021		/*!< Clear DTR signal - Only for compatibility : please use \ref COM_FIOCTL_CLEAR_DTR */
#define USBCOM_FIOCTL_SET_RTS            0x8022		/*!< Set RTS signal. Flow control must be disabled - Only for compatibility : please use \ref COM_FIOCTL_SET_RTS */
#define USBCOM_FIOCTL_CLEAR_RTS          0x8023		/*!< Clear RTS signal. Flow control must be disable - Only for compatibility : please use \ref COM_FIOCTL_CLEAR_RTS */
...
#define USBDEV_FIOCTL_GET_PID            0x8026		/*!< COM6 : Get PID, parameter is unsigned int */
#define USBDEV_FIOCTL_GET_VID            0x8027		/*!< COM6 : Get VID, parameter is unsigned int */
...
#define USBDEV_FIOCTL_GET_STATE          0x8009 	/*!< COM5 : gets device state - parameter type is unsigned int : 
                                                       1 = a "set configuration" command has been received 
                                                       0 = device is not connected
                                                   */
#define USBDEV_FIOCTL_CNX                0x8010 	/*!< COM5 : gets device connection state - parameter type is unsigned int :
                                                       1 = device is connect
                                                       0 = device is not connected
                                                   */
#define USBDEV_FIOCTL_HANG               0x8011		/*!< COM5 : gets device connection state - parameter type is unsigned int : 
                                                       1 = USB bus is suspended or disconnected 
                                                       0 = device is operationnal state
                                                   */
#define USBDEV_FIOCTL_SET_VID            0x8012   /*!< COM5 : @deprecated  Set VID, parameter is unsigned int.  Only for compatibility : use \ref USBDEV_FIOCTL_SET_MODE.*/
#define USBDEV_FIOCTL_SET_PID            0x8013   /*!< COM5 : @deprecated  Set PID, parameter is unsigned int.  Only for compatibility : use \ref USBDEV_FIOCTL_SET_MODE.*/
#define USBDEV_FIOCTL_SET_MAX_PWR        0x8014   /*!< COM5 : Set max power consumption.
                                                       Parameter type is unsigned int
                                                       unit is 1 mA 
                                                  */
#define USBDEV_FIOCTL_SET_SPEED          0x8015   /*!< COM5 : Set USB device bus speed: HS(1)/FS(0) if available */

#define USBDEV_FIOCTL_START_DRIVER       0x8024		/*!< COM5 : Start USB device stack */
#define USBDEV_FIOCTL_STOP_DRIVER        0x8025		/*!< COM5 : Stop USB device stack */

#define USBDEV_FIOCTL_START_HID          0x8028    /*!<  COM5 : @deprecated Activate default HID mode. Only for compatibility : use \ref USBDEV_FIOCTL_SET_MODE. \return 0. \warning only for the products which support \ref USB_DEVICE_MODE_HID */
#define USBDEV_FIOCTL_STOP_HID           0x8029    /*!<  COM5 : @deprecated Deactivate all HID modes, Only for compatibility : use \ref USBDEV_FIOCTL_SET_MODE. return to CDC class.\return 0.*/
#define USBDEV_FIOCTL_HID_STATUS         0x8030    /*!<  COM5 : @deprecated Get HID status On/Off.   Only for compatibility : use \ref USBDEV_FIOCTL_GET_MODE.*/
#define USBDEV_FIOCTL_START_HID_SV       0x8031    /*!<  COM5 : @deprecated Activate Sesame Vitale HID mode. This mode is no longer supported */
...
#define USBDEV_FIOCTL_SWITCH_MODE	     0x8035    /*!<  COM5 : Switch the USB device mode without saving it. The previous mode will be restored at the terminal startup \param int* \ref USB_DEVICE_MODE. \return \ref USB_DEVICE_CODE. \note @li supported by SDK>=8.2.@li supported if \ref FIOCTL_GET_VERSION (COM5) returns >=2.@li To swith mode and save it, use \ref USBDEV_FIOCTL_SET_MODE*/

#define USBDEV_FIOCTL_SET_USER_SERIAL_NUMBER	0x8036    /*!<  COM5 : enable and set the serial number string. \param char* user string (max size 20, 0 included) \return \ref USB_DEVICE_CODE.
...
if (handle!=NULL)
{
    strcpy (szSN,"MyString");
   if (fioctl(USBDEV_FIOCTL_SET_USER_SERIAL_NUMBER,szSN,handle)==USBDEV_OK)
   {
...
if (handle!=NULL)
{
   nState=USBDEV_SERIAL_NUMBER_ENABLED
   if (fioctl(USBDEV_FIOCTL_SET_SERIAL_NUMBER_STATUS,&nState,handle)==USBDEV_OK)
   {
...
//!    if (fioctl(USBDEV_FIOCTL_GET_MODE,&nMode,handle)==USBDEV_OK)
//!    {
//!       if (nMode!=USB_DEVICE_MODE_HID)
//!       {
//!          // to set the mode
//!          nMode=USB_DEVICE_MODE_HID;
//!          if (fioctl(USBDEV_FIOCTL_SET_MODE,&nMode,handle)==USBDEV_OK)
//!          {
//!          }
//!          ->no usb reset expected.
//!        }
//!    }
//!    fclose(handle);
//! }
...
<tr><td>iSC220</td><td>0x0B00</td><td>0x0062</td></tr>
...
<tr><td>E560</td><td>0x0B00</td><td>0x0067</td></tr>
</table>
*/
...
<tr><td>iSC220</td><td>0x0B00</td><td>0x0075</td></tr>
