 ...
 * Warning: the abort_key is enable during the connection to the GPRS network but this key is not enable during the PPP connection.
 * - 0x00 : No key. 
 ...
 * It is possible to cancel only during GPRS network connection.   
// \if SIMULATION
...
/*! @brief This function returns the value of requested GPRS option.
 *
 * This function returns the value of requested GPRS option.
 *
 * @param option : requested GPRS option.
 * - GPRS_PARAM_LOGIN
 * - GPRS_PARAM_PASSWORD
 * - GPRS_PARAM_APN
 * - GPRS_PARAM_NETWORK
 ...
 * @return
 *  - GPRS_OK
 *  - GPRS_ERR_BAD_PARAM, if requested GPRS option does not exist
 *  - GPRS_ERR_NO_GPRS.
...
/*! @brief This function set the current slim slot.
 *	
 * @param sim_slot : new slim slot (0 or 1).
 * @return
 * - CR_ENTRY_OK.
 * - CR_ENTRY_NOK Bad param or function not implemented.
 * - CR_ENTRY_CANCEL Aborted by user.
 * @note
 * - This function reset the terminal after slim slot selection 
 * - See gprs_errno for detailled value when return is CR_ENTRY_NOK
 * - GPRS_ERRNO_DRIVER_NOT_OPEN DGPRS driver not open. 
 * - GPRS_ERRNO_NOT_IMPLEMENTED DGPRS function not implemented. 
 * - GPRS_ERR_BAD_PARAM DGPRS Bad value for param. 
 * @par none
 *
 * @link KSFAM_GPRS Back to top @endlink
*/
int gprs_select_sim_slot(int sim_slot);
