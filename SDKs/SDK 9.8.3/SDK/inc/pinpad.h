...
/*! @brief This function return TRUE if PINPAD has a CAM reader
 * @return None
 *
 * @link KSFAM_PPS_MANAGER Back to top @endlink 
*/
// \if SIMULATION
//  PPS_DLLReader:KSFAM_PPS_MANAGER:
// \endif
extern int        PPS_DLLReader           (void);
...
/*! @brief This function is used to get the number of pinpad connected
 * @return number of pinpad (can be 0) if successful or -1 if error occurred.
 * @par exit None
 *
 * @link KSFAM_PPS_MANAGER Back to top @endlink 
*/
// \if SIMULATION
//  @GSIM_F:PPR_GetNbPinpad:KSFAM_PPS_MANAGER:
// \endif
extern int           PPR_GetNbPinpad(void);


/*! @brief This function is used to switch pinpad
 * @return RET_PP_OK (0) if successful or -1 if error occured.
 * @par exit None
 *
 * @link KSFAM_PPS_MANAGER Back to top @endlink 
*/
// \if SIMULATION
//  @GSIM_F:PPR_SwitchPinpad:KSFAM_PPS_MANAGER:
// \endif
extern int           PPR_SwitchPinpad(void);
...
/*! @brief This function forces a BEEP when a key is pressed on the PINPAD in function calls PPS_Num_Entry or PPS_Fct_Entry
 * @param beepOn : TRUE or FALSE
 * @return
 *   Previous value if OK
 *   -1 if function not implemented
 *  @par exit none
 *
 * @link KSFAM_PPS_MANAGER Back to top @endlink 
*/
// \if SIMULATION
//  @GSIM_F:PPR_SetBeepOnKey:KSFAM_PPS_MANAGER:
// \endif
extern int PPR_SetBeepOnKey(int beepOn);

//!see \ref PPS_Num_Entry
extern void          Saisie_Numerique_Ppad    (TAB_ENTRY_STRING *tab_numerique);
//!see \ref PPS_Read_Entry
extern unsigned char Read_Saisie_Ppad         (BUFFER_SAISIE    *buf_saisie_ppad);
//!see \ref PPS_Stop_Entry
extern void          Stop_Saisie_Ppad         (void);
//!see \ref PPS_Fct_Entry
extern void          Saisie_Fonction_Ppad     (TAB_ENTRY_FCT    *tab_fonction);
//!see \ref PPS_DLLReader
