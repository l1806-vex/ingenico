...
//	int iNbKey, i2;
    ...
    for (i=0; i<iNbParts; i++)
	{
		...
		iRet = pprintf("Secret Area ID(s):\n"); CHECK(iRet>=0, lblKO);
		for (i1=0; i1<iNbId; i1++)                               // Get list of secret areas
		{
			if (tiList[i1] == 0x00002030)
			{
//				iRet = pprintf("%08x => Key ID(s):\n", tiList[i1]); CHECK(iRet>=0, lblKO);
//				iRet = SEC_listTLVKeys (txConfParts[i].SecureType, 0x00002030, sizeof(txKeyList),
//									    &iNbKey, txKeyList);
//				CHECK(iRet==OK, lblKO);
//				for (i2=0; i2<iNbKey; i2++)                      // Get list of keys from my secret area
//				{
//					iRet = pprintf("             %08x\n", txKeyList[i2].uiTLVKeyId); CHECK(iRet>=0, lblKO);
//				}
