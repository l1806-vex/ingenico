...
static service_desc_t
		Services[] = { { 0, GIVE_YOUR_DOMAIN, (SAP) Main,
				(unsigned char) SERVICES_HIGH_PRIORITY }, { 0, AFTER_RESET,
				(SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
				IS_NAME, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
				{ 0, IS_STATE, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						IDLE_MESSAGE, (SAP) Main,
						(unsigned char) SERVICES_DEFAULT_PRIORITY }, { 0,
						MORE_FUNCTION, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						KEYBOARD_EVENT, (SAP) Main,
						(unsigned char) SERVICES_DEFAULT_PRIORITY }, { 0,
						STATE, (SAP) Main,
						(unsigned char) SERVICES_DEFAULT_PRIORITY }, { 0,
						CONSULT, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0, MCALL,
						(SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY }, {
						0, IS_TIME_FUNCTION, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						TIME_FUNCTION, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						IS_CHANGE_INIT, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						MODIF_PARAM, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						IS_EVOL_PG, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						IS_DELETE, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						FILE_RECEIVED, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						MESSAGE_RECEIVED, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						IS_CARD_SPECIFIC, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						CARD_INSIDE, (SAP) Main,
						(unsigned char) SERVICES_DEFAULT_PRIORITY }, { 0,
						IS_FOR_YOU_AFTER, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY }, { 0,
						DEBIT_NON_EMV, (SAP) Main,
						(unsigned char) SERVICES_HIGH_PRIORITY } };
...
/************+++GLOBAL VARIABLES+++*********************/
T_GL_HGRAPHIC_LIB global_graphicLib;
/************---GLOBAL VARIABLES---*********************/
...
int give_your_domain(NO_SEGMENT no, void *p1, S_INITPARAMOUT *param_out) {
	...
	param_out->returned_state[param_out->response_number].mask = MSK_MDP
			| MSK_SWIPE | MSK_TYPE_PPAD | MSK_PINPAD | MSK_STANDARD
			| MSK_LANGUE | MSK_FRMT_DATE | MSK_DATE;
	// International domain
	param_out->returned_state[param_out->response_number].application_type
			= TYP_EXPORT;
...
int after_reset(NO_SEGMENT no, void *p1, S_TRANSOUT *param_out) {
	unsigned char chgt;
	TYPE_CHGT type;
	CGUI_CONTEXT_STRUCT MngContext;
	ulong result=0;

	// Creates an instance of the graphics library.
	// This creation takes some time.
	// To be quicker it can also be instanciated only once in application's life
	global_graphicLib = GL_GraphicLib_Create();

	// Enable header, footer  and disable led in order to Maximize canvas size
	goalSInitContext(&MngContext,false);

	first_init(no, &chgt, (unsigned char *) &type); // New software loaded ?
	if (chgt == 0xFF) // Yes, just loaded with first execution
	{
		result = GL_Dialog_Message  ( global_graphicLib,appName,coldReset,GL_ICON_NONE,GL_BUTTON_VALID,2*GL_TIME_SECOND);
		raz_init(no); // Reset indicator downloading
	} else // No, already loaded and executed
	{
		result = GL_Dialog_Message  ( global_graphicLib,appName,warmReset,GL_ICON_NONE,GL_BUTTON_VALID,2*GL_TIME_SECOND);
	}

	ttestall(0, 200); // Wait for 2s. //TODO remettte 2*100
...
int is_name(NO_SEGMENT no, void *p1, S_ETATOUT *param_out) {
	strcpy(param_out->returned_state[param_out->response_number].appname,
			appName);
...
int is_state(NO_SEGMENT no, void *p1, S_ETATOUT *param_out) {
	...
	param_out->returned_state[param_out->response_number].state.response
			= REP_OK;
...
int idle_message(NO_SEGMENT no, void *p1, void *p2) {
	...
	result = GL_Dialog_Message  ( global_graphicLib,appName,idleMsg,GL_ICON_NONE,GL_BUTTON_VALID,2*GL_TIME_SECOND);
//	goalSReleaseContext();
	return FCT_OK;
}
...
int more_function(NO_SEGMENT no, void *p1, void *p2) {
	   ...
	   CGUI_CONTEXT_STRUCT MngContext;
	   const char *MenuUser[] =
	   { "Details Dialog", "Details Widget", "Details Resource","Exit", 0 };

	   // Disables header, footer, led in order to Maximize canvas size
	   goalSInitContext(&MngContext,true);

	   do
	   {
	      //Display the menu using GOAL
	      choice = GL_Dialog_Menu(global_graphicLib, "Select", MenuUser,choice, GL_BUTTON_DEFAULT, GL_KEY_0, GL_TIME_INFINITE);
	      switch (choice)
	      {
	      case 0:USRinputDetailsDiag();	break;
	      case 1:USRinputDetailsWdg();	break;
	      case 2:USRinputDetailsRes();	break;
	      //exit
	      default:  bExit = true;		break;
	      }
	   } while (bExit == false);
...
int modif_param(NO_SEGMENT noappli, S_MODIF_P *param_in, void *p2) {
	...
	fopen("DISPLAY", "w");
	printf("MODIF_PARAM\n%04x",
			(int) param_changed.etatout.returned_state[0].state.mask);
	ttestall(0, 200);
	fclose(stdout());
	return (FCT_OK);
}
 ...
 * \param    p1 (-I) 
 * \param    param_out (-O) 
 *                          - response: REP_OK (App authorizes donwloading process)
 *                                      REP_KO (App refuses any downloading process)
 *  
 * \return FCT_OK
 *
 * \see sdk30.h
 ...
 * \param    p1 (-I) 
 * \param    param_out (-O)
 *                          - response: DEL_YES (App authorizes deletion process)
 *                                      DEL_NO (App refuses any deletion process)
 *  
 * \return FCT_OK
 *
 * \see sdk30.h
...
int is_for_you_after(NO_SEGMENT no, S_TRANSIN *param_in, S_CARDOUT *param_out) {
	...
	if (param_in->support == CHIP_SUPPORT) {
		if (param_in->power_on_result == 0) {
			// accept this card
			param_out->returned_state[param_out->response_number].cardappnumber
					= 1;
			param_out->returned_state[param_out->response_number].cardapp[0].priority
					= CARD_PROCESSED;
		} else {
			// reject the card
			param_out->returned_state[param_out->response_number].cardappnumber
	...
	// give my card name
	strcpy(
			param_out->returned_state[param_out->response_number].cardapp[0].cardappname,
			"Template");
