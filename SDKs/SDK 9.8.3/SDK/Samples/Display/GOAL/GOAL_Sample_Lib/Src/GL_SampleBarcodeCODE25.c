...
static T_GL_HWIDGET CreateBarcode(T_GL_HWIDGET parent)
{
	...
	GL_Barcode_SetMimeType(barcode, GL_MIME_BARCODE_CODE25);
	
	// Expand in all directions
	GL_Widget_SetExpand(barcode, GL_DIRECTION_ALL);

	// Stretch barcode in all directions
	GL_Widget_SetTransformation(barcode, GL_TRANSFORMATION_STRETCH_ALL);

	// Set the value of barcode
	GL_Barcode_SetValue(barcode, "2525252", strlen("2525252") + 1);

	// Configure the left and right margin
	GL_Barcode_SetParamInteger(barcode, GL_BARCODE_1D_SIDE_MARGIN, 10);

	// Activate checksum for the barcode
	GL_Barcode_SetParamInteger(barcode, GL_BARCODE_CODE25_CHECKSUM, 1);
	
	return barcode;
}


// sample function to display a barcode code 25
...
// sample function to print a barcode code 25
static void BarcodeOnPrinter(T_GL_HGRAPHIC_LIB graphicLib)
{
	...
	GL_Widget_SetMinSize(barcode, 384, 100, GL_UNIT_PIXEL);
