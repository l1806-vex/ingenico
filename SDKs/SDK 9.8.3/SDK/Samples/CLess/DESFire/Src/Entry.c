...
{ "Desfire Thunder", "Format PICC", "Authent. Host", "Change Key Entry", "Reset Key Entry", "Free Context"};
...
int give_your_domain(NO_SEGMENT no, void *p1, S_INITPARAMOUT *param_out)
{
   ...
   param_out->returned_state[param_out->response_number].mask = MSK_MDP
         | MSK_SWIPE | MSK_TYPE_PPAD | MSK_PINPAD | MSK_STANDARD | MSK_LANGUE
         | MSK_FRMT_DATE | MSK_DATE;
 ...
 * \see sdk30.h
 */
int after_reset(NO_SEGMENT no, void *p1, S_TRANSOUT *pParamOut)
{
   //unsigned char ChangeFlag, ChangeType;
   unsigned int nResult;

   // Clear the output parameter
   memset(pParamOut, 0, sizeof(*pParamOut));

   // Load the contactless DLL
   nResult = TPasslib_open(); // The return value of the ObjectLoad function.
   // 0 = Successful
   ...
   case ERR_TPASS_NOT_CONFIGURED:// =No contactless reader is configured in the manager
      DrawString(
            "TPass_IsConnected()\n=TPASS_NOT_CONFIGURED\n                                    \nNo contactless reader\nis configured in the   \nmanager.",
            0, 2, _OFF_);
      ttestall(0, 300);
      return STOP;
      break;

   case ERR_TPASS_DISCONNECTED: // = There is no contactless reader connected
      DrawString(
            "TPass_IsConnected()\n=ERR_TPASS_DISCONNECTED\n                                    \nThere is no contactless\nreader connected.        ",
            0, 2, _OFF_);
      ttestall(0, 300);
      return STOP;
      break;
...
int more_function(NO_SEGMENT no, void *p1, void *p2)
{
   ...
   do
   {
      ...
      {
      case 0:
         printf("Desfire Thunder\nRunning...");
         Sample_Desfire(0);
         bContinue = 0;
         break; // Sample_Desfire_Thunder selected
      case 1:
         printf("Desfire Format PICC\nRunning...");
         Sample_Desfire(1);
         bContinue = 0;
         break; // Sample_Format_PICC selected
      case 2:
         printf("MiFARE Authenticate Host\nRunning...");
         Sample_Desfire(2);
         bContinue = 0;
         break; // mifareSAM_AuthenticateHost selected
      case 3:
         printf("MiFARE Change Key Entry\nRunning...");
         Sample_Desfire(3);
         bContinue = 0;
         break; // Sample_ChangeKeyEntry selected
      case 4:
         printf("MiFARE Reset Key Entry\nRunning...");
         Sample_Desfire(3);
         bContinue = 0;
         break; // Sample_ResetKeyEntry selected
      case 5:
         printf("MiFARE free Context\nRunning...");
         Sample_Desfire(4);
         bContinue = 0;
         break; // Sample_FreeContext selected
      default:
         bContinue = 2;
