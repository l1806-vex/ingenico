...
#define LABEL_DOWNLOAD                     "SEQ_DWNLD"
#define LABEL_TICKET                       "SEQ_TICKET"
#define LABEL_MONTANT                      "SEQ_MONTANT"
#define LABEL_DATE                         "SEQ_DATE_SV"
#define LABEL_LICENCE                      "SEQ_LICENCE"     		/* PBE : Serveur Licence 04-979 */
#define LABEL_COLOR                        "SEQ_COLOR"
#define LABEL_PINPAD_MODE                  "SEQ_MODE"
...
#define SERVICE_NAVI_GESTIONNAIRE				  121
#define SERVICE_ETAT_MAINTENANCE 				  122
...
//! \brief Data structure for define the system pack coherence
typedef struct {	
  unsigned char version[CAR_MAX_PACK_VERSION];						//!< Version of the pack.
  unsigned char name[CAR_MAX_PACK_NAME];							//!< name of the pack.	
  unsigned char crc[CAR_MAX_PACK_CRC];								//!< CRC.
  unsigned char patch[CAR_MAX_PACK_PATCH];							//!< patch version of the pack manager.
  unsigned char amendement[CAR_MAX_PACK_AMENDEMENT];				//!< amendment version of the pack.
}Pack_Infos_t;
...
/*! @brief This function return the number of CAM on a configuration
 * - TERMINAL
 * - TERMINAL + PINPAD
 * @return
 *
 *   FUNCTION_NOT_IMPLEMENTED  if this function does not exit
 *   or Number of CAM
 *
 *   @include param_Sample_04.c
 *
 * @link KSFAM_PSQ Back to top @endlink
 */
extern int TM_GetNbCam(void);


/*! @brief This function returns :
 *  @param hard_card_reader : Pointer to the hardware version of card reader
 *  @param soft_card_reader : Pointer to the software version of card reader
 *
 * @return
 *
 *   FUNCTION_NOT_IMPLEMENTED if this function does not exit
 *   or 0
 *
 *  @include param_Sample_05.c
 *
 * @link KSFAM_PSQ Back to top @endlink
 */
extern int TM_GetCardReaderInfos(unsigned char *hard_card_reader,unsigned char *soft_card_reader);

/*! @brief This function returns :
 *  @param SizeFlash : The size of flash memory
 *  @param SizeRam : The size of ram memory
 *  @param RamFree : The size of Ram memory free
 *  @param FlashFreeCode : The size of Flash memory free in the code area
 *  @param FlashFreeData : The size of Flash memory free in the data area
 *  @param SizeBlocksKO: The number of block KO on the flash memory
 *
 * @return
 *
 *   FUNCTION_NOT_IMPLEMENTED if this function does not exit
 *   OK if the information is available
 *   KO if the information is not available
 *
 *   @include param_Sample_03.c
 *
 * @link KSFAM_PSQ Back to top @endlink
 */
extern int TM_GetInfoMemorySize(int *SizeFlash,int *SizeRam,int *RamFree,int *FlashFreeCode,int *FlashFreeData,int *SizeBlocksKO);
