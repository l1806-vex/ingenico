 ...
 * \deprecated Functions to manage assertion.
 *
 *
 * \author Ingenico
...
//! \addtogroup VGE_DBG
//! @{
//!	* @deprecated	DBG is deprecated since SDK 9.2.	Use functions from GTL library.
...
#define ASSERT_FAILED_DIAG		0xffff		/*!< \deprecated Diagnostic generated when an assertion fails. */
...
//! \deprecated If \a __USE_ASSERT__ is defined, it checks that the condition \a test is \a TRUE. If the condition is \a FALSE, <b>the execution is stopped</b>.<br>
...
//! \deprecated If \a __USE_ASSERT__ is defined, it checks that the condition \a test is \a OK (=0). If the condition is not \a OK, <b>the execution is stopped</b>.<br>
//! If \a __USE_ASSERT__ is not defined, the condition <b>\a test is compiled</b> but <b>the result is not tested</b>.\n
//! When assertion is false, this function proposes to developpers to continue, to do a breakpoint, to reset the terminal or to print the assertion.
//! In case of brakpoint chosen, on Ingedev it is not possible to continue to run the code.
...
//! \endcond
	...
	int __VGE_DBG_Assert_Test (const int bTest, const char * szCondition, const char * szFile, const int nLine) AVL_DEPRECATED ;
