...
//!
//! If the secret area has the format C_SEC_TLV or not exist on BL2 the scheme to call is "TlvMAKeyGen", otherwise an error is returned.\n
...
//! <TR><TD></TD></TR>
//! <TR><TD>  <strong> WARNING: </strong> <b> The application must verify the integrity and the authenticity of the RSA public Key by using the signature Telium system verification. </b></TD></TR>
//! <TR><TD></TD></TR>
//! </TABLE>
//!
...
//! The key block to input for the RSA public key encryption is the following MSB first:
//! -	1 byte set to 0x6A
//! -	1 byte to 0x00
//! -	the random generated key (size according to the 3rd parameter)
//! -	random bytes for padding ((modulus length in bytes – 3 – key length) bytes).
//! -	1 byte set to 0xBC
...
//! If the secret area has the format C_SEC_TLV the scheme to call is “TlvHMac”. Otherwise an error is returned. .\n
...
//! \param[in] iKeyMACIndex MAC key value index : part of the MAC key to use (0 for the 1rst part or 1 for the 2nd part of the MAC key value or 2 for the all key)
...
//! 
//! If the secret area has the format C_SEC_TLV the scheme to call is “TlvAESCiph”. Otherwise an error is returned..\n
...
//! \brief SEC_LoadKeyHMAC() To load a MAC key in a secret area (key Digest HMAC-SHA224 or HMAC-SHA256).  
//!  The key value to load is DES/TDES ciphered (ECB mode).
//!  The length of the ciphering key must be greater than the length of the MAC key to load.
//!  The TDES ciphering key can be 16 bytes length (TLV_TYPE_KTDES) or 24 bytes length (TLV_TYPE_KTDES_24).
//!
//!  The MAC key can be treated as 2 parts; it can be used to compute a HMAC-SHA256 or a digest HMAC-SHA224 (see DLL digest).
//!
//!  If the secret area has the format C_SEC_TLV or does not exist on BL2 and more, the scheme to call is "TlvLoadHMAC", otherwise an error is returned.
//!
//!  Scheme to sign with your application: NAM_TLV_LOAD_HMAC "TlvLoadHMAC"
//!
//! \param[in] secureType Secure part to use to store the MAC key
//! \param[in] ptHMACKeyId MAC Key Id of the key to load (cAlgoType has to be set to TLV_TYPE_MAC)
//! \param[in] ptCipherKeyId Ciphering Key Id (cAlgoType has to be set to  TLV_TYPE_KDES, TLV_TYPE_KTDES or TLV_TYPE_KTDES_24)
//! \param[in] pt_InputData DES/TDES Ciphered MAC key value (ECB mode) (its length is 8, 16 or 24 bytes)
//! \param[in] iUsage MAC key usage; values are defined in tlvVar_def.h (CIPHERING_KEY, CIPHERING_PIN or CIPHERING_DATA)
//! \param[in] uiHMACKeyLength Length in bytes of the MAC Key to load
//! \return OK (0) is successful, otherwise error
int SEC_LoadKeyHMAC (T_SEC_PARTTYPE secureType, T_SEC_DATAKEY_ID * ptHMACKeyId, T_SEC_DATAKEY_ID * ptCipherKeyId,
					unsigned char * pt_InputData, int iUsage, unsigned int uiHMACKeyLength);
