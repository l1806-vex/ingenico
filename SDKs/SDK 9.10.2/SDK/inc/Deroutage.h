...
#ifndef __DEROUTAGE_H_INCLUDED__
#define __DEROUTAGE_H_INCLUDED__
...
typedef  struct
{
	...
	unsigned char IndexComUsb;					/*USB COM index to use*/
	unsigned char FlagEnd;					
}DEROUTE_T;
...
typedef struct 
{
	...
	unsigned char usb_slave_base;	//TRUE - FALSE
	unsigned int radio_mask;	//Radio mask (comes from SYS_FIOCTL_GET_RADIO_MASK)

	unsigned char product_name[16];		//Product Name : string with \\0
	unsigned char manuf_config[32];		//Manufacturing Config : string with \\0
	unsigned char product_code[4]; 		//Product code (ex : M40) : string with \\0
	unsigned char activ_profile[4];		//Activation profile (ex : A98) : string with \\0
	unsigned char serial_number[9];		//Serial number : string with \\0
	unsigned char ruf1[3];		//ruf

	unsigned int flash_size;
	unsigned int ram_size;
	unsigned int booster_type;	//BOOSTER_2... (comes from SYS_FIOCTL_GET_SECURE_UNIT_TYPE)

	int pci_pts_version;			//PCI_PTS_V2 - PCI_PTS_V3 (comes from SYS_FIOCTL_GET_PCI_PTS_VERSION)
	int ingestrust_type; 			//INGETRUST_TYPE_NONE... (comes from SYS_FIOCTL_GET_INGETRUST_TYPE)
...
#define START_PMD_USB_SOCLE			16		/*to start usb pmd (COMU) */
#define PRINTER_PMD_GR				17		/*To send graphical tickets*/
...
#endif   // __DEROUTAGE_H_INCLUDED__
