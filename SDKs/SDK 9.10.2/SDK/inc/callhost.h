...
#ifndef __CALLHOST_H_INCLUDED__
#define __CALLHOST_H_INCLUDED__
...
/*! @brief Remote call with using a PPP channel established by application ( \link CALLHOST_SAMPLE Sample \endlink)*/
...
typedef struct {
  ...
  void            *handler;       /*!< OEMC file handler */
} S_CALLBACK_CONNECT;
...
/*! @brief Define the system call extended parameters (used with \ref REMOTE_IP_EXTENDED). The members are right filled with space. Before setting, the struct should be set with 0x20 */
typedef struct
{
T_CALL_NUMBER  CallNumber;										/*!< number to call Host:
...
#include "callhost_def.h"
#ifdef __FRAMEWORK_TELIUM_2__
#include "callhost_deprecated.h"
#endif


#endif   // __CALLHOST_H_INCLUDED__
