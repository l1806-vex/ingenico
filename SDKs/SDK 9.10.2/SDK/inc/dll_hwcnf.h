...
//! \defgroup HWCNF_SWIPE Swipe Management
...
//! \defgroup HWCNF_APPLE_IAP Apple IAP
//! @{
		//! \defgroup HWCNF_APPLEBT_ERR Apple Bluetooth link error codes
//! @}

//! \defgroup HWCNF_APPLE_SPP Apple SPP
//! @{
		//! \defgroup HWCNF_APPLEBT_ERR Apple Bluetooth link error codes
//! @}
...
//! \defgroup HWCNF_BLUETOOTH
    //! @{
         //! \defgroup HWCNF_OPEN_BT
    //! @}
...
/*!
 * @brief Obtain Mac Address both for Bluetooth and non-Bluetooth device, included Wifi.
 * @param mac_addr : pointer to an array of 6 char
 * @return
 *  @li  0 if parameters could be obtained.
 *  @li -1 ETHERNET_LOAD_ERR.
 *  @li -2 ETHERNET_LOAD_MOUNT_FAILED.
 *  @li -3 ETHERNET_LOAD_OPEN_FAILED.
 *
...
#define HWCNF_BTPRINTER_ERR_NOT_SUPPORTED	(-2)	/*!< The external Bluetooth printer management is not supported*/
#define HWCNF_BTPRINTER_ERR_DISABLED		(-3)	/*!< The external Bluetooth printer is disabled*/
...
//! @brief parameters of \ref HWCNF_ConfigureExternalPrinter (to display Bluetooth printer settings from an application )
//! @{
...
#define HWCNF_BTPRINTER_ENABLED				1	//!< external Bluetooth printer enabled - iSPM only
#define HWCNF_BTPRINTER_DISABLED				0	//!< external Bluetooth printer disabled - iSPM only
...
//! @addtogroup HWCNF_SWIPE
//! @brief Swipe configure
//! @{

#define HWCNF_GPRS_FILTER_OFF       0	/*!< GPRS filter OFF (default value) */	
#define HWCNF_GPRS_FILTER_ON		1	/*!< GPRS filter ON */

#define HWCNF_GPRS_FILTER_OK         0
#define HWCNF_GPRS_FILTER_BAD_PARAM  1

/*!
 * @brief Set the GPRS filter for SWIPE reading
 * @param handle Handle to SWIPE peripheral
 * @param value  Value \ref HWCNF_GPRS_FILTER_ON to enable \ref HWCNF_GPRS_FILTER_OFF to disable
 * @return \ref HWCNF_GPRS_FILTER_OK if successful else \ref HWCNF_GPRS_FILTER_BAD_PARAM
 * @note To ensure compatibility with the existing filtering gprs is active by default after each fopen.
*/
#ifdef __FRAMEWORK_TELIUM_PLUS__
extern int HWCNF_SetGPRSFilter(Telium_File_t *handle,int value);
#else
extern int HWCNF_SetGPRSFilter(FILE *handle,int value);
#endif
...
//! @addtogroup HWCNF_APPLEBT_ERR
//! @{

#define HWCNF_APPLEBT_OK					0		/*!< OK  */
#define HWCNF_APPLEBT_ERR_BAD_PARAM		(-1)		/*!< Bad parameter */
#define HWCNF_APPLEBT_ERR_NOT_SUPPORTED	(-2)		/*!< This link is not supported*/
#define HWCNF_APPLEBT_ERR_DISABLED		(-3)		/*!< This link is disabled -> Bluetooth address not set*/
#define HWCNF_APPLEBT_ERR_NOT_IMPLEMENTED	(-4)	/*!< The dll needs to be updated*/
#define HWCNF_APPLEBT_ERR_NOT_LOADED		(-9999)	/*!< dll not loaded*/	
//! @}



//! @addtogroup HWCNF_APPLE_SPP
//! @brief Apple SPP settings (iSMP only)
//! @{

/*!
 * @brief Get the Bluetooth address of the Apple SPP link
 * @param bt_string : the address of the bluetooth device (string 20bytes with 0) -> "XXXX:XX:XXXXXX"
 * @return \ref HWCNF_APPLEBT_OK	if successful else \ref HWCNF_APPLEBT_ERR
 * 
*/
extern int HWCNF_GetAppleSPPBluetoothAddressString(char bt_string[20+1]);

/*!
 * @brief Set the Bluetooth address of he Apple SPP link
 * @param bt_string : the address of the bluetooth device (string 20bytes with 0) -> "XXXX:XX:XXXXXX"
 * @param bt_pincode : pincode (string with 0) -> "1234"
 * @return \ref HWCNF_APPLEBT_OK	if successful else \ref HWCNF_APPLEBT_ERR
 * @note if bt_string=NULL, the SPP link is disabled (= remove device).
 * 
*/
extern int HWCNF_SetAppleSPPBluetoothAddressString(char bt_string[20+1],char bt_pincode[20+1]);

//! @}


//! @addtogroup HWCNF_APPLE_IAP
//! @brief Apple IAP settings (iSMP-C only)
//! @{

/*!
 * @brief Get the Bluetooth address of the Apple IAP link
 * @param bt_string : the address of the bluetooth device (string 20bytes with 0) -> "XXXX:XX:XXXXXX"
 * @return \ref HWCNF_APPLEBT_OK if successful else \ref HWCNF_APPLEBT_ERR
 * 
*/
extern int HWCNF_GetAppleIAPBluetoothAddressString(char bt_string[20+1]);

/*!
 * @brief Set the Bluetooth address of he Apple IAP link
 * @param bt_string : the address of the bluetooth device (string 20bytes with 0) -> "XXXX:XX:XXXXXX"
 * @param bt_pincode : pin code (string with 0) -> "1234"
 * @return \ref HWCNF_APPLEBT_OK	if successful else \ref HWCNF_APPLEBT_ERR
 * @note if bt_string=NULL, the IAP link is disabled (= remove device)
 * 
*/
extern int HWCNF_SetAppleIAPBluetoothAddressString(char bt_string[20+1],char bt_pincode[20+1]);
...
//! @addtogroup HWCNF_OPEN_BT
//! @brief Open BT configure
//! @{

/*!
 * @brief Return the local address of the bluetooth device
 * @param addr : addr the address of the bluetooth device
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_GetLocalAddress (dbluetooth_device_addr_s *addr);

/*!
 * @brief Return the server channel on the OPEN_BT connection
 * @param server_channel_ptr: pointer
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_OpenBluetooth_GetServerChannel (int *server_channel_ptr);

/*!
 * @brief Set the visibility of the device on the OPEN_BT connection
 * @param visibility : visibility of the device
 *  - OPEN_BT_VISIBILITY_VISIBLE   device becomes visible to Bluetooth scan request
 *  - OPEN_BT_VISIBILITY_INVISIBLE device becomes invisible to Bluetooth scan request
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_Pairing_SetVisibility(int *visibility);

/*!
 * @brief Set the PIN code for the OpenBT Bluetooth link
 * @param pincode : PIN code for the OpenBT Bluetooth link
 *  - pincode value
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_SetPincode(char *pincode);

/*!
 * @brief Set the name for the OpenBT Bluetooth link
 * @param name : name for the OpenBT Bluetooth link
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_OpenBluetooth_SetName(char *name);
 ...
 * - other if unsuccessful.
 */
extern int HWCNF_OpenBluetooth_GetName(char *name);


/*!
 * @brief Remove a pairing to an identified device
 * @param addr : the address of the bluetooth device
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_RemovePairedDevice(dbluetooth_device_addr_s *addr);

/*!
 * @brief returns the max number of device may be paired
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - number max set to 32.
 *
 */
extern int HWCNF_Bluetooth_GetMaxNumberOfPairedDevice(void);

/*!
 * @brief This function get the friendly name of a pairing device
 * @param Id : Id of the device in the paired device list ( 0 to 31 )
 * @param friendly_name : address of the buffer
 * @param sizeoffriendlyname : size of buffer (size max of a friendly name set to 44)
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * -
 * - pointer on the buffer.
 */
extern char* HWCNF_Bluetooth_GetFriendlyNameOfPairedDevice(int Id,char* friendly_name,int sizeoffriendlyname);


/*!
 * @brief Get the address of a paired device
 * @param Id : Id of the device in the paired device list ( 0 to 31 )
 * @param addr : the address of the bluetooth device
 *
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - OK if succesful.
 * - KO if the Id is unknown.
 */
extern int HWCNF_Bluetooth_GetAdressOfPairedDevice (int Id, dbluetooth_device_addr_s *addr);

/*!
 * @brief Get the Class of a paired device
 * @param Id : Idof the device in the paired device list ( 0 to 31 )
 ...
 * - KO if the Id is unknown.
 */
extern int HWCNF_Bluetooth_GetClassOfPairedDevice (int Id, int *classOfDevice);

/*!
 * @brief Start a search for discoverable devices
 * @param cod : Class Of Device to search (Set to 0 to search all Class)
 * @param cod_mask : Mask on Class Of Device (Set to 0 to disable the mask) 
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_StartSearchDiscoverableDevice(int cod, int cod_mask);

/*!
 * @brief Get a list of discoverable devices
 * @param time_out : waiting time for the search (0 to 180 seconds max)
 * @param results_list : number and list of devices
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_GetListOfDiscoverableDevice(int time_out,dbluetooth_search_results_s *results_list);


/*!
 * @brief Stop a search for discoverable devices
 * @return
 * - FUNCTION_NOT_IMPLEMENTED if function does not exist
 * - 0 if succesful.
 * - other if unsuccessful.
 */
extern int HWCNF_Bluetooth_StopSearchDiscoverableDevice(void);
