...
#ifndef __LOADER_API_DEF_H_INCLUDED__
#define __LOADER_API_DEF_H_INCLUDED__
 ...
 *
 * This function read full uncompressed object at each call. 
 * Object must be first oppened by ObjectOpen. 
 * ObjectSeek must not be used with this function. 
 *
 * @param buffer : pointer to area to store data.
 * @param size : buffer size in bytes. Must be large enough to receive full uncompressed object.
 * @param obj_fd : pointer to handle returned by ObjectOpen.
 * @return 
 * - 0 if no error. Else :
 * - 1 : object not found
 * - 3 : no enough memory
...
#endif   // __LOADER_API_DEF_H_INCLUDED__
