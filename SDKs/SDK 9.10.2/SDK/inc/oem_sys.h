...
#ifndef __OEM_SYS_H_INCLUDED__
#define __OEM_SYS_H_INCLUDED__

#include "oem_sys_def.h"
#include "uiram.h"
...
#define	CANAL_LOG		 	            23		/*!< LOG IUC channel */
#define CANAL_TKBD                     24
#define	NB_MAX_CANAUX		 	        25		/*!< Max IUC channel number */
...
#define EVENT_BL3_LOG                     0x00000200		/*!< BL3 log event */
#define EVENT_TKBD                        0x00000400		/*!< Touch Keyboard event */
#define EVENT_RUF3                        0x00000800		/*!< Ruf3 event */
#define EVENT_SEND_IAPP_PINPAD            0x00001000		/*!< Send on Pinpad Iapp event */
...
#define EVENT_SCHEMA_PINPAD				  0x08000000		/*!< Pinpad scheme event */
#define EVENT_SCHEMA_USB_READER			  0x10000000		/*!< Reader USB scheme event */
#define EVENT_SEND_IAPP2                  0x20000000		/*!< Send on Iapp event */
#define EVENT_SCHEMA_PINPAD2			  0x40000000		/*!< Pinpad scheme event */
#define EVENT_SEND_IAPP_PINPAD2           0x80000000		/*!< Send on second Pinpad Iapp event */
...
// ---------------------------------------------------------------------
// Trame de Gestion du RESET_24H
// ---------------------------------------------------------------------
typedef struct
{
   unsigned char   SousCmd;             // sous commande RESET_24H : SET / STATUS
   unsigned char   Ruf;                 // Ruf (pour combler le trou)
   unsigned char   Status;              // Status du RESET_24 (ON=1)
   unsigned char   IsMockup;            // Etat Mockup ou non de l'apareil (Mockup=1)
   unsigned int    Counter;             // Valeur restante du compteur 24 heure (en secondes)
...
typedef struct
{
    BUF_ENTETE_T              	EnteteTrame;
    unsigned char             	Cmd;
    MODE_RESET_24H_T            ModeReset24H;
}
ENCODE_BUF_MODE_RESET_24H_T;


// ---------------------------------------------------------------------
// Trame de Validation/Invalidation du LeverLock
// ---------------------------------------------------------------------
typedef struct
{
   unsigned char   SousCmd;             // sous commande Lever Lock : ON / OFF
   unsigned char   Ruf[3];              // Ruf (pour combler le trou)
   int             CptRenduSousCmd;     // Compte rendu de la sous command
} MODE_LEVER_LOCK_T;
...
typedef struct
{
    BUF_ENTETE_T              	EnteteTrame;
    unsigned char             	Cmd;
    MODE_LEVER_LOCK_T 			ModeLeverLock;
}
ENCODE_BUF_MODE_LEVER_LOCK_T;


// ---------------------------------------------------------------------
// Trame de Validation/Invalidation de l'écran arrière
// ---------------------------------------------------------------------
typedef struct
{
   unsigned char   SousCmd;             // sous commande SetBackScreen : ON / OFF
   unsigned char   Ruf[3];              // Ruf (pour combler le trou)
   int             CptRenduSousCmd;     // Compte rendu de la sous command
} SET_BACK_SCREEN_T;
...
typedef struct
{
    BUF_ENTETE_T              	EnteteTrame;
    unsigned char             	Cmd;
    SET_BACK_SCREEN_T 			SetBackScreen;
}
ENCODE_BUF_SET_BACK_SCREEN_T;


/* ------------- *
 * -- LOG BL3 -- *
 * ------------- */
#define LOG_PATH           "/SYSTEM/BOOSTER3LOG.DIA"
#define LOG_SIZE           1024*16 //16k

// ----------------------------------------------------
// Definition des tailles de LOG
// ----------------------------------------------------
#define LOG_BUFFER_SIZE    96
#define LOG_HEADER_SIZE    32
#define LOG_MSG_SIZE       (LOG_BUFFER_SIZE - LOG_HEADER_SIZE) /* 64 bytes */

// Structure d'un log
// ------------------
typedef struct _LOG_MSG_CONTENT_T{
	char           Header [LOG_HEADER_SIZE];
	char           Content[LOG_MSG_SIZE];
}LOG_MSG_CONTENT_T;

// Union {(Message brute) ou (header+content)}
// -------------------------------------------
typedef union _LOG_MSG_U{
	LOG_MSG_CONTENT_T Decode;
	char              Brute[sizeof(LOG_MSG_CONTENT_T)];
}LOG_MSG_U;

// trame LOG (entete+data)
// -----------------------
typedef struct
{
    BUF_ENTETE_T     EnteteTrame;
...
// Union {(Trame brute) ou (Entete + Data)}
// ----------------------------------------
typedef union
{
    unsigned char    DataBrute[sizeof(ENCODE_BUF_LOG_T)];
    ENCODE_BUF_LOG_T DataDecode;
}
BUF_LOG_T;
/*
============================================
...
#define SYSTEME_CMD_MODE_LEVER_LOCK		 	  0xA4  /*!< System command to Set or Unset the Lever Lock of the iUR250    */
   #define _LEVER_LOCK_OFF				 (0x00)		/*!< System command to Set the Lever Lock of the iUR250    */
   #define _LEVER_LOCK_ON				 (0x01)		/*!< System command to Unset the Lever Lock of the iUR250    */
#define SYSTEME_CMD_GET_INGETRUST_LEVEL	 	  0xA5  /*!< System command to Get the Ingetrust level */

#define SYSTEM_CMD_ENABLE_POINT_JAUNE         0xA6  /*!< System command to enable the yellow dot comb -NOT TO BE USED- (specific configuration) */

#define SYSTEM_CMD_GET_WAKEUP_EVENT           0xA7  /*!< System command to get the touch wakeup event -NOT TO BE USED- : iWB specific configuration */
#define SYSTEME_CMD_SET_BACK_SCREEN		 	  0xA8  /*!< System command to Switch On or Switch Off the Back Screen of the iUC180B    */
   #define _BACK_SCREEN_OFF				 (0x00)		/*!< System command to Switch Off the Back Screen of the iUC180B    */
   #define _BACK_SCREEN_ON				 (0x01)		/*!< System command to Switch On the Back Screen of the iUC180B    */
#define SYSTEME_CMD_COMMISSIONING_LOG		  0xA9  /*!< System command to Write/Read the Commissioning Log */

#define SYSTEME_CMD_QUOTA_CLESS         0x41        /*!< System command to change Cless behwviour, used to stop field after each polling loop */

#define SYSTEME_CMD_RESET_24H                 0xAA  /*!< System command to Set the RESET 24H  Mode   */
   #define _RESET_24H_SET                    (0x69) /*!< System command to Set the "RESET 24H        */
   #define _RESET_24H_STATUS                 (0x26) /*!< System command to Get the "RESET 24H status */
...
#define SYSTEME_SOUS_CMD_MAINTENANCE_START_SECURED_INTER_UC_LINK  0x00000006 /*!< internal use only FT11643 */
		...
		#define SYSTEME_SOUS_CMD_MAINTENANCE_MANUFACTORING       0x00000003		/*!< Manufactoring maintenance command */
				...
				#define SYSTEME_SCM_MANUFACTORING_FLASH_LOCK_STATE         0x00000007		/*!< Command to get the flash lock state */
...
#define SYSTEM_CMD_DISABLE_POINT_JAUNE_PP 0x1A		/*!< System command to disable the yellow dot comb on pinpad -NOT TO BE USED- (specific configuration) */
#define SYSTEM_CMD_ENABLE_POINT_JAUNE_PP  0x1B		/*!< System command to enable the yellow dot comb on pinpad */
...
#define SYSTEME_CMD_RUF_1_C30				  0x66		/*!< Command to do a ruf1 action on C30 */
#define SYSTEME_CMD_RUF_1_CAD30USR			  0x76		/*!< Command to do a ruf1 action on External Reader */
...
#define FONCTION_PERIODIQUE_VISUPPR2           		9		/*!< Periodical function number of second pinpad display */
#define FONCTION_PERIODIQUE_CLAVIERPPR2				10		/*!< Periodical function number of second pinpad keyboard */

#define	NB_MAX_FONCTIONS_PERIODIQUES         		11		/*!< Maximum number of periodical functions */
...
#define SWIPE_CMD_DATA_ALTERNATE_HEAD 11	/*!< Cmd field value :ISO incoming data (origin=optionnal BL3 alternate head) */
#define SWIPE_CMD_GPRS_FILTERING_SET 12		/*!< Cmd field value :set/clr gprs filtering */
...
#define CLNFCF_CMD_GET_VERSION   0x44       /*!< get version of NFCF driver */
...
//CLESS NFCF
#define CLESS_CMD_NFCFDETECT     0x61
#define CLESS_CMD_NFCFACTIVATE   0x62
#define CLESS_CMD_NFCFAPDU       0x63
#define CLESS_CMD_NFCFDESELECT   0x64
#define CLESS_CMD_NFCFRELEASE    0x65
#define CLESS_CMD_NFCFOPEN       0x66
#define CLESS_CMD_NFCFCLOSE      0x67
#define CLESS_CMD_NFCFXX_FORMAT  0x68
...
#define TSCREEN_CMD_GET_FIRMWARE_VERSION  0x0A  /*!< Get Firmware version of K2 SiP chip */
#define TSCREEN_CMD_GET_CHIP_TYPE         0x0B  /*!< Get type of chip TH<-BL */
#define TSCREEN_CMD_FIRMWARE_DATA         0x0C  /*!< Sending the block of new firmware to the booster */
...
#define TSCREEN_CMD_HW_RESET_MXT          0x23  /*!< Hardware Reset MXT224 Chip */
#define TSCREEN_CMD_SET_DRAWING_AREA   0x24  /*!< Configure the effective drawing area */
#define TSCREEN_CMD_SET_STYLUS            0x25  /*!< Tell Booster tscreen driver that BStylus is used */
...
#define TSCREEN_ONE_MATRIX_K2_SZ    64 /*!< Number of bytes in K2 Cirque Compensation matrix */
...
typedef enum
{
   ...
   TSCREEN_PANEL_RP02_EXT_K2        = 5,               /*!< RP02, extended version of panel, K2 chip */
   /*------------------------*/
   TSCREEN_PANEL_UNKNOWN  = 255              /*!< Unknown panel */
}
TSCREEN_PANEL_TYPE_T;

/*! @brief Enumeration data type for different chips used for touch screen implementation */
typedef enum
{
   TSCREEN_CHIP_PINNACLE     = 0,                     /*!< Pinnacle (tm) Cirque */
   TSCREEN_CHIP_MXT224       = 1,                     /*!< MXT224 (tm) Atmel */
   TSCREEN_CHIP_K2           = 2,                     /*!< K2 SiP (tm) Cirque */
   TSCREEN_CHIP_UNKNOWN      = 255                    /*!< Unknown chip */
}
TSCREEN_CHIP_TYPE_T;

/*! @brief Enumeration data type for different stylus available */
typedef enum
{
   TSCREEN_STYLUS_STD       = 0,                     /*!< Standard 4mm stylus for Cirque Chip */
   TSCREEN_STYLUS_BIO_V1    = 1,                     /*!< Biometric stylus V1 for Cirque chip */
   TSCREEN_STYLUS_UNKNOWN   = 255                    /*!< Unknown chip */
}
TSCREEN_STYLUS_TYPE_T;
...
/*! @brief Firmware block */
typedef struct
{
   unsigned int address;      /*!< address in updatable chip */
   unsigned int length;       /*!< length of data */
   unsigned char data[512];   /*!< firmware data */
}
TSCREEN_FWBLOCK_T;

/*! @brief  Touchscreen area format */
typedef struct
{
   unsigned char xorigin;	      /*!< X coordinate */
   unsigned char yorigin;        /*!< Y coordinate */
   unsigned char xsize;          /*!< X size */
   unsigned char ysize;          /*!< Y size */
}
TSCREEN_AREA_T;
...
typedef union
{
   ...
   unsigned short             fwversion;  /*!< Firmware version of K2 SiP chip */
   ...
   TSCREEN_CHIP_TYPE_T        chiptype;   /*!< Type of chip */
   ...
   TSCREEN_FWBLOCK_T          fw_data;
   TSCREEN_AREA_T             drawingArea;  /*!< Drawing area definition */
   TSCREEN_STYLUS_TYPE_T      stylus;       /*!< Stylus type currently plugged */
...
#define SECURITE_CMD_BL3_EMUL_PINPAD 0x04        /*!< internal used only */
#define SECURITE_BL3_PINPAD_NOT_SECURED 0x00     /*!< internal used only */
#define SECURITE_BL3_PINPAD_SECURED     0x01     /*!< internal used only */
#define SECURITE_BL3_PINPAD_EMUL_MODE_ON 0x02     /*!< internal used only */
/**
\endcond */
...
#define MAX_TKBD_LINE      60
#define TKBD_DUMP_NOTREQ  0x00
#define TKBD_DUMP_REQ     0x01


#define TKBD_DUMP_FILE	 "/SYSTEM/TKBD_DUMP.TXT"
#define TBKD_DUMP_SIZE   2048
...
{
   BUF_ENTETE_T  EnteteTrame;           /*!< InterUC format */
   TKBD_UCPACK_T pack[MAX_TKBD_LINE];   /*!< Touch screen command*/
   unsigned char nb;
   unsigned char cmd;
} TKBD_UCDECODE_T;

/*! @brief  Touch keyboard buffer to be sent TH<->BL */
typedef union
{
    unsigned char    DataBrute[sizeof(TKBD_UCDECODE_T)];
    TKBD_UCDECODE_T  DataDecode;
} TKBD_UC_T;

/*! @}  */


#endif   // __OEM_SYS_H_INCLUDED__
