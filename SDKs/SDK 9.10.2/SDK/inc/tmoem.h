...
#ifndef TM_OEM_MANAGER_H
#define TM_OEM_MANAGER_H
...
//! \brief Define access type to DLL via DllLink
typedef void* (*DllGetProcAddressType)(char*);

/*+++******* EXPORTED TYPES ****************************************---*/
// Naming convention: xyz_AbcdeFgh_t


#ifdef __FRAMEWORK_TELIUM_PLUS__
/*! @brief Define a file descriptor handle */
typedef struct Telium_File_t Telium_File_t;
//struct Telium_File_Struct;
//typedef struct Telium_File_Struct * Telium_File_t;
//#define Telium_File_t FILE

/*! @brief Define a DATE descriptor*/
...
#define PARAM_FILE_GRAPHIC_MANAGER_BW 0xAD54
...
#else
int  SV_power_down(Telium_File_t *handle);
#endif
...
#else
int  SV_input_command(Telium_File_t *handle,COMMAND_CAM *inputcommand);
#endif
...
#else
int  SV_output_command(Telium_File_t *handle,COMMAND_CAM *outputcommand);
#endif
...
#else
int  SV_power_on(Telium_File_t *handle,HISTORIC *historical);
#endif
...
#else
int  SV_read_date(Telium_Date_t *date);
#endif
...
#else
int  SV_write_date(Telium_Date_t *date);
#endif
...
/*! @brief Calls the TMS for remote software iPP target if the emulation mode is enable
* @param param_telechgt : The list of required/optional parameters depends on the network
*  type acceded ( [] : optional parameter). :
*  <table cellpading="0" cellspacing="0" border="1" width="100%">
*  <tr><th> 'reseau' </th><th> t_noappel </th><th> t_transpac </th><th> apn </th><th> t_login </th><th> t_password</th></tr>
...
int remote_IPP (S_PARAM_TLCHGT *param_telechgt);


/*! @brief This function Gets the activity report value returned by Ingestate at the last connection.
 * @return
 * - 0 : OK
 * - -1 : not implemented
 * - Other value returned by Ingestate
 *
 * @link KSFAM_M2OS Back to top @endlink
*/
int TM_GetLastCallhostActivityReport(void);
...
#define C_CONF_IPP285	9
#define C_CONF_IPP310	10
