...
static bool OnPrint(T_GL_HMESSAGE message)
{
	...
	// IMPORTANT : Sets the mininal size of the signature
	GL_Widget_SetMinSize(signature, 384, 100, GL_UNIT_PIXEL);
...
void GL_SampleSignature(T_GL_HGRAPHIC_LIB graphicLib)
{
	...
	mainLayout = GL_Layout_Create(window);
		...
		signature = GL_Signature_Create(mainLayout);
			...
			// Force the signature to fit in space available
			GL_Widget_SetExpand(signature, GL_DIRECTION_ALL);

			// Set the margins arround the signature
