...
// Sample of skin modification
void GL_SampleSkinModification(T_GL_HGRAPHIC_LIB graphicLib)
{
	T_GL_HWIDGET window;
	T_GL_HWIDGET button;
	T_GL_HWIDGET layout;
	T_GL_HWIDGET skinButton;

	// Gets the skin handle of the button
	skinButton = GL_GraphicLib_GetSkinWidget(graphicLib, GL_TYPE_BUTTON);

	// Change the focus image of the button
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_FOCUS_IMAGE_Oxxxx), "file://flash/HOST/TU.TAR/wgusuite/ButFocus(left).wgu");
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_FOCUS_IMAGE_xxOxx), "file://flash/HOST/TU.TAR/wgusuite/ButFocus(center).wgu");
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_FOCUS_IMAGE_xxxxO), "file://flash/HOST/TU.TAR/wgusuite/ButFocus(right).wgu");

	// Change the background image of the button
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_BACK_IMAGE_Oxxxx), "file://flash/HOST/TU.TAR/wgusuite/Button(left).wgu");
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_BACK_IMAGE_xxOxx), "file://flash/HOST/TU.TAR/wgusuite/Button(center).wgu");
	GL_Widget_SetSource(GL_Widget_SearchId(skinButton, GL_ID_SKIN_BACK_IMAGE_xxxxO), "file://flash/HOST/TU.TAR/wgusuite/Button(right).wgu");
	...
	layout = GL_Layout_Create(window);
		...
		// Creating the first button with the skin modified
		...
		// Creating second button with the skin modified
		button = GL_Button_Create(layout);
		...
		// Force reloading the skin button
		GL_Widget_Destroy(skinButton);

		// Creating third button with the default skin
		button = GL_Button_Create(layout);
