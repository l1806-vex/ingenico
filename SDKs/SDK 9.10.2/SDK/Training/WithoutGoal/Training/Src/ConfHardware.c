...
//       INGENICO                                INGEDEV 7                   
//============================================================================
//       FILE  CONFHARDWARE.C                      (Copyright INGENICO 2011)
//============================================================================
...
// SYS_FIOCTL_GET_ICC_READER_HARD_VERSION => ICC reader hardware version:4 digits ASCII.
// SYS_FIOCTL_GET_ICC_READER_SOFT_VERSION => ICC reader software version:4 digits ASCII.
// SYS_FIOCTL_BOOSTER_GET_ACTIVATION_MANUFACTURING_DATE => Get manufacturing date.
// SYS_FIOCTL_BOOSTER_GET_ACTIVATION_PROFILE => Get profile.
// SYS_FIOCTL_REPUDIATION_INFO_GET => Get list of repudiation keys.
...
// SYS_FIOCTL_GET_SECURITY_FIRMWARE_ID => Get security firmware version parameter.
// SYS_FIOCTL_BOOSTER_GET_ROM_VERSION => Get Booster ROM version.
// SYS_FIOCTL_THUNDER_GET_ROM_VERSION => Get Thunder ROM version.
// Add _DEPRECATED_OEM_MODEM_SDK091000_ inside Properties\Build Configurations\Users Symbols
// if needed and not yet replaced.
// MODEM_FIOCTL_IDENT => Gets modem identification.
#include "SDK30.H"
#include "Training.h"
...
//  regarding the hardware of the terminal.
...
void ConfHardware(doubleword uiCom)
{
    ...
    if (IsMODEM() == 1)
    {
    	pxModem = fopen("MODEM", "r*"); CHECK(pxModem!=NULL, lblKO);
...
lblEnd:
	if (pxModem)
		fclose(pxModem);                                                                         // Close "modem" channel
