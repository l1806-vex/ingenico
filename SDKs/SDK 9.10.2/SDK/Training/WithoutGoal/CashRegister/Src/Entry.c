...
//      PRIVATE DATA                                                        
//****************************************************************************
static const char zAppName[] = "CASHREG";
...
int more_function( NO_SEGMENT no, void *p1, void *p2 )
{
	...
	if (pxDsp != NULL)
	{
		...
		DisplayMedium(0, 0, " Cash Register  ");
		DisplayMedium(1, 0, "    Listener    ");
		DisplayMedium(2, 0, "      COM5      ");
		DisplayMedium(3, 0, "  115200,N,8,1  ");
		ttestall(0, 5*100);             // Wait 2s
...
int message_received(NO_SEGMENT no, S_MESSAGE_IAM *param_in, void *p2)
{
	...
	if (param_in->type == 2)                                                         // *** IAM message received from task ***
	{
		...
		CHECK(iRet>=0, lblKO);
		iRet = Print24((char*)param_in->value, param_in->length);                    // Print the message received
		CHECK(iRet>=0, lblKO);
		iRet = pprintf("\n\n\n\n\n");
		CHECK(iRet>=0, lblKO);
