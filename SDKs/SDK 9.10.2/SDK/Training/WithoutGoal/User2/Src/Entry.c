...
//      debit_non_emv : Process a non EMV chip card or Mag stripe or card manual entry transaction.
...
#include "User2.h"
#include "TlvTree.h"
#include "Bmp\Card0U.h"
#include "Bmp\Card1U.h"
#include "Bmp\Card2U.h"
#include "Bmp\Card3U.h"
#include "Bmp\Card4U.h"
#include "Bmp\Card5U.h"
#include "Bmp\Card6U.h"
#include "Bmp\Card7U.h"
#include "Bmp\Card8U.h"
#include "Bmp\Card9U.h"
#include "Bmp\Card10U.h"
...
const byte *tpcImgTbl[MAX_IMG] =
{
	Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U,
	Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U, Card0U,
	Card1U, Card2U, Card3U, Card4U, Card5U, Card6U, Card7U, Card8U, Card9U,
	Card10U, Card10U, Card10U, Card10U, Card10U, Card10U, Card10U, Card10U, Card10U, Card10U,
	Card9U, Card8U, Card7U, Card6U, Card5U, Card4U, Card3U, Card2U, Card1U
};
	...
	if (pxDsp != NULL)
	{
		do
		{
			...
			case 0:                              // Function1 selected
				...
				DisplayMedium(0, 0, "Function1");
				DisplayMedium(1, 0, "Running...");
				ttestall(0, 2*100);              // Wait 2s
				bContinue=FALSE;
				break;
			case 1:                              // Function2 selected
				_clrscr();                       // Clear screen
				DisplayMedium(0, 0, "Function2");
				DisplayMedium(1, 0, "Running...");
				ttestall(0, 2*100);              // Wait 2s
				bContinue=FALSE;
				break;
			...
			case 4:                              // Function5 selected
				_clrscr();                       // Clear screen
				DisplayMedium(0, 0, "Function5");
				DisplayMedium(1, 0, "Running...");
				ttestall(0, 2*100);              // Wait 2s
				bContinue=FALSE;
				break;
			default:                             // Abort key pressed
				bContinue=FALSE;
				break;
			}
		} while(bContinue);
...
int message_received(NO_SEGMENT no, S_MESSAGE_IAM *param_in, void *p2)
{
	...
	// **********************************
	_clrscr();                                                                    // Clear screen
	DisplaySmall(0, 0, "USER2");                                                  // Display receiver USER2
	...
	// *****************************
	IamMsgToTraining();                                                           // Send back message to the sender TRAINING
...
{
	byte ucHdr=1;
	FILE *pxPrn=NULL, *pxDsp=NULL, *pxKbd=NULL;
	char tcDisplay[25+1];
...
int IacRegisterTraining(doubleword size, void *data)
{
    ...
    // ========================
    _clrscr();                                                                             // Clear screen
    DisplaySmall(0, 0, "USER2");
    DisplaySmall(1, 0, "Msg from TRAINING:");
    Display25(2, tcRsp, strlen(tcRsp));                                                    // Display response

    // Print message received
	// ======================
    pxPrn = fopen("PRINTER", "w-*"); CHECK(pxPrn!=NULL, lblKO);                            // Open the "printer" channel
    iRet = pprintf("\x1b""E""        I.A.C Demo\n\n""\x1b""F"); CHECK(iRet>=0, lblKO);     // Print demo in progress
    iRet = pprintf("\x1b""E""USER2\n""\x1b""F"); CHECK(iRet>=0, lblKO);
    iRet = pprintf("Msg from TRAINING:\n"); CHECK(iRet>=0, lblKO);
...
lblEnd: 
    ...
    if(pxPrn)                                                                              // Close "printer" channel
		fclose(pxPrn);
	if(pxDsp)
        fclose(pxDsp);                          // Close "display" peripheral
...
int give_interface(word no, void *p1, void *p2)
{
	...
	static service_desc_t txServices[] =
	{
	    ...
	    { 0, SELECT_FUNCTION,              (SAP)Main, SERVICES_LOW_PRIORITY },   // Service Manager => Process a non EMV chip card or Mag stripe or card manual entry transaction (CREDIT/CANCEL/PRE AUTH)
