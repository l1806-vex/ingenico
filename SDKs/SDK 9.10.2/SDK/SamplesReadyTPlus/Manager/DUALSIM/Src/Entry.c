...
static service_desc_t Services[] = { { 0, GIVE_YOUR_DOMAIN, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
...
int give_your_domain(NO_SEGMENT no, void *p1, S_INITPARAMOUT *param_out)
{
   ...
   param_out->returned_state[param_out->response_number].mask = MSK_MDP | MSK_SWIPE | MSK_TYPE_PPAD | MSK_PINPAD | MSK_STANDARD | MSK_LANGUE | MSK_FRMT_DATE | MSK_DATE | MSK_TYPE_RESEAU;
...
// Print parameter file received
   ...
   if (strcmp(param_in->file_name, "DUALSIM.PAR") == 0)
   {
      ret = PAR_Import("/HOST/DUALSIM.PAR");
   }
