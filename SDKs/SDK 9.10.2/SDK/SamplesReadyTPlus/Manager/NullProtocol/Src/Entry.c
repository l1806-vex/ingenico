...
static service_desc_t Services[] = {
                                     ...
                                     { 0, IDLE_MESSAGE, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
                                     { 0, MORE_FUNCTION, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
                                     { 0, KEYBOARD_EVENT, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
                                     { 0, STATE, (SAP) Main, (unsigned char) SERVICES_HIGH_PRIORITY },
...
int after_reset(NO_SEGMENT no, void *p1, S_TRANSOUT *param_out)
{
   ...
   task = CurrentTask();
   FUN_Set_ManagerTask(task);
   /* Start Protocol Task */
   pttask = (t_topstack *) fork((PFONCFORK) FUN_Task_Protocol, NULL, NULL);
...
{
   guiScreen("Null Protocol", "NULL PROTOCOL\n SAMPLE", GL_ICON_NONE, 0);
   return (FCT_OK);
}
/**
...
int file_received(NO_SEGMENT no, S_FILE *param_in, void *p2)
{
   ...
   if (strcmp(param_in->file_name, "NULLPROTOCOL.PAR") == 0)
   {
      PAR_Import("/HOST/NULLPROTOCOL.PAR");
   }
