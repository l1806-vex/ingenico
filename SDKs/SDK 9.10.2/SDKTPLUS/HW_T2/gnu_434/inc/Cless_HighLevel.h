...
//! \addtogroup ClessGenericSelectionEntryPointFunctions
//! @{

//! \brief Get the PPSE Application Selection PreProcessing results.
//! \param[out] hOutputTlvTree Output TLV-Tree containing the PPSE Application Selection Pre Processing results.
//! \return
//!		- \ref CLESS_STATUS_OK if process correctly completed.
//!		- \ref CLESS_STATUS_EP_NOT_LOADED if PPSE Application Selection is not loaded in the terminal.
...
//!		- \ref CLESS_STATUS_BAD_COMPONENT_INIT if PPSE Application Selection cannot be used because of a bad initialisation.
int Cless_Generic_EntryPoint_GetPreProcessingResults (TLV_TREE_NODE hOutputTlvTree);

//! \brief Get an Unpredictable Number as generated by PPSE Application Selection.
...
//!	- \ref TAG_EP_KERNEL_TO_USE : kernel to be used for this AID (as provided as application selection input data, if present).
//!	- \ref TAG_EP_ASSOCIATED_KERNEL_ID : kernel identifier associated to the AID (as provided as application selection input data, if present).
...
//!	- \ref TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS : Terminal Transaction Qualifiers (if present, for Visa AID only, modified according to the EMVCo Entry Point specification).
...
//!		- \ref CLESS_STATUS_EP_NOT_LOADED if PPSE Application Selection component is not loaded.
//!		- \ref CLESS_STATUS_NOT_FOUND if there is no available candidate list within PPSE Application Selection module.
//!		- \ref CLESS_STATUS_LACK_OF_MEMORY if there is not enough memory.
//!		- \ref CLESS_STATUS_KO if an error occurred.
//! \note This function requests PPSE Application Selection module to provide its candidate list.
//! \note PPSE Application Selection module automatically clears the buffer, whatever the value of \a bClearSharedExchange.
...
//! \brief Perform PPSE Application Selection pre-conditions and pre-processing treatments using data given with \ref Cless_ExplicitSelection_LoadData.
//! \return
//!		- \ref CLESS_STATUS_OK if process correctly completed.
//!		- \ref CLESS_STATUS_EP_NOT_LOADED if PPSE Application Selection application is not loaded.
//!		- \ref CLESS_STATUS_KO if an error occurred.
...
//!	- For the "PPSE Application Selection" method, it consists of
...
//!		- \ref TAG_EP_ERROR_INFORMATION : Error code information of PPSE Application Selection module (if present).
//!		- \ref TAG_GENERIC_APPLIED_METHOD : Applied Application selection method (\ref GENERIC_METHOD_PPSE in this case).
...
//!		- \ref TAG_EP_FINAL_SELECT_STATUS_WORD : Final SELECT status word.
//!		- \ref TAG_EP_FINAL_SELECT_RESPONSE : Final SELECT card response.
//!		- \ref TAG_GENERIC_APPLIED_METHOD : Applied Application selection method (\ref GENERIC_METHOD_LOA in this case).
//!		- At least one AID candidate element, corresponding to the AID that matched (each candidate element is stored in a \ref TAG_EP_CANDIDATE_LIST_ELEMENT constructed tag) :
//!			- \ref TAG_EMV_DF_NAME : the DF Name returned by the card (and the one sent in the Final SELECT command (as supported by the card)).
//!			- \ref TAG_EMV_AID_TERMINAL : AID as supported by the terminal (provided as input parameter).
//!			- \ref TAG_EMV_APPLI_PRIORITY_INDICATOR : application priority indicator as returned by the card (if present).
...
//!		- \ref CLESS_STATUS_KO if an error occurred.
//!	- For PPSE Application Selection method :
...
//!		- \ref CLESS_STATUS_EP_CLESS_SELECTION_NOT_ALLOWED if PPSE Application Selection method is not allowed as no any AID is available (according to pre processing).
//!		- \ref CLESS_STATUS_COMMUNICATION_ERROR there is a communication error with the card (card removed for example).
...
//!		- \ref CLESS_STATUS_BAD_COMPONENT_INIT if PPSE Application Selection module cannot be used because of a bad initialisation.
//!		- \ref CLESS_STATUS_MISSING_PARAM if a mandatory parameter is missing.
//!		- \ref CLESS_STATUS_INVALID_PARAM if one of the provided parameters is incorrect.
//!		- \ref CLESS_STATUS_EP_NOT_LOADED if PPSE Application Selection module is not loaded.
//!		- \ref CLESS_STATUS_KO if an error occurred.
...
//!		- \ref CLESS_STATUS_KO if an error occurred.
//!	- PPSE Application Selection method :
//!		- \ref CLESS_STATUS_OK if results correctly got.
//!		- \ref CLESS_STATUS_LACK_OF_MEMORY if there is not enough memory to complete the processing or to copy results in the output buffer.
//!		- \ref CLESS_STATUS_NO_MATCHING_AID if there is no matching AID (candidate list is empty).
//!		- \ref CLESS_STATUS_CARD_BLOCKED the presented card is blocked.
...
//! \brief This function performs PPSE (using PPSE Application Selection module) method.
...
//!		- \ref CLESS_STATUS_BAD_PPSE_SW if the status word returned by the card on PPSE is not expected.
//!		- \ref CLESS_STATUS_BAD_COMPONENT_INIT if PPSE Application Selection module cannot be used because of a bad initialisation.
//!		- \ref CLESS_STATUS_EP_CLESS_SELECTION_NOT_ALLOWED if it is not possible to perform PPSE Application Selection as there is no available AID for this method (no AID or no pre processing has been performed).
//! \note The output buffer (\a pSharedDataStruct) is automatically cleared.
int Cless_ExplicitSelection_Selection_PpseMethodProcess(T_SHARED_DATA_STRUCT* pSharedDataStruct);

//! brief This function removes the AID having the highest priority in the candidate list for the PPSE method (done by PPSE Application Selection module).
//! \param[out] pSharedDataStruct Shared Exchange structure containing the PPSE method process result. Refer to \ref Cless_ExplicitSelection_Selection_ApplicationSelectionProcess
//! function for more information about the possible returned data.
//! \return
//!		- \ref CLESS_STATUS_OK if results correctly got.
...
//! \brief This function return response of PPSE processing (using PPSE Application Selection module) method.
//! \param[out] pSharedDataStruct Shared Exchange structure containing the PPSE method process result. Refer to \ref Cless_ExplicitSelection_Selection_ApplicationSelectionProcess
//! function for more information about the possible returned data.
//! \return
...
//! \brief This function return response of PPSE processing result.
//! \param[in] bClearSharedExchange \a TRUE if \a pSharedExchange shall be cleared, \a FALSE if not.
//! \param[out] pSharedDataStruct Shared Exchange structure updated with card response using folowing TAGS :
//! TAG_EP_PPSE_CARD_RESPONSE and TAG_EP_PPSE_STATUS_WORD.
//! \return
//!		- \ref CLESS_STATUS_OK if PPSE response correctly provided.
//!		- \ref CLESS_STATUS_NOT_FOUND if there is no available PPSE response.
//!		- \ref CLESS_STATUS_LACK_OF_MEMORY if there is not enough memory to complete the processing.
//!		- \ref CLESS_STATUS_BAD_COMPONENT_INIT if component cannot be used because of a bad initialisation.
int Cless_Generic_Selection_PpseMethodGetPpseResponse(T_SHARED_DATA_STRUCT * pSharedDataStruct, const int bClearSharedExchange);
//! @}

//! \brief Adds card commands and responses used to realise LOA selection.
//! \param[in] bClearSharedExchange \a TRUE if \a pSharedExchange shall be cleared, \a FALSE if not.
//! \param[out] pSharedDataStruct Shared exchange buffer updated with the card command orders et responses used for selection.
//!	- \ref TAG_GENERIC_SELECT : Indicates a card command/response/SW used to select an application.
//!	    - \ref TAG_GENERIC_SELECT_COMMAND : Indicates the card command used to select an application.
//!	    - \ref TAG_GENERIC_SELECT_CARD_RESPONSE : Indicates the card response to select command.
//!	    - \ref TAG_GENERIC_SELECT_CARD_SW : Indicates the card response status to select command.
//! \return 
//!		- \ref CLESS_STATUS_OK if candidate list is not empty and one or several candidate element are provided.
//!		- \ref CLESS_STATUS_LACK_OF_MEMORY if there is not enough memory.
//!		- CLESS_STATUS_INVALID_PARAM if \a pSelectResponseListStruct is \a NULL or if \a pElement is NULL.
int Cless_Generic_Selection_ListOfAidMethodGetListOfCommandsAndResponses(T_SHARED_DATA_STRUCT * pSharedDataStruct, const int bClearSharedExchange);
//! @}
...
//! Here is described what are the possible data to be provided in the TLV Tree for card detection and application selection.<br>
//!	Note: If the TLV Tree is empty, then the application will be considered as not responding to CLESS_GIVE_INFO.
//!
...
//! For AID criterion, it is possible to use PPSE Application Selection module. In this case, pre processing could be performed according to EMVCo Entry Point specification.
...
//!	- PPSE Application Selection module (closed to the one specified by EMVCo).
...
//!	- \ref TAG_EP_KERNEL_TO_USE : Kernel to be used for this AID. Refer to EntryPointPossibleKernels for more information about the possible values. Note this data is mandatory if the B1b7 option in \ref TAG_EP_AID_OPTIONS is not set (support of new tag TAG_EP_CARD_KERNEL_IDENTIFIER and EP 2.1 pre processing) or this the option is set and \ref TAG_EP_ASSOCIATED_KERNEL_ID is not present.
//!	- \ref TAG_EP_ASSOCIATED_KERNEL_ID : Associated kernel identifier for this AID. Note this data is strongly recommanded if the B1b7 option in \ref TAG_EP_AID_OPTIONS is set (support of new tag TAG_EP_CARD_KERNEL_IDENTIFIER and EP 2.1 pre processing). It could be replaced with \ref TAG_EP_KERNEL_TO_USE, but this will limit the supported kernel ID (restricted to few International kernel IDs).
...
//!		node_Ppse [label="PPSE method with PPSE Application Selection module",size="50,2",style=filled,fillcolor=plum];
...
//!	- The PPSE Application Selection module shall be loaded (if payment has to be performed or if PPSE method has to be used).
...
//! When called, the card is in the field, the transaction has started, and the following transaction steps has been performed :
//!	- Card detection and activation.
//!	- If it is an ISO14443-4 card: APDUs before, AIDs (PPSE Application Selection and List Of AIDs) and APDUs after.
//!
//! <br><br><h2>Service calling type and environment</h2>
...
//!			- \ref TAG_GENERIC_ERROR_INFORMATION indicates the error code information (if present).
//!		- Application Selection result : it depends on the applied method :
//!			- For the "PPSE Application Selection" method, it consists of (if a method matched) :
//!				- \ref TAG_EP_NUMBER_OF_CANDIDATE_AID_IN_LIST : the number of AID within the candidate list.
//!				- \ref TAG_EP_FINAL_SELECT_COMMAND_SENT : Final SELECT command sent to the card.
//!				- \ref TAG_EP_FINAL_SELECT_STATUS_WORD : Final SELECT status word.
//!				- \ref TAG_EP_FINAL_SELECT_RESPONSE : Final SELECT card response.
...
//!		node_ApduBefore [label="APDU Before method",size="50,2",style=filled,fillcolor=plum];
//!		node_IsApduBeforeMatched [label="APDU Before matched ?",size="50,2",shape="diamond",style=filled,fillcolor=white];
//!		node_Ppse [label="PPSE method with PPSE Application Selection module (AID method)",size="50,2",style=filled,fillcolor=plum];
//!		node_IsPpseMatched [label="AID matched ?",size="50,2",shape="diamond",style=filled,fillcolor=white];
//!		node_Loa [label="List Of AID method",size="50,2",style=filled,fillcolor=plum];
//!		node_IsLoaMatched [label="AID matched ?",size="50,2",shape="diamond",style=filled,fillcolor=white];
//!		node_ApduAfter [label="APDU After method",size="50,2",style=filled,fillcolor=plum];
...
//! \addtogroup EntryPointNewOption9F2AAndNewPreProcessing
//! @{
//!	<h2>Overview</h2>
//!	New features have been implemented within the PPSE method processing. It is now allowed to manage:<br>
//!		- The new \ref TAG_EP_CARD_KERNEL_IDENTIFIER tag that can be returned by the card.<br>
//!		- The new pre processing as described in the EMV Book B, section 3.1.<br>
//!	
//!	<br>
//!	These new features are AID dependant, and can be activated through the Byte 1 bit 7 of \ref TAG_EP_AID_OPTIONS. Note that if this option is supported, the notion of EMVCo kernel is not supported anymore.<br>
//!	<br><br>
//!	
//!	<h2>Management of Card Kernel Identifier</h2>
//!	The new \ref TAG_EP_CARD_KERNEL_IDENTIFIER tag, if returned by the card, is now managed (and if the option to support this is set in \ref TAG_EP_AID_OPTIONS).
//!	However, the old \ref TAG_EP_CLESS_APPLI_CAPABILITY_TYPE will continue to be managed, but it will depend on different criterion:
//!	<table>
//! <tr><th>New option supported	<th>Card returns \ref TAG_EP_CLESS_APPLI_CAPABILITY_TYPE	<th>Card returns \ref TAG_EP_CARD_KERNEL_IDENTIFIER		<th>Tag used
//! <tr><td>Yes						<td>Yes														<td>Yes													<td>\ref TAG_EP_CARD_KERNEL_IDENTIFIER is used
//! <tr><td>Yes						<td>Yes														<td>No													<td>\ref TAG_EP_CLESS_APPLI_CAPABILITY_TYPE is used
//! <tr><td>Yes						<td>No														<td>Yes													<td>\ref TAG_EP_CARD_KERNEL_IDENTIFIER is used
//! <tr><td>Yes						<td>No														<td>No													<td>None
//! <tr><td>No						<td>Yes														<td>Yes													<td>\ref TAG_EP_CLESS_APPLI_CAPABILITY_TYPE is used
//! <tr><td>No						<td>Yes														<td>No													<td>\ref TAG_EP_CLESS_APPLI_CAPABILITY_TYPE is used
//! <tr><td>No						<td>No														<td>Yes													<td>None
//! <tr><td>No						<td>No														<td>No													<td>None
//! </table>
//!	<br>
//!	Here is the format of \ref TAG_EP_CARD_KERNEL_IDENTIFIER, as described in EMVCo Book B, table 3-4 and table 3-5:<br>
//!	@image html EntryPointCardKernelIdTable1.PNG
//!	<br>
//!	@image html EntryPointCardKernelIdTable2.PNG
//!	<br><br>
//!	A new tag has been created to allow the full support of \ref TAG_EP_CARD_KERNEL_IDENTIFIER in order an AID can be configured with a kernel identifier matching 
//!	with this new card tag. This tag is \ref TAG_EP_ASSOCIATED_KERNEL_ID). It has the same format as \ref TAG_EP_CARD_KERNEL_IDENTIFIER (and so, it cannot be configured
//!	with a 2-bytes length).<br>
//!	<br>
//!	From the AID configuration perspective, here are the rule to configure the kernel ID associated to an AID (\ref TAG_EP_KERNEL_TO_USE and \ref TAG_EP_ASSOCIATED_KERNEL_ID):<br>
//!		- If the option is not set in the \ref TAG_EP_AID_OPTIONS, then the \ref TAG_EP_KERNEL_TO_USE shall be present (mandatory data) in the AID structure.<br>
//!		- If the option is not set in the \ref TAG_EP_AID_OPTIONS, then at least \ref TAG_EP_KERNEL_TO_USE or \ref TAG_EP_ASSOCIATED_KERNEL_ID shall be present:<br>
//!			- If \ref TAG_EP_ASSOCIATED_KERNEL_ID is present, then this will be use (and \ref TAG_EP_KERNEL_TO_USE will be ignored if present).<br>
//!			- If \ref TAG_EP_ASSOCIATED_KERNEL_ID is not present, then \ref TAG_EP_KERNEL_TO_USE shall be present and will be used.<br>
//!	
//!	<br>
//!	For more details regarding the management of \ref TAG_EP_CARD_KERNEL_IDENTIFIER, please refer to EMVCo Book B, table 3-2, table 3-4, table 3-5, and Book B section 3.3.2.5.<br>
//!	<br><br>
//!	
//!	<h2>New pre-processing</h2>
//!	This new option allows to use the new pre-processing as described in the EMV Book B, section 3.1. Here are the main differences compared to the currently used pre-processing :<br>
//!		- \ref TAG_EP_TERMINAL_TRANSACTION_QUALIFIERS can be now used for all the AID (and not specifically for AID configured with the Visa kernel ID).<br>
//!		- The results when the transaction amount is zero are different as well.<br>
//!	
//!	<br>
//!	Here are the differences of pre processing when amount is zero, between the new processing and the current one (the differences are indicated in red):<br>
//!	@image html EntryPointPPZeroComparison.PNG
//!	<br>
//!	
//!	For more information about th enew pre-processing, please refer to EMVCo Book B, section 3.1.<br>
//!	<br><br>
//! @}
