 ...
 * @{
   ...
   * @{
      ...
      * - MIFARE Plus cards,
      * - MIFARE Ultralight and Ultralight C cards,
      * - MIFARE SAM cards.
      *
      ...
      * @{
      *    \details This module implements DESFire commands compliant with both DESFire
      ...
      * @}
      *
      * \defgroup ClessMFP MIFARE Plus card management
      *
      * \defgroup ClessMFU MIFARE Ultralight card management
      *
      * \defgroup MifareSAM MIFARE SAM management
      *
      * \defgroup MifareKeyManagement MIFARE Key management functions
      *
   * @}
 * @}
...
#define  DESFIRE_PROCESSING_ERROR      0x0200 //!< Internal processing error (from 0x200 to 0x2FF)
#define  DESFIRE_DLL_INIT_OK           0x0000 //!< Initialization of DLL OK
#define  DESFIRE_DLL_NOT_LOADED        0x0100 //!< The MIFARE DLL is not loaded
//! @}
...
/* File types */
 ...
 enum T_DESFIRE_FILE_TYPES
 {
    ...
    DFT_CYCLIC_RECORD_FILE_WITH_BACKUP = 0x04  //!< Cyclic record file with a backup mechanism
 };
...
struct S_DESFIRE_FILE_SETTINGS
{
   ...
   unsigned short int usiAccessRights; //!< Access Rights on 16 bits [Read][Write][Read & Write][Change]. See \ref DAR_READ, \ref DAR_WRITE, \ref DAR_READ_WRITE, \ref DAR_CHANGE_AR
   union
   {
      struct
      {
...
typedef enum
{
   APDU_NATIVE, //!< Commands are sent in desfire native format : [INS][DATA]
   APDU_WRAPPED //!< Commands are wrapped to be sent in desfire ISO/IEC 7816-3 format.
...
//! - \ref APDU_NATIVE : Frames in DESfire Native style.
//! - \ref APDU_WRAPPED : Frames in in ISO/IEC 7816-3 style.
//! \return Return a pointer to the initialised context. NULL if an error occurred.
...
extern T_DESFIRE_CONTEXT* desfire_NewContextWithSAM(Telium_File_t* hSAM, T_MIFARE_SAM_MODE samMode, T_DESFIRE_APDU_STYLE apduStyle);
#else
extern T_DESFIRE_CONTEXT* desfire_NewContextWithSAM(FILE* hSAM, T_MIFARE_SAM_MODE samMode, T_DESFIRE_APDU_STYLE apduStyle);
#endif
...
//! \brief Get the SAM context of a DESFire context.
//! \param[in] pContext Pointer to the T_DESFIRE_CONTEXT (see \ref desfire_NewContextWithSAM)
//! \return Return a pointer to \ref T_MIFARE_SAM_CONTEXT context. NULL if an error occurred.
extern T_MIFARE_SAM_CONTEXT* desfire_GetSAMContext(T_DESFIRE_CONTEXT *pContext);
...
//! \brief Authentication between PCD and PICC using SAM with DESFire key numbering.
//! \details A \ref desfire_SelectApplication or \ref desfire_SelectApplicationWithAid should
...
//! 3K3DES and AES key
//! - 0 : RFU </td></tr>
//! <tr><td>Bit 2</td><td> 0 : RFU </td></tr>
//! <tr><td>Bit 1</td><td> 0 : Not used (internally set to 1)</td></tr>
//! <tr><td>Bit 0</td><td> 0 : Not used (internally set according to diversififcation input)</td></tr>
//! </table>
//! \param[in] ucKeyNo SAM/DESFire key number according to current selected application.
//! \param[in] ucKeyVersion SAM key version.
//! \param[in] iDivInpLen Diversification input length in bytes ( or 0 for no diversification).
//! \param[in] pDivInp Diversification input (or NULL for no diversification).
//! \return Command status code or CLESS status code (0x00 if no error occurred)
...
extern int desfire_AuthenticateWithSAM(T_DESFIRE_CONTEXT *pContext,
      ...
      unsigned char ucKeyVersion, int iDivInpLen, const void* pDivInp);

//! \brief Authentication between PCD and PICC using SAM.
//! \details
//! \param[in,out] pContext Pointer to the application context.
//! \param[in] ucCommand Authentication mode to use :
//! - AUTHENTICATE_LEGACY : For DES or 2K3DES with DESFire chaining mode (compliant EV0 and EV1)
...
//! - 0 : RFU </td></tr>
//! <tr><td>Bit 2</td><td> 0 : RFU </td></tr>
//! <tr><td>Bit 1</td><td> 0 : By default</td></tr>
//! <tr><td>Bit 0</td><td> 0 : Not used (internally set according to diversififcation input)</td></tr>
//! </table>
//! \param[in] ucKeyNo DESFire Key number(0x00 to 0x0D).
//! \param[in] ucSAMKeyNo SAM Key number (0x00 to 0x7F).
//! \param[in] ucSAMKeyVersion SAM Key version (0x00 to 0xFF).
//! \param[in] iDivInpLen Diversification input length in bytes (or 0 for no diversification).
//! \param[in] pDivInp Diversification input (or NULL for no diversification).
//! \return Command status code or CLESS status code (0x00 if no error occurred)
//! \sa \ref desfire_NewContextWithSAM, \ref desfire_NewContext, \ref desfire_Authenticate, \ref mifare_New3desKey, mifare_New3desKeyWithVersion,
//! \ref mifare_New3k3desKey, \ref mifare_New3k3desKeyWithVersion, \ref mifare_NewAesKey,
//! \ref mifare_NewAesKeyWithVersion, \ref mifare_NewDesKey, \ref mifare_NewDesKeyWithVersion
int desfire_AuthenticateWithSAMKey(T_DESFIRE_CONTEXT *pContext,
      unsigned char ucCommand, unsigned char ucAuthMode, unsigned char ucKeyNo,
      unsigned char ucSAMKeyNo, unsigned char ucSAMKeyVersion, int iDivInpLen, const void* pDivInp);
...
//! \code
//! desfire_AuthenticateWithSAM(pContext, AUTHENTICATE_LEGACY, 0x00, ucKeyNo, 0, 0, NULL );
//! \endcode
//! \param[in,out] pContext Pointer to the application context.
...
extern int desfire_Authenticate(T_DESFIRE_CONTEXT *pContext, unsigned char ucKeyNo, T_MIFARE_KEY *pKey);
...
//! This command will have the same effect than :
//! \code
//! desfire_AuthenticateWithSAM(pContext, AUTHENTICATE_ISO, 0x00, ucKeyNo, 0, 0, NULL );
//! \endcode
//! \param[in,out] pContext Pointer to the application context.
//! \param[in] ucKeyNo Identify the key that the PICC shall use to encipher/deciper.
//! \param[in] pKey Pointer to the key that will be used to encipher/decipher
...
//! , \ref desfire_AuthenticateWithSAM, \ref desfire_NewContextWithSAM
extern int desfire_AuthenticateIso(T_DESFIRE_CONTEXT *pContext, unsigned char ucKeyNo, T_MIFARE_KEY *pKey);
...
//! This command will have the same effect than :
//! \code
//! desfire_AuthenticateWithSAM(pContext, AUTHENTICATE_AES, 0x00, ucKeyNo, 0, 0, NULL );
//! \endcode
//! \param[in,out] pContext Pointer to the application context.
//! \param[in] ucKeyNo Identify the key that the PICC shall use to encipher/deciper.
...
//! , \ref desfire_AuthenticateWithSAM, \ref desfire_NewContextWithSAM
extern int desfire_AuthenticateAes(T_DESFIRE_CONTEXT *pContext, unsigned char ucKeyNo, T_MIFARE_KEY *pKey);
...
extern int desfire_ChangeKeySettings(T_DESFIRE_CONTEXT *pContext, unsigned char ucSettings);
...
//! (key 0x00) itself changed. In this case only the new key is involved in the
...
//! <tr><td>Bit 4</td><td>
//! - 1 : The DESFire card master key shall be changed
...
//! <table><tr><th>Bits Value</th><th>Description</th></tr>
//! <tr><td>Bit 7 - 6</td><td> For DESFire Card Master key (otherwise RFU):
//! - 00 : New Card Master key is a DES/2K3DES key
//! - 01 : New Card Master key is a 3K3DES key
//! - 10 : New Card Master key is a AES key </td></tr>
//! <tr><td>Bit 5-4</td><td>RFU</td></tr>
//! <tr><td>Bit 3 - 0</td><td> number of the DESFire key to be changed (should be indentical to \a ucConfig bits 3-0)</td></tr>
//! </table>
...
extern int desfire_GetCardUID(T_DESFIRE_CONTEXT *pContext, unsigned char *pucUID, int * piUidLength);
...
//! \ref desfire_GetApplicationIds
extern int desfire_DeleteApplication(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_AID *pAid);
...
extern int desfire_GetApplicationIds(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_AID tAids[], int *piCount);
...
extern int desfire_SelectApplicationWithAid(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_AID *pAid);
...
//! This command automatically calls the 'SAM_SelectApplication' command if SAM is used.
//! \param[in,out] pContext Pointer to the application context.
...
//! \return Command status code or CLESS status code (0x00 if no error occurred)
extern int desfire_GetVersion(T_DESFIRE_CONTEXT *pContext, struct S_DESFIRE_VERSION_INFO *pVersionInfo);
...
extern int desfire_GetFileIds(T_DESFIRE_CONTEXT *pContext, unsigned char tucFiles[], int *piCount);
...
//! \ref desfire_CreateLinearRecordFile, \ref desfire_CreateCyclicRecordFile
extern int desfire_GetIsoFileIds(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_ISO_FID tucFiles[], int *piCount);
...
extern int desfire_DeleteFile(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId);
...
//! <table><tr><th>Bits Value</th><th>Description</th></tr>
//! <tr><td>Bit 15 - 12</td><td>GetValue/Debit Access rights</td></tr>
//! <tr><td>Bit 11 - 8</td><td>GetValue/Debit/LimitedCredit Access rights</td></tr>
//! <tr><td>Bit 7 - 4</td><td>GetValue/Debit/LimitedCredit/Credit Access rights</td></tr>
...
//! <table><tr><th>Bits Value</th><th>Description</th></tr>
//! <tr><td>Bit 7 - 2</td><td>RFU.</td></tr>
//! <tr><td>Bit 1</td><td>Free get value feature (see \ref desfire_GetValue):
//! - 0 : Free get value feature is disabled.
//! - 1 : Free get value feature is enabled.
//! <tr><td>Bit 0</td><td>Limited credit feature (see \ref desfire_LimitedCredit):
...
extern int desfire_ChangeFileSettings(T_DESFIRE_CONTEXT *pContext,
      T_DESFIRE_FID fileId, unsigned char ucComSettings, unsigned int uiAccessRights);
//! @}
...
extern int desfire_ReadData(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulDataToReadLen, unsigned long* pulDataReadLen, void *pData);
...
//! desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS,
//! \ref desfire_ClearRecordFile, \ref desfire_CommitTransaction, \ref desfire_AbortTransaction
extern int desfire_ReadDataCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, unsigned long ulOffset,
      unsigned long ulDataToReadLen, unsigned long* pulDataReadLen, void *pData, int iCommSettings);
...
extern int desfire_WriteData(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulLength, const void *pData);
...
//! desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS,
//! \ref desfire_ClearRecordFile, \ref desfire_CommitTransaction, \ref desfire_AbortTransaction
extern int desfire_WriteDataCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulLength, const void *pData, int iCommSettings);
...
//! desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS,
//! \ref desfire_ClearRecordFile, \ref desfire_CommitTransaction, \ref desfire_AbortTransaction
extern int desfire_GetValue(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int *piValue);
...
extern int desfire_Credit(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int iAmount);
...
//! \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS
extern int desfire_Debit(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int iAmount);
...
//! \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS
extern int desfire_DebitCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int iAmount, int iComSettings);
...
//! \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS
extern int desfire_LimitedCredit(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int iAmount);
...
//! \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords, \ref desfire_ReadRecordsCS
extern int desfire_LimitedCreditCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId, int iAmount, int iComSettings);

//! \brief Write data to a record in a cyclic or linear record file.
...
//! \param[in,out] pContext Pointer to the application context.
//! \param[in] fileId File identifier of the value file.
//! \param[in] ulOffset Position within a record where the write operation has to start.
//! This parameter has to be in the range 0x000000 to record size -1.
//! \param[in] ulLength Number of bytes to be written.
//! This parameter has to be in the range 0x000001 to 0xFFFFFF.
//! \param[in] pData Pointer to a buffer containing the data to write.
//! \return Command status code or CLESS status code (0x00 if no error occurred)
...
extern int desfire_WriteRecord(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulLength, const void *pData);
...
extern int desfire_WriteRecordCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulLength, const void *pData, int iComSettings);
...
//! with the key specified for \"Read\" or \"Read & Write\" access. \n
//! This function send a first command to the PICC to get the communication
//! settings and the record size of the file (see \ref desfire_GetFileSettings),
//! then the read record command.
//! Depending on the communication settings of the file, data will be sent
//! by the PICC either in plain, MACed or enciphered.\n
...
//! \param[in] ulNbRecToRead Number of records to be read from the PICC. Records are
...
//! command.
//! \warning The buffer pData shall be big enough to receive the data read. In most case,
//! the size of pData shall be at least equal to ulNbRecToRead * record size but if ulNbRecToRead
//! is equal to 0, the whole file will be read so pData shall have at least the same size than the
//! file which is read.
//! \sa \ref desfire_ReadData, \ref desfire_ReadDataCS, \ref desfire_WriteData, \ref
//! desfire_WriteDataCS, \ref desfire_GetValue, \ref desfire_GetValueCS, \ref desfire_Credit, \ref
//! desfire_CreditCS, \ref desfire_Debit, \ref desfire_DebitCS, \ref desfire_LimitedCredit,
//! \ref desfire_LimitedCreditCS, \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecordsCS
extern int desfire_ReadRecords(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulNbRecToRead, unsigned long* pulDataReadLen, void *pData);
...
//! one less than the number of records specified in the \"create cyclic record file\"
//! command.
//! \remarks If \a ulNbRecToRead is not null, a desfire_GetFileSettings command is internally sent.
//! \sa \ref desfire_ReadData, \ref desfire_ReadDataCS, \ref desfire_WriteData, \ref
//! desfire_WriteDataCS, \ref desfire_GetValue, \ref desfire_GetValueCS, \ref desfire_Credit, \ref
//! desfire_CreditCS, \ref desfire_Debit, \ref desfire_DebitCS, \ref desfire_LimitedCredit,
//! \ref desfire_LimitedCreditCS, \ref desfire_WriteRecord, \ref desfire_WriteRecordCS, \ref desfire_ReadRecords
extern int desfire_ReadRecordsCS(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId,
      unsigned long ulOffset, unsigned long ulNbRecToRead, unsigned long* pulDataReadLen,
...
extern int desfire_ClearRecordFile(T_DESFIRE_CONTEXT *pContext, T_DESFIRE_FID fileId);
