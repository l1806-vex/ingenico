...
static void __PrintDiagnostics (void)
{
	...
	if ((nResult != TPASS_CLESS_TYPE_TELIUM_PASS) && (nResult != TPASS_CLESS_TYPE_VENDING_PASS))
		return;
...
void CheckPresence (void)
{
	...
	bPresent = TPass_GetDeviceType();
	if (bPresent == TPASS_CLESS_TYPE_EMBEDDED)
	{	
		printf(" TYPE EMBEDDED");
		ttestall(0,200);
		return;
	}
...
void ResetTest (void)
{
	...
	// what is the current device type used?
	if (TPASS_CLESS_TYPE_EMBEDDED == TPass_GetDeviceType() )
	{
		printf(" only with TP+\n");
		ttestall(0,200);
		fclose(hDisplay);
 ...
 *
 * Description		:	Gets all the cless external reader information
 * 						(TeliumPass or Vending Pass).
 *
 ************************************************************************/
void InfoTest (void)
{
	...
	r = TPass_GetDeviceType();
	if ( (r != TPASS_CLESS_TYPE_TELIUM_PASS) && (r != TPASS_CLESS_TYPE_VENDING_PASS))
	{
		hDisplay = fopen("DISPLAY", "w");
		printf(" only with TP+\n");
		ttestall(0,200);
 ...
 * name:        DisplayTest
 *
 * definition:  display info in the TP+ screen
 *
 * @param       type of test
 *
 ************************************************************/
