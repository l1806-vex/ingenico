...
static int _SDSA_BlackListLoad(const char* szPath, const char* szFile);
static int _SDSA_BlackListSave(const char* szPath, const char* szFile);
...
{
	unlink("/" FILE_DISK_LABEL "/" BLACK_LIST_FILE_NAME);
	SDSA_BlackListInit();
}
...
{
	return _SDSA_BlackListLoad(FILE_DISK_LABEL, BLACK_LIST_FILE_NAME);
}
...
{
	return _SDSA_BlackListSave(FILE_DISK_LABEL, BLACK_LIST_FILE_NAME);
}
...
int SDSA_BlackListIsPanAscii(const char* szPan, int nPanSeqNumber)
{
	...
	{
		if (strcmp(g_BlackList[i].m_Pan, szPan) == 0)		// If Pan founded
		{
...
int SDSA_BlackListIsPan(int nLength, const unsigned char* pPan, int nPanSeqNumber)
{
	...
	return SDSA_BlackListIsPanAscii(szPan, nPanSeqNumber);
}
...
int SDSA_BlackList_LoadNewBlackList(const TLV_TREE_NODE hInputTlvTree)
{
	...
	if(hCurrentNode != NULL)
	{
		...
		{
			// Get the node tag
			if(TlvTree_GetTag (hCurrentNode) == TAG_EMV_APPLI_PAN)
			{
				// Add the black list entry
				memcpy(g_BlackList[g_nBlackListSize].m_Pan, TlvTree_GetData(hCurrentNode), TlvTree_GetLength(hCurrentNode));
				hCurrentNode = TlvTree_GetNext (hCurrentNode);
				if (hCurrentNode != NULL)
				{
					if(TlvTree_GetTag (hCurrentNode) == TAG_EMV_APPLI_PAN_SEQUENCE_NUMBER)
					{
						memcpy(&(g_BlackList[g_nBlackListSize].m_nPanSeqNumber), TlvTree_GetData(hCurrentNode), TlvTree_GetLength(hCurrentNode));
					
						g_nBlackListSize++;
					}
					else
					{
						GTL_Traces_TraceDebug("SDSA_BlackList_LoadNewBlackList : bad format");
						return (FALSE);
					}
				}
				else
	...
	GTL_Traces_TraceDebug("Black list size: %d", g_nBlackListSize);

	for(i=0;i<g_nBlackListSize;i++)
	{
		GTL_Traces_TraceDebug("PAN: %02X %02X %02X %02X %02X", g_BlackList[i].m_Pan[0],g_BlackList[i].m_Pan[1],g_BlackList[i].m_Pan[2],g_BlackList[i].m_Pan[3],g_BlackList[i].m_Pan[4]);
		GTL_Traces_TraceDebug("PAN SEQ: %d", g_BlackList[i].m_nPanSeqNumber);
	}				
	return (TRUE);
}
