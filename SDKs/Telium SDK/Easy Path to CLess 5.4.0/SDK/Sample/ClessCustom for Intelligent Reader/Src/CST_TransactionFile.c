...
static unsigned short __CST_TransactionFile_crc16_ccitt(const void *buff, int len);
static int __CST_TransactionFile_FillTlvTree(unsigned char * tr_buf, int tr_len, TLV_TREE_NODE *phOutputTlvTree);
...
void CST_TransactionFile_Init (T_Transaction_File *pt_Tr_File, char * FilePath)
{
...
int CST_TransactionFile_Check (T_Transaction_File *pt_Tr_File)
{
	int ret, ret_exist;
					...
					if (nb_elt_read == 1)
					{
						if (T_length > 0)
						{
							...
							if (nb_elt_read == 1)
							{
								...
								crc16_calculated = __CST_TransactionFile_crc16_ccitt(puc_buffer, T_length);
...
int CST_TransactionFile_AddTransaction (T_Transaction_File *pt_Tr_File, TLV_TREE_NODE hInputTlvTree)
{
	int ret_exist, ret;
	...
	if(hInputTlvTree != NULL)
	{
		...
		if (pt_Tr_File->hFile != NULL)
		{
			...
			if(First_trans)
			{
				crc16_trans = __CST_TransactionFile_crc16_ccitt(&puc_begin_buf[TRANS_FILE_HEADER_SIZE + INT_LEN], Trans_len);
			}
			else
			{
				crc16_trans = __CST_TransactionFile_crc16_ccitt(&puc_begin_buf[INT_LEN], Trans_len);
...
int CST_TransactionFile_GetTransaction (T_Transaction_File *pt_Tr_File, TLV_TREE_NODE *phOutputTlvTree, unsigned int Trans_pos, int * ptr_Trans_len)
{
	int ret_exist, ret;
			...
			{	
				...
				if (nb_elt_read == 1)
				{
					if (*ptr_Trans_len > 0)
					{
						...
						{
							// calculate CRC16 on data read
							crc16_calculated = __CST_TransactionFile_crc16_ccitt(puc_buffer, *ptr_Trans_len);
							...
							if (nb_elt_read == 1)
							{
								...
								{
									if (__CST_TransactionFile_FillTlvTree(puc_buffer, *ptr_Trans_len, phOutputTlvTree))
									{
										ret = TRANS_FILE_OK;
									}
								}
...
int CST_TransactionFile_GetNextTransaction (T_Transaction_File *pt_Tr_File, TLV_TREE_NODE *phOutputTlvTree, unsigned int *pt_trans_pos)
{
	int ret, T_length;
	
	ret = CST_TransactionFile_GetTransaction (pt_Tr_File, phOutputTlvTree, *pt_trans_pos, &T_length);
...
int CST_TransactionFile_Destroy (T_Transaction_File *pt_Tr_File)
{
	int ret_fct, ret;
...
int CST_TransactionFile_CopyValidTrans (T_Transaction_File *pt_In_Tr_File, T_Transaction_File *pt_Out_Tr_File)
{
	int ret;
...
int CST_TransactionFile_Rename (T_Transaction_File *pt_In_Tr_File, T_Transaction_File *pt_Out_Tr_File)
{
	int ret;
