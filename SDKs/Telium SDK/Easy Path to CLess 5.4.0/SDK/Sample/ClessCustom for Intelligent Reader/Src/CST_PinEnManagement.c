...
/* ========================================================================	*/
static int __CST_LoadRootKey(void);
...
static int __CST_FreeSecretArea(void)
{
	int L_iRet;

	T_SEC_DATAKEY_ID stKeyId;

	GTL_Traces_TraceDebug("__CST_FreeSecretArea");
...
static int __CST_LoadRootKey(void)
{
    ...
    // Warning, if a reset occurs at this step, a Scheme or secret area access has occured
...
int CST_InjectKey (void)
{
	...
	if (iret != OK)
	{
		...
		GTL_Traces_DiagnosticText("SEC_isSecretArea %d\n",iret);
	}
	else
	{
		iret = __CST_LoadRootKey();
		
		if (iret != OK)
		{
			GTL_Traces_DiagnosticText("__CST_LoadRootKey %d\n",iret);
		}
	}
	return iret;
}
...
int CST_OnlineEncipher (unsigned char pucPAN [],	unsigned char pucEncipheredPin [],int *iLenEnc)
{
	int iRes;
	int iAllIsOk;
	...
	if (iRes != OK)
	{
    	GTL_Traces_DiagnosticText("SEC_Iso9564 %d\n",iRes);
	    GTL_Traces_DiagnosticText(" %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n",iso9564Result[0],iso9564Result[1],iso9564Result[2],
