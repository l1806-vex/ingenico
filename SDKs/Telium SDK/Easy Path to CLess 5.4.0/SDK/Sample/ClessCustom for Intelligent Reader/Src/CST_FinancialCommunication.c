/**
 * \file	CST_FinancialCommunication.c
...
static void __CST_FinancialCommunication_BuildCommDel (DEL * pDelIn, const int nMsgType);
static int __CST_FinancialCommunication_HostConfigConnect(void);
static int __CST_FinancialCommunication_HostCommConnect (void);
static int __CST_FinancialCommunication_HostCommConfigure (DEL * inDEL);
static int __CST_FinancialCommunication_IsCommComponentLoaded (const unsigned char ucAppliType);
...
static int __CST_FinancialCommunication_HostCommConfigure (DEL * inDEL)
{
	...
	while (pElt != NULL)
	{
		if (pElt->tag == TAG_COMM_STATUS_CODE)
		{
			...
			{
				GTL_Traces_TraceDebug("__CST_FinancialCommunication_HostCommConfigure : Error occured (status = %02x)", usStatusCode);
			}
...
static int __CST_FinancialCommunication_HostCommConnect (void)
{
	...
	while (pElt != NULL)
	{
		if (pElt->tag == TAG_COMM_STATUS_CODE)
		{
			...
			{
				GTL_Traces_TraceDebug("__CST_FinancialCommunication_HostCommConnect : Error occured (status = %02x)", usStatusCode);
			}
...
static int __CST_FinancialCommunication_HostConfigConnect(void)
{
	...
	if (__CST_FinancialCommunication_IsCommComponentLoaded(I_COMM_DEFAULT_TYPE_APPLI))
	{
		// Configure communication component
		if (__CST_FinancialCommunication_HostCommConfigure (&ConfigDEL) == TRUE)
		{
			// Establish the connection with the communication component
			if (__CST_FinancialCommunication_HostCommConnect() == TRUE)
			{
				bIsConnected = TRUE;
			}
			else
			{
				GTL_Traces_TraceDebug ("__CST_FinancialCommunication_HostConfigConnect : __CST_FinancialCommunication_HostCommConnect fails\n");
			}
		}
		else
		{
			GTL_Traces_TraceDebug ("__CST_FinancialCommunication_HostConfigConnect : __CST_FinancialCommunication_HostCommConfigure failed\n");
		}
	}
	else
	{
		GTL_Traces_TraceDebug ("__CST_FinancialCommunication_HostConfigConnect : __CST_FinancialCommunication_IsCommComponentLoaded failed\n");
	}
...
//! \param[in,out] pSharedData Data to be sent to the host for online authorisation (input) and response tags are added to it (output).
//! \param[in] nKernelUsed Identifies the kernel that performed the transaction.
//! \return
//!		- \ref TRUE if correctly performed.
//!		- \ref FALSE if an error occured.

int CST_FinancialCommunication_ManageAuthorisation (T_SHARED_DATA_STRUCT * pSharedData, int nKernelUsed)
{
	DEL *pt_OutputDEL;
	DataElement Elt;
	DataElement * pElt;
	int cr;
	int nPosition, nAmountPosition;
	...
	if (!isConnected)
	{
		isConnected = __CST_FinancialCommunication_HostConfigConnect();
	}
	...
	if (isConnected)
	{
		...
		__CST_FinancialCommunication_BuildCommDel (&pt_AuthoDEL, COMM_AUTHOR_MSG);
	
		nPosition = SHARED_EXCHANGE_POSITION_NULL;

		if(nKernelUsed == SDSA_KERNEL_IDENTIFIER_EXPRESSPAY)
		{
			nAmountPosition = SHARED_EXCHANGE_POSITION_NULL;

			do{
				cr = GTL_SharedExchange_FindNext(pSharedData, &nPosition, TAG_EMV_AMOUNT_AUTH_NUM, &ulReadLength, (const unsigned char **)&pReadValue);

				if (cr == STATUS_SHARED_EXCHANGE_OK)
				{
					nAmountPosition = nPosition - 9;
				}
			} while (cr == STATUS_SHARED_EXCHANGE_OK);

			nPosition = nAmountPosition;
		}
		...
		while (pElt != NULL)
		{
			...
			if (pElt->tag == TAG_COMM_STATUS_CODE)
			{
				...
				{
					GTL_Traces_TraceDebug("CST_FinancialCommunication_ManageAuthorisation : COMM_Send_And_Receive_Message error occured (status = %02x)", usStatusCode);
				}
			}
	...
	{
		GTL_Traces_TraceDebug("CST_FinancialCommunication_ManageAuthorisation : Not connected");
	}
