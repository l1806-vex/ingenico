...
static int __ClessSample_PayPass_RetrieveTransactionOutcome (T_SHARED_DATA_STRUCT * pResultDataStruct, int * pTransactionOutcome);
static int __ClessSample_PayPass_RetrieveStart (T_SHARED_DATA_STRUCT * pResultDataStruct, int * pStart);
static int __ClessSample_PayPass_RetrieveUirdMessage (T_SHARED_DATA_STRUCT * pResultDataStruct, int * pUirdMessage);
static int __ClessSample_PayPass_RetrieveCvmToApply (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pCvm);
static int __ClessSample_PayPass_RetrieveCardType (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned short * pCardType);
static int __ClessSample_PayPass_RetrieveErrorIndicationByte (T_SHARED_DATA_STRUCT * pResultDataStruct, const unsigned int nByteNumber, unsigned char * pErrorIndicationByte);
...
static int __ClessSample_PayPass_RetrieveFieldOffValue (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pFieldOff);
...
static int __ClessSample_PayPass_RequestDataToKernel (T_SHARED_DATA_STRUCT * pDataRequest, unsigned long ulTag)
{
	...
	if (pDataRequest != NULL)
	{
		...
		{
			// An error occurred
			GTL_Traces_TraceDebug("__ClessSample_PayPass_RequestDataToKernel : An error occurred when getting tag to the PayPass kernel (cr=%02x)", cr);
			nResult = FALSE;
		}
	}
	else
...
static int __ClessSample_PayPass_RetrieveTransactionOutcome (T_SHARED_DATA_STRUCT * pResultDataStruct, int * pTransactionOutcome)
{
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveTransactionOutcome : Unable to get the outcome parameter set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
static int __ClessSample_PayPass_RetrieveErrorIndicationByte (T_SHARED_DATA_STRUCT * pResultDataStruct, const unsigned int nByteNumber, unsigned char * pErrorIndicationByte)
{
	...
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveErrorIndicationByte : Invalid nBytesNumber = %02X", nByteNumber);
		nResult = FALSE;
		goto End;
	}
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveErrorIndicationByte : Unable to get the Discretionary data set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveErrorIndicationByte : Unable to initialise the shared buffer with the Discretionary data set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveErrorIndicationByte : Unable to find the error indication in the Discretionary data set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
static int __ClessSample_PayPass_RetrieveUirdMessage (T_SHARED_DATA_STRUCT * pResultDataStruct, int * pUirdMessage)
{
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveUirdMessage : Unable to get the UIRD from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
static int __ClessSample_PayPass_RetrieveCvmToApply (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pCvm)
{
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveCvmToApply : Unable to get the outcome parameter set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
...
static int __ClessSample_PayPass_RetrieveFieldOffValue (T_SHARED_DATA_STRUCT * pResultDataStruct, unsigned char * pFieldOff)
{
	...
	if (cr != STATUS_SHARED_EXCHANGE_OK)
	{
		GTL_Traces_TraceDebug("__ClessSample_PayPass_RetrieveFieldOffValue : Unable to get the outcome parameter set from the C2 kernel response (cr = %02x)", cr);
		nResult = FALSE;
		goto End;
	}
	...
	if (pSharedStructure != NULL)
	{
		...
		else
		{
			...
			if (nResult != KERNEL_STATUS_OK)
			{
				...
				GTL_Traces_TraceDebug("ClessSample_PayPass_DebugActivation : Error occurred during PayPass Debug activation (nResult = %02x)", nResult);
			}
		}
...
static void __ClessSample_PayPass_DisplayUirdMsg (T_SHARED_DATA_STRUCT * pDataStruct, const int bPhoneMessage)
{
	...
	if (!__ClessSample_PayPass_RetrieveUirdMessage (pDataStruct, &nUirdMessage))
	...
	case PAYPASS_UIRD_MESSAGE_ID_APPROVED:
		...
		break;
	case PAYPASS_UIRD_MESSAGE_ID_APPROVED_SIGN:
		ClessSample_GuiState_DisplayScreenText (CLESS_SAMPLE_SCREEN_SIGNATURE_OK, nMerchantLanguage, nCardHolderLanguage);
		break;
	case PAYPASS_UIRD_MESSAGE_ID_DECLINED:
...
int ClessSample_PayPass_CustomiseStep (T_SHARED_DATA_STRUCT * pSharedData, const unsigned char ucCustomisationStep)
{
	...
	case (STEP_PAYPASS_MCHIP_GET_CERTIFICATE):
		...
		{
			// An error occurred when retrieving the CA Public Key data in the parameters
...
//! \param[in] pResultDataStruct Structure containing the C2 kernel output.
//! \return
//!		- \ref TRUE if no problem occurred.
//!		- \ref FALSE if an error occurred.
...
static int __ClessSample_PayPass_FillBufferForOnlineAuthorisation (T_SHARED_DATA_STRUCT * pTransactionData, T_SHARED_DATA_STRUCT * pOnlineAuthorisationData)
{
	...
	if ((pTransactionData != NULL) && (pOnlineAuthorisationData != NULL))
	{
		...
		{
			GTL_Traces_TraceDebug ("__ClessSample_PayPass_FillBufferForOnlineAuthorisation : Unable to retrieve Data Record");
			bErrorDetected = TRUE;
		}
		...
		if (!bErrorDetected)
		{
			...
			{
				GTL_Traces_TraceDebug ("__ClessSample_PayPass_FillBufferForOnlineAuthorisation : Unable to retrieve discretionary Data");
				bErrorDetected = TRUE;
			}
		}
...
int ClessSample_PayPass_PerformTransaction (T_SHARED_DATA_STRUCT * pDataStruct)
{
	...
	{	
		...
		if (!__ClessSample_PayPass_RetrieveTransactionOutcome (pDataStruct, &nTransactionOutcome))
		...
		if (!__ClessSample_PayPass_RetrieveCvmToApply (pDataStruct, &ucCvm))
			ucCvm = PAYPASS_OPS_CVM_NA;

		if (!__ClessSample_PayPass_RetrieveFieldOffValue (pDataStruct, &ucFieldOff))
			ucFieldOff = PAYPASS_OPS_FIELD_OFF_REQUEST_NA;

		// Retrieve the card type
		if (!__ClessSample_PayPass_RetrieveCardType (pDataStruct, &usCardType))
			usCardType = 0;
		...
		case (PAYPASS_OPS_STATUS_APPROVED):
			...
			{
				ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_SCREEN_SIGNATURE_REQUIRED, -1, -1);
			}
			else
			{
				// Display the message
			...
			if (ucCvm == PAYPASS_OPS_CVM_SIGNATURE)
			{
				...
				if (HelperQuestionYesNo (&tMsg, 30, merchLang))
				{
					...
					ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_SCREEN_SIGNATURE_OK, -1, -1);
					bSaveInBatch = TRUE;
				}
				else
				{
					// Signature is incorrect
					ClessSample_GuiState_DisplayScreen (CLESS_SAMPLE_SCREEN_SIGNATURE_KO, -1, -1);
				}
			}
			break;
		...
		default: // Error case
			...
			if ((nTransactionOutcome == PAYPASS_OPS_STATUS_END_APPLICATION) && (nTransactionStart == PAYPASS_OPS_START_B))
			{
				...
				__ClessSample_PayPass_RetrieveErrorIndicationByte (pDataStruct, PAYPASS_EI_L1_BYTE, &ucErrorIndicationByte);
			...
			else // Standard error case
			{
				...
				__ClessSample_PayPass_RetrieveErrorIndicationByte (pDataStruct, PAYPASS_EI_L3_BYTE, &ucErrorIndicationByte);
...
static void __ClessSample_PayPass_AddRecordToBatch (T_SHARED_DATA_STRUCT * pSharedData, unsigned short usCardType)
{
	...
	T_TI_TAG tRequestedTags[] = {TAG_PAYPASS_DATA_RECORD, TAG_PAYPASS_DISCRETIONARY_DATA, TAG_EMV_TRANSACTION_TYPE, TAG_EMV_TRANSACTION_DATE};
	...
	if (pBatchData != NULL)
	{
		...
		if (nResult == KERNEL_STATUS_OK)
		{
			...
			{
				GTL_Traces_TraceDebug ("__ClessSample_PayPass_AddRecordToBatch : Unable to retrieve Data Record");
				bErrorDetected = TRUE;
			}
			...
			if (!bErrorDetected)
			{
				...
				{
					GTL_Traces_TraceDebug ("__ClessSample_PayPass_AddRecordToBatch : Unable to retrieve discretionary Data");
					bErrorDetected = TRUE;
				}
				...
				if (!bErrorDetected)
				{
					...
					{
						if (GTL_SharedExchange_AddTag (pBatchData, TAG_EMV_TRANSACTION_TYPE, ulReadLength, pReadValue)!= STATUS_SHARED_EXCHANGE_OK)
						{
							GTL_Traces_TraceDebug ("__ClessSample_PayPass_AddRecordToBatch : Unable to copy the local structure with discretionary data");
							bErrorDetected = TRUE;
						}
					...
					{
						if (GTL_SharedExchange_AddTag (pBatchData, TAG_EMV_TRANSACTION_DATE, ulReadLength, pReadValue)!= STATUS_SHARED_EXCHANGE_OK)
						{
							GTL_Traces_TraceDebug ("__ClessSample_PayPass_AddRecordToBatch : Unable to copy the local structure with discretionary data");
							bErrorDetected = TRUE;
						}
...
{
	// Local variables
	int cr, nPositionElement;
	unsigned long ulAidReadLengthElement;
	unsigned char *pAidReadValueElement;

	// Protection against bad parameters
	if ((pKernelDataStruct == NULL) || (pAidParameters == NULL))
		return(FALSE);

	// TAG_EP_CLESS_FLOOR_LIMIT : 0x9F92810F
	nPositionElement = SHARED_EXCHANGE_POSITION_NULL;
	cr = GTL_SharedExchange_FindNext (pAidParameters, &nPositionElement, TAG_EP_CLESS_FLOOR_LIMIT, &ulAidReadLengthElement, (const unsigned char **)&pAidReadValueElement);
	if (cr == STATUS_SHARED_EXCHANGE_OK)  // If tag with AID informations found
	{
		GTL_SharedExchange_AddTag(pKernelDataStruct, TAG_EP_CLESS_FLOOR_LIMIT, ulAidReadLengthElement, pAidReadValueElement);
	}

	// TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_NO_DCV : 0x9F91851C
	nPositionElement = SHARED_EXCHANGE_POSITION_NULL;
	cr = GTL_SharedExchange_FindNext (pAidParameters, &nPositionElement, TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_NO_DCV, &ulAidReadLengthElement, (const unsigned char **)&pAidReadValueElement);
	if (cr == STATUS_SHARED_EXCHANGE_OK)  // If tag with AID informations found
	{
		GTL_SharedExchange_AddTag(pKernelDataStruct, TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_NO_DCV, ulAidReadLengthElement, pAidReadValueElement);
	}

	// TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_DCV : 0x9F91851D
	nPositionElement = SHARED_EXCHANGE_POSITION_NULL;
	cr = GTL_SharedExchange_FindNext (pAidParameters, &nPositionElement, TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_DCV, &ulAidReadLengthElement, (const unsigned char **)&pAidReadValueElement);
	if (cr == STATUS_SHARED_EXCHANGE_OK)  // If tag with AID informations found
	{
		GTL_SharedExchange_AddTag(pKernelDataStruct, TAG_PAYPASS_CLESS_TRANSACTION_LIMIT_DCV, ulAidReadLengthElement, pAidReadValueElement);
	}

	// TAG_EP_CLESS_CVM_REQUIRED_LIMIT : 0x9F92810E
	nPositionElement = SHARED_EXCHANGE_POSITION_NULL;
	cr = GTL_SharedExchange_FindNext (pAidParameters, &nPositionElement, TAG_EP_CLESS_CVM_REQUIRED_LIMIT, &ulAidReadLengthElement, (const unsigned char **)&pAidReadValueElement);
	if (cr == STATUS_SHARED_EXCHANGE_OK)  // If tag with AID informations found
	{
		GTL_SharedExchange_AddTag(pKernelDataStruct, TAG_EP_CLESS_CVM_REQUIRED_LIMIT, ulAidReadLengthElement, pAidReadValueElement);
	}
