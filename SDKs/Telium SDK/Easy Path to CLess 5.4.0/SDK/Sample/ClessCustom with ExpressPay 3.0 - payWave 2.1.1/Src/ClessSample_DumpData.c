...
void ClessSample_DumpData_DumpTlvTreeNodeWithTitle (char * pLabel, TLV_TREE_NODE TlvTreeNodeToDump)
{
	...
	if (TlvTreeNodeToDump != NULL)
	{
		...
		if (ClessSample_DumpData_DumpOpenOutputDriver())
		{
			...
			while (pResearch != NULL)
			{
				...
				ClessSample_DumpData ("T:%08x L:%02x V:", TlvTree_GetTag (pResearch), nDataLength);
...
void ClessSample_DumpData_DumpTitle (char * pLabel)
{
	if (ClessSample_DumpData_DumpOpenOutputDriver())
	{
		if (ClessSample_DumpData_DumpGetOutputId() == CUSTOM_OUTPUT_PRINTER)
