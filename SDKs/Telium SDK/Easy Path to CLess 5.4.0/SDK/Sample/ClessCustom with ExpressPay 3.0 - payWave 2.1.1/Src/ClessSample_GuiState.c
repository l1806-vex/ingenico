...
extern T_GENERIC_TRANSACTION_DATA T_GenericTransactionData;  /*!< Global variable for transaction file. */
...
/*	case (CLESS_SAMPLE_SCREEN_ONLINE_PIN_REQUIRED):
	...
	case (CLESS_SAMPLE_SCREEN_SIGNATURE_OK):
		ClessSample_GuiState_DisplayScreenText (ulScreenIdentifier, nMerchantLang, nCardholderLang);
		if (!UIH_IsRemoveCardTaskRunning())
			HelperLedsOff();
		Helper_WaitUserDisplay();
	...
	case (CLESS_SAMPLE_SCREEN_SIGNATURE_KO):
		ClessSample_GuiState_DisplayScreenText (ulScreenIdentifier, nMerchantLang, nCardholderLang);
		if (!UIH_IsRemoveCardTaskRunning())
			HelperLedsOff();
		Helper_WaitUserDisplay();
	...
	case (CLESS_SAMPLE_SCREEN_STOPPED):
		ClessSample_GuiState_DisplayScreenText (ulScreenIdentifier, nMerchantLang, nCardholderLang);
		Helper_WaitUserDisplay();
		break;
...
/////////////////////////////////
	...
	case (CLESS_SAMPLE_EXPRESSPAY_SCREEN_SIGNATURE_OK_ONLINE_APPROVED):
		ClessSample_GuiState_DisplayScreenText(ulScreenIdentifier, nMerchantLang, nCardholderLang);
		if (!UIH_IsRemoveCardTaskRunning())
			HelperLedsOff();
		Helper_WaitUserDisplay();
	...
	case (CLESS_SAMPLE_EXPRESSPAY_SCREEN_PROCESSING):
		ClessSample_GuiState_DisplayScreenText(ulScreenIdentifier, nMerchantLang, nCardholderLang);
		if (!UIH_IsRemoveCardTaskRunning())
			HelperLedsOff();
		Helper_WaitUserDisplay();
...
/*	case (CLESS_SAMPLE_SCREEN_IDLE):
	...
	case (CLESS_SAMPLE_SCREEN_PRESENT_CARD):
		...
		bAmountCanBeDisplayed = T_GenericTransactionData.gs_bIsAmountPresent;
		...
		if (bAmountCanBeDisplayed)
		{
			...
			ClessSample_Common_FormatAmount(ucFormat, ClessSample_Fill_GiveMoneyLabel(), ucPosition, T_GenericTransactionData.gs_ulTransactionAmountBin, ucFormattedAmountMessage);
...
/*	case (CLESS_SAMPLE_SCREEN_PROCESSING):
	...
	case (CLESS_SAMPLE_SCREEN_PHONE_INSTRUCTIONS):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(STD_MESS_PHONE_INSTRUCTIONS_L1, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_2, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(STD_MESS_PHONE_INSTRUCTIONS_L2, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(STD_MESS_PHONE_INSTRUCTIONS_L1, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer(ERASE, HELPERS_CUSTOMER_LINE_1, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(STD_MESS_PHONE_INSTRUCTIONS_L2, nCardholderLanguage, &tMsg);
	...
	case (CLESS_SAMPLE_EXPRESSPAY_SCREEN_PROCESSING):
		if (auCustomerDisplayAvailable) // If a customer display is available
		{
			ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PROCESSING, nMerchantLanguage, &tMsg);
			 Helper_DisplayTextMerchant(ERASE, HELPERS_MERCHANT_LINE_3, &tMsg, NOLEDSOFF);
			ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PLEASE_WAIT, nMerchantLanguage, &tMsg);
			Helper_DisplayTextMerchant(NO_ERASE, HELPERS_MERCHANT_LINE_4, &tMsg, NOLEDSOFF);
			Helper_RefreshScreen(NOWAIT, HELPERS_MERCHANT_SCREEN);
		}
		ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PROCESSING, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (ERASE, HELPERS_CUSTOMER_LINE_2, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		ClessSample_Term_Read_Message(EXPRESSPAY_MESS_PLEASE_WAIT, nCardholderLanguage, &tMsg);
		Helper_DisplayTextCustomer (NO_ERASE, HELPERS_CUSTOMER_LINE_3, &tMsg, CLESSSAMPLE_ALIGN_CENTER, NOLEDSOFF);
		Helper_RefreshScreen(NOWAIT, HELPERS_CUSTOMER_SCREEN);
		break;
