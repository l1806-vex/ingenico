...
static int _ClessSample_DisplayEnterPinMsg(int nb_try, int *PtrUsePinpad, int nCardHolderLang);
	...
	*PtrUsePinpad = (int)ClessSample_IsPinpadPresent();
	...
	if ((TPass_GetDeviceType() == TPASS_CLESS_TYPE_IPP2XX) && (!ClessSample_UseGraphicalFunctionOnPPAD()))
	{
		if ((ul_InputAmount % 100) != 0)
		    sprintf(buffer, "%s%lu.%02lu %s", tMSG.message, ul_InputAmount/100, ul_InputAmount%100, ClessSample_Fill_GiveMoneyLabel());
		else
		    sprintf(buffer, "%s%lu %s", tMSG.message, ul_InputAmount/100, ClessSample_Fill_GiveMoneyLabel());
	}
	else
	{
		if ((ul_InputAmount % 100) != 0)
		    sprintf(buffer, "%s%lu.%02lu %s\n", tMSG.message, ul_InputAmount/100, ul_InputAmount%100, ClessSample_Fill_GiveMoneyLabel());
		else
		    sprintf(buffer, "%s%lu %s\n", tMSG.message, ul_InputAmount/100, ClessSample_Fill_GiveMoneyLabel());
	}
...
static unsigned char _ClessSample_EnterPin(int nb_try, BUFFER_SAISIE *buffer, int nCardHolderLang)
{
	...
	pinpadok = _ClessSample_DisplayEnterPinMsg(nb_try, &PinPadUsed, nCardHolderLang);
	...
	cr_enter_pin = _ClessSample_ManageKeys(buffer, PinPadUsed);	// pinpad must be used if present
...
static unsigned char _ClessSample_ManageKeys(BUFFER_SAISIE *buffer, int isPinpad)
{
	...
	MSGinfos       MSGTmp;
	T_CUTERMkey    key = 0; // Initialisation value not used, allows to avoid compilation warning
	PIN_RESULT     cr;
