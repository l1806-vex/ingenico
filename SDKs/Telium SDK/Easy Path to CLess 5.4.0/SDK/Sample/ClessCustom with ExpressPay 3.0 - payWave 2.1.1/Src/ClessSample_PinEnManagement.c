...
static int _ClessSample_LoadRootKey(void)
{
	...
	// It is necessary to remove the previously loaded key before loading it again. If not, the LoadKey fails.
...
int ClessSample_InjectKey (void)
{
	...
	if (iret != OK)
	{
		...
		sprintf(acStr, "SEC_isSecretArea %d\n",iret);
		upload_diagnostic_txt((unsigned char *)acStr);
	}
	else
	{
		...
		if (iret != OK)
		{
			sprintf(acStr, "_ClessSample_LoadRootKey %d\n",iret);
			upload_diagnostic_txt((unsigned char *)acStr);
		}
	}
	return iret;
...
int ClessSample_OnlineEncipher (unsigned char pucPAN [],	unsigned char pucEncipheredPin [],int *iLenEnc)
{
	...
	if (iRes != OK)
	{
	    sprintf(acDebug, "SEC_Iso9564 %d\n",iRes);
    	upload_diagnostic_txt((unsigned char *)acDebug);
    	
	    sprintf(acDebug, " %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n",iso9564Result[0],iso9564Result[1],iso9564Result[2],
