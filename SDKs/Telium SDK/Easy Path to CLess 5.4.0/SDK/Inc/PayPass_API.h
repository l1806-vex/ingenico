/**
 * \file     PayPass_API.h
 * \brief    This describes the PayPass 2.x kernel API.
 *
 * \author    Ingenico
...
//! \addtogroup PayPassInterfaceFunctions
//! @{
...
//!		- \ref TAG_PAYPASS_TRANSACTION_CATEGORY_CODE : PayPass merchant category code.
...
//!		- \ref TAG_EP_AID_ADDITIONAL_RESULTS : Application selection additional results for the chosen AID (indicates the PPSE Application Selection results, such as floor limits exceeded, etc).
...
//!		- \ref TAG_EP_FINAL_SELECT_RESPONSE : Final SELECT response provided by the application selection. It will be analysed by the PayPass kernel itself.
//!		- \ref TAG_EP_FINAL_SELECT_STATUS_WORD : Final SELECT response status word provided by the application selection. It will be analysed by the PayPass kernel itself.
//!		- \ref TAG_PAYPASS_INT_MSTRIPE_TERMINAL_AVN_LIST : Terminal Application version number supported for MStripe cards for this AID.
//!		- \ref TAG_PAYPASS_INT_MCHIP_TERMINAL_AVN_LIST : Terminal Application version number supported for MChip cards for this AID.
//!		- \ref TAG_PAYPASS_TERMINAL_CAPABILITIES_CVM_REQ : Terminal capabilities to be used if CVM is required (amount greater than \a TAG_EP_CLESS_CVM_REQUIRED_LIMIT). Terminal capabilities is automatically set by the kernel depending on the application selection results.
//!		- \ref TAG_PAYPASS_TERMINAL_CAPABILITIES_NO_CVM_REQ : Terminal capabilities to be used if CVM is not required (amount lower than \a TAG_EP_CLESS_CVM_REQUIRED_LIMIT). Terminal capabilities is automatically set by the kernel depending on the application selection results.
//!		- \ref TAG_PAYPASS_DEFAULT_UDOL : Default UDOL (only necessary for MStripe transactions).
//!		- \ref TAG_PAYPASS_MSTRIPE_INDICATOR : Indicates if MStripe is supported or not for this AID.
//!		- \ref TAG_EMV_INT_TAC_DEFAULT : Terminal Action Code Default (MChip only).
//!		- \ref TAG_EMV_INT_TAC_DENIAL : Terminal Action Code Denial (MChip only).
//!		- \ref TAG_EMV_INT_TAC_ONLINE : Terminal Action Code Online (MChip only).
//!		- \ref TAG_PAYPASS_INT_SUPPORTED_CAPK_INDEX_LIST : List of CA keys supported for the chosen AID (MChip only).
...
//!		- <dfn>\ref TAG_PAYPASS_INT_USE_REFUND_FLOW</dfn> : Indicates if refund transaction flow shall be used or not. If not required, the default transaction flow is used.
...
//!			- \ref TAG_PAYPASS_TRANSACTION_OUTCOME : Indicates the transaction outcome (approved, online request, declined, use another interface or terminated). Note if a refund transaction is used and correctly completed, the transaction outcome is set to "Approved".
//!			- \ref TAG_KERNEL_CARD_TYPE : Card type (refer to \ref PayPassPossibleCardTypes for more information about the possible values).
//!		- Additional data for "Offline Approve" or "Online" MChip transactions :
//!			- \ref TAG_PAYPASS_TRANSACTION_CVM : Transaction CVM (No CVM, Signature or Online PIN).
...
//!		- Additional data for MStripe transactions :
//!			- \ref TAG_PAYPASS_TRACK2_DATA : Track 2 Data.
//!			- \ref TAG_PAYPASS_TRACK1_DATA : Track 1 Data (if present).
//!			- \ref TAG_PAYPASS_DD_CARD_TRACK1 : Discretionary Data Card Track 1 (if present).
//!			- \ref TAG_PAYPASS_DD_CARD_TRACK2 : Discretionary Data Card Track 2.
//!			- \ref TAG_PAYPASS_THIRD_PARTY_DATA : PayPass Third Party Data (if present).
//!			- \ref TAG_EMV_DF_NAME : DF Name.
//!			- \ref TAG_EMV_APPLICATION_LABEL : Application Label (if present).
//!			- \ref TAG_EMV_APPLI_PREFERED_NAME : Application Preferred Name (if present).
...
//!		Please refer to \ref PayPassTransactionFlowSteps for more information about the existing transaction steps.
...
//!	- \ref KERNEL_STATUS_CANCELLED : if transaction has been cancelled by the custom application (using \a PayPass_Cancel() function).
...
//!	anyway if requested (using \ref PayPass_ResumeTransaction() function).

int PayPass_DoTransaction (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	- Output data are the same as \ref PayPass_DoTransaction() function (as it continues the transaction).
//! \return Kernel processing status code (identical than the \ref PayPass_DoTransaction() function).
//! The returned value depends on the step where the transaction is resumed.
//!	For example, \ref KERNEL_STATUS_COND_OF_USE_NOT_SATISFIED cannot be returned if you resume the transaction after the \ref STEP_PAYPASS_INITIATE_APPLI_PROCESSING step.

int PayPass_ResumeTransaction (T_SHARED_DATA_STRUCT * pDataStruct);
...
int PayPass_GetAllData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	nResult = PayPass_GetData (pSharedDataStruct);
...
int PayPass_GetData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	nResult = PayPass_LoadData (pSharedDataStruct);
//!	if (nResult != KERNEL_STATUS_OK)
//!	{
//!		// An error occurred when requesting tags to the kernel.
//!		// ...
...
int PayPass_LoadData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	- \ref PayPass_DoTransaction().
//!	- \ref PayPass_ResumeTransaction().

int PayPass_Cancel (void);
...
//!	- Time measurements : it manages in the database the \ref TAG_KERNEL_DEBUG_TIME tag that can be retrieved using \ref PayPass_GetData() function).
...
//!	nResult = PayPass_DebugManagement (pSharedDataStruct);
//!	if (nResult != KERNEL_STATUS_OK)
//!	{
//!		// An error occurred when requesting tags to the kernel.
//!		// ...
...
int PayPass_DebugManagement (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	- \ref TAG_KERNEL_APPLICATION_IDENT that indicates the kernel identifier (in ASCII format, \a 3349 followed by the PayPass kernel version on 4 digits).
...
int PayPass_GetInfos (T_SHARED_DATA_STRUCT * pDataStruct);
