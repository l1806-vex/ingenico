/**
 * \file     ExpressPay_API.h
 * \brief    This describes the ExpressPay 2 kernel API.
 *
 *
 * \author    Ingenico
...
//! \addtogroup ExpressPayInterfaceFunctions
//! @{

//! \brief This function performs the ExpressPay transaction with the presented card.
...
//!		- \ref TAG_EXPRESSPAY_TERMINAL_CAPABILITIES : Specific ExpressPay terminal capabilities.
//!		- \ref TAG_EXPRESSPAY_FULL_ONLINE_EMV_REMOVAL_TIMEOUT : ExpressPay removal timeout.
//!		- \ref TAG_EXPRESSPAY_CONTACT_EMV_CAPABLE : set to 1 terminal is capable to perform a contact EMV transaction.
...
//!	- \ref KERNEL_STATUS_CANCELLED : if transaction has been cancelled by the custom application (using \a ExpressPay_Cancel() function).
...
//!	anyway if requested (using \ref ExpressPay_ResumeTransaction() function).

int ExpressPay_DoTransaction (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	- Output data are the same as \ref ExpressPay_DoTransaction() function (as it continues the transaction).
//! \return Kernel processing status code (identical than the \ref ExpressPay_DoTransaction() function).
//! The returned value depends on the step where the transaction is resumed.

int ExpressPay_ResumeTransaction (T_SHARED_DATA_STRUCT * pDataStruct);



//! \brief This function allows to perform the second part of an ExpressPay Full Online EMV Mode transaction.
//! This function shall be called after the \ref ExpressPay_DoTransaction() function and after the online authorization.
//!	It begins by analysing the provided Issuer Authorization response and perform the transaction until completion.
//! The ExpressPay kernel performs the following steps:
//!		- Issuer authentication (if Issuer Authentication Data received)
//!		- Perform SDA if requested and terminal was unable to go online
...
//!	- Output data are the same as \ref ExpressPay_DoTransaction() function (as it continues the transaction).
//! \return Kernel processing status code (identical than the \ref ExpressPay_DoTransaction() function) and:
...
int ExpressPay_DoTransactionPostProcessing (T_SHARED_DATA_STRUCT * pDataStruct);
...
int ExpressPay_GetAllData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	nResult = ExpressPay_GetData (pSharedDataStruct);
...
int ExpressPay_GetData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	nResult = ExpressPay_LoadData (pSharedDataStruct);
//!	if (nResult != KERNEL_STATUS_OK)
//!	{
//!		// An error occurred when requesting tags to the kernel.
//!		// ...
...
int ExpressPay_LoadData (T_SHARED_DATA_STRUCT * pDataStruct);
...
//!	- \ref ExpressPay_DoTransaction().
//!	- \ref ExpressPay_ResumeTransaction().

int ExpressPay_Cancel (void);
...
//!	- Time measurements : it manages in the database the \ref TAG_KERNEL_DEBUG_TIME tag that can be retrieved using \ref ExpressPay_GetData() function).
...
//!	nResult = ExpressPay_DebugManagement (pSharedDataStruct);
//!	if (nResult != KERNEL_STATUS_OK)
//!	{
//!		// An error occurred when requesting tags to the kernel.
//!		// ...
...
int ExpressPay_DebugManagement (T_SHARED_DATA_STRUCT * pDataStruct);
...
int ExpressPay_GetInfos (T_SHARED_DATA_STRUCT * pDataStruct);
