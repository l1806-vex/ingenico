...
static void EMVDC_START_Function(unsigned short * error_code);
...
void EMVCUST_Get_AID_ICS_TlvTree(TLV_TREE_NODE *phInputTLVTree, TLV_TREE_NODE *phOutputTLVTree)
{
	...
	const unsigned long RequiredTags[] = {TAG_TERMINAL_TYPE, TAG_TERMINAL_COUNTRY_CODE,
		...
		TAG_CUST_IS_CARDHOLDER_CONFIRMATION,TAG_CUST_IS_AMOUNT_UNKNOWN_BEFORE_CVM,TAG_CUST_IS_SELECTABLE_KERNEL,TAG_CUST_IS_DELETABLE_TAC,TAG_CUST_LOG_DURING_TR };
	unsigned char StatusCode[2];
...
//===========================================================================
void InitwStructList(wStructListe *pMenu, int flag, int offset)
{
	int MAXX, MAXY;
...
//_	{
	...
	if(Get_StateWGUI())
	{
		...
		IconLst.title.message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_EMV_APP, merchLang);
	}
	else
	{
...
#endif
	{
		for (index=0 ; index < ELVEC(NavigationEmvdc) ; index++ )
		{
			if(Get_StateWGUI())
			{
				InitMSGinfos(&IconLst.tab[index]);
				IconLst.tab[index].message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, NavigationEmvdc[index].itemlabel, merchLang);
				CUWGUI_load_icon(NavigationEmvdc[index].itemlabel, &IconLst.List[index].name_icon);
				IconLst.List[index].file_directory = (char *)file_dir;
			}
			else
			{
		...
		if(Get_StateWGUI())
		{
			IconLst.List[index].name_icon = NULL;
			IconLst.tab[index].message = NULL;
		}
		else
		{
			Menu.tab[index] = NULL;
...
{
#if ((!defined(_DONT_USE_F4_KEY_)) && defined(_USE_SPED_))
...
void EMVCUST_Periodic_Call_TLVTree(TLV_TREE_NODE *phOutputTLVTree)
{
	...
	cmpdate = SLC_Ecart_date (&DateNextTransf, &date);
	if (cmpdate == 0)
	{
		if (SLC_Ecart_heure (&DateNextTransf, &date) > 0)
		{
...
void EMVCUST_Process_Step_TlvTree(int function, TLV_TREE_NODE *phInputTLVTree, TLV_TREE_NODE *phOutputTLVTree)
{
	...
	switch (function)
	{
		case EMVDC_START:
			...
			EMVDC_START_Function(&error_code);
			break;
...
static void EMVDC_STOP_Function_Tlvtree(unsigned short *error_code, unsigned short *ptr_usMerchantLang)
{
	...
	if (isTrlog != TRUE)
	{
			...
			(*error_code == TAG_CUST_ICC_REDUNDANT_DATA) )
		{
			...
			{
				if (Get_TypeCode ((NO_SEGMENT) i) == I_EMVCUST_Type_Appli)
				{
#ifdef _USE_UCM_
					if(!UCM_Present)
#endif
...
void EMVCUST_Voice_Referral_TlvTree (TLV_TREE_NODE *phInputTLVTree, TLV_TREE_NODE *phOutputTLVTree)
{
	...
	if (elt.ptValue[0] == 1)
	{
		...
		else
		{
			...
			Menu.Fenetre.titre.message = (unsigned char *) CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_REFERRAL, lg_code);
			...
			Menu.tab[0].message = (unsigned char *) CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_APPROVE, lg_code);
			Menu.tab[1].message = (unsigned char *) CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_REJECT, lg_code);
			...
			for (index = 0; index < 3; index++)
			{
				...
				CUWGUI_load_yesno_icon(index, &MenuWgui.Icon[index].name_icon);
			}
...
void EMVCUST_Authorization_TlvTree (TLV_TREE_NODE *phOutputTLVTree)
{
	...
	else
	{
		...
		if ((cr !=1)&&(CUTERMgetTerminalType()==T_CUTERM_X930) && (Comm_Protocol != COMM_PROTOCOL_TYPE_YES ))
		{
...
void EMVCUST_Choose_Language_Menu_TlvTree (TLV_TREE_NODE *phOutputTLVTree)
{
	...
	if (foundM && foundS && iIsSupported)
	{
		...
		InitMSGinfos(&(Menu.Fenetre.titre));
		Menu.Fenetre.titre.message = (unsigned char *) CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_SELECT_LANG, merchLang);
		MenuWgui.title.message = Menu.Fenetre.titre.message;
		...
		for (i=0 ; (i * 2) < (int) eltS.length ; i++ )
		{
			...
			for (j = 0; j < C_NBLANGHOLDER; j++)
			{
					...
					(ucLangCode [1] == lang_table [j].iso639_code [1]))
				{
					...
					Menu.tab[i].message = (unsigned char *) lang_table [j].lang_name;

					InitMSGinfos(&(MenuWgui.tab[i]));
					MenuWgui.tab[i].message = (unsigned char *) lang_table [j].lang_name;

					// Optional icon
					MenuWgui.Icon[i].file_directory = (char *)file_dir;
					CUWGUI_load_flag_icon(lang_table[j].table_id, &MenuWgui.Icon[i].name_icon);
				}
			}
		}
		...
		if(UCM_Present)
		{
			...
			do
			{
				sprintf(ctMessage, "%.16s\n%s",Menu.tab[index].message, ptrMsg);
...
#endif
				if (strcmp (lang_table [j].lang_name, Menu.tab[buffer.d_entry[0]].message) == 0)
				{
					DataElementExt data;
					data.elt.length = 2;
...
void EMVCUST_Choose_Account_Type_Menu_TlvTree (TLV_TREE_NODE *phOutputTLVTree)
{
	...
	CUDB_Get_cardholder_language(&lg_code);
	InitMSGinfos(&(Menu.Fenetre.titre));
	Menu.Fenetre.titre.message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_ACCOUNT_TYPE, lg_code);
	MenuWgui.title.message = Menu.Fenetre.titre.message;
	...
	Menu.tab[0].message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_AT_SAVING ,lg_code);
	Menu.tab[1].message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_AT_DEBIT ,lg_code);
	Menu.tab[2].message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_AT_CREDIT ,lg_code);
	Menu.tab[3].message = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_AT_default ,lg_code);
	Menu.tab[4].message = NULL;
	...
	for (index = 0; index < 5; index++)
	{
		...
		// Optional icon
		MenuWgui.Icon[index].file_directory = (char *)file_dir;
		CUWGUI_load_trans_icon(index, &MenuWgui.Icon[index].name_icon);
	}
		...
		if (elt.ptValue[0] == 1)
		{
		...
		#endif
			{
				...
				else
				{
					...
					InitMSGinfos(&(Menu.Fenetre.titre));
					Menu.Fenetre.titre.message = pc_l1_DisplayTxt;
					MenuWgui.title.message = Menu.Fenetre.titre.message;
