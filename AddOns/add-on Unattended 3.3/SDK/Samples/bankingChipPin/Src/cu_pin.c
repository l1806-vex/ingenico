...
#ifdef _USE_SPED_
static unsigned char cardonsped = FALSE;
#endif
...
#ifdef _USE_SPED_
static unsigned char ExtInputKeysWithSchemes (void);
#endif
...
/*&END FT/09/5296/MA    SEC_ISO9563 has to be called only if Injection successfull */
         trace(0x6E02,strlen("CUPIN_EnterPinOnLine"),"CUPIN_EnterPinOnLine");
	...
	if ( cr_enter_pin == CU_INPUT_PIN_OK )
	{
		...
		{
			unsigned char  security_status = 0;
			unsigned short int cr_read_online_pin = 0;

			cr_read_online_pin = Ext_Read_Online_Pin (0,(BUFFER_SAISIE*) buffer, &security_status);

			if ((cr_read_online_pin==CR_SAISIE_PINPAD_OK)&&(security_status!=2)&&(security_status!=3))
			{
				cr_enter_pin = INPUT_PIN_ON;
			}
			else
									...
									unsigned short * ptrBypass)
{
	...
	while  (cr_enter_pin == ERROR_INPUT)
	{
         ...
         trace(0x6E02,strlen("CUPIN_EnterPinOffLine"),"CUPIN_EnterPinOffLine");
...
static int DisplayEnterPinMsg(int nb_try, unsigned char PPTestType, unsigned char AllowTermIfPPadHS, int *PtrUsePinpad)
{
	...
	*PtrUsePinpad = FALSE;
	...
	if (UCM_Present)
	{
		...
		iLIBUCM_Display_Option( UCMC_DISPLAY, pc_l_DisplayTxt, UCMDISPLAY_APPEND|UCMDISPLAY_NEW_LINE | UCMDISPLAY_CLOSE_AT_END |UCMDISPLAY_CENTER, 0 ) ; /* UCM */
...
#endif
	{
		...
		if (PPTestType == CU_PPTEST_NOTEST)
			*PtrUsePinpad = FALSE;
#ifdef _USE_SPED_
		else if (PPTestType == CU_PPTEST_CARDONPPAD)
			*PtrUsePinpad = CUTERMCardOnPPAD();
#endif
...
static unsigned char SelectInputKeyMethod (int nb_try, ENTRY_BUFFER *buffer, unsigned char type_pin, unsigned char AllowTermIfPPadHS)
{
	...
	if (UCM_Present)
	{
...
#ifdef _USE_SPED_
		/*&BEGIN FT/08/3149/MA   faulty management of pin input on Sped terminal */
		if (CUTERMIsSpedConnected() == B_TRUE)
		{
			int PinPadUsed;

			PPTestType = CU_PPTEST_CARDONPPAD;

			cardonsped = CUTERMCardOnPPAD ();
			DisplayEnterPinMsg(nb_try, PPTestType, FALSE, &PinPadUsed);
			if (cardonsped)
			{
				Method_selector = CU_METHOD_EXT_WITH_SCHEMES;
			}
			else
			{
		...
		/*&END FT/08/3149/MA   faulty management of pin input on Sped terminal */
...
#ifdef _USE_SPED_
					cr_enter_pin = ExtInputKeysWithSchemes();
#endif
					break;
								...
								int isPinpad, int PinpadValue,int type_pin)
{
	...
	while(!end_enter_pin)
	{
		...
		{
			trace(0x6E02,strlen("ManageKeys begin CU_SchGetKey"),"ManageKeys begin CU_SchGetKey");
		...
		else if ((isPinpad == B_TRUE) && (KeyFunction & CU_CUTERM_GETPPPIN))
		{
			...
			trace(0x6E02,strlen("begin CUTERMgetPPpin"),"begin CUTERMgetPPpin");
		...
		{
			// use TRACE  Id 0xA000 : my application
			trace(0x6E02,strlen("begin CUTERMgetKey"),"begin CUTERMgetKey");
			...
			case CORRECT:
				...
				if (range!=0)
				{
					...
					else
					{
						...
						if (UCM_Present)
						{
							iLIBUCM_Display_Option( UCMC_DISPLAY, text, UCMDISPLAY_CLEAR | UCMDISPLAY_OPEN_IF_NOT | UCMDISPLAY_APPEND, 0 ) ;
						}
						else
#endif
						{
	...
	if (ksu_sch) 
		if (cr_initsch == 0)
		{
			...
			// use TRACE  Id 0xA000 : my application
			trace(0x6E02,strlen("debut pin entry SEC_PinEntry stop cmd"),"debut pin entry SEC_PinEntry stop cmd");

			cr_endsch = SEC_PinEntry (&uiEventToWait,&ucdataout, &ibToContinue);

			// use TRACE  Id 0xA000 : my application
			trace(0x6E02,strlen("fin pin entry SEC_PinEntry stop cmd"),"fin pin entry SEC_PinEntry stop cmd");
		}
...
//===========================================================================
//! \brief This function manages the keys entry with external pinpad using shemes (as P30 pinpad).
//! Only the final input with OK, CORRECT, CANCEL, TIMEOUT will be managed by this function.
//! \return The encipherement result : 
//! - PP_HS : Problem with the pinpad, or with the booster.
//! - CU_INPUT_PIN_OK : PIN entry and encipherement was successfull.
//! - CANCEL_INPUT : PIN entry was canceled.
...
static unsigned char ExtInputKeysWithSchemes (void)
{
	...
	unsigned char    cr_pin_Request = 0;
	TAB_ENTRY_STRING entry;

	// Local variables initialisation
	cr_enter_pin = CANCEL_INPUT;

	Ext_Clear_Line (0);
	Ext_Display(0,"ENTER PIN\n");

	entry.mask = 0x0000;				// (numerics,green,red,yellow keys enabled)
	entry.time_out = 120;
	entry.line     = 1;					// Echo on line 1
	entry.column   = 0;
	entry.echo     = 2;					// Echo stars
	entry.nb_max   = 12;				// up to 12 digits
	entry.nb_min   = 4;					// 4 digits min

	cr_pin_Request = Ext_Pin_Entry_Request (0, &entry,NULL);

	switch (cr_pin_Request)
	{
	case ICC_OK :
		// end of pin entry
		cr_enter_pin = CU_INPUT_PIN_OK;
	...
	case ICC_PIN_CANCELLED :
		cr_enter_pin = CANCEL_INPUT;
		break;

	case ICC_PIN_ENTRY_TIMEOUT :
		cr_enter_pin = TO_INPUT;
		break;
...
static T_Bool CU_SchGetKey(  T_CUTERMkey *xp_key,unsigned int ihid_key)
{
	...
	// use TRACE  Id 0xA000 : my application
	trace(0x6E02,strlen("begin pin entry"),"begin pin entry");
	...
	{
		upload_diagnostic_txt(" Errors detected\n"); 
		res_key = FALSE;
	}
...
#ifdef _USE_SPED_
		if (CUTERMIsSpedConnected() == B_FALSE)
		{
#endif
			*PinpadValue=0xff;
...
int  CU_SetPinEntry ( short TO_inter_car, short TO_first_car,unsigned int * ihid_key, int type_pin)
{
	...
	*ihid_key= 0x2A;
	...
	if (UCM_Present)
	{
		...
		pinplace = C_SEC_CARD ; /*C_SEC_PINCODE;*/
	}
	else
#endif
	{
	...
	// use TRACE  Id 0xA000 : my application
	trace(0x6E02,strlen("SEC_PinEntryInit"),"SEC_PinEntryInit");

	iret = SEC_PinEntryInit (&stEntryConfig,pinplace);
	
	// use TRACE  Id 0xA000 : my application
	trace(0x6E02,strlen("end SEC_PinEntryInit"),"end SEC_PinEntryInit");
