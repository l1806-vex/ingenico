...
void CUPRINT_PrintString(unsigned char *pucString_p, int iWait_p )
{
   ...
   if(UCM_Present)
   {
      if( iLIBUCM_Print_Exist(UCMC_PRINT ) > 0 )
      {
         if( iWait_p != 0 )
         {
            iLIBUCM_Print_Option(UCMC_PRINT, pucString_p, UCMPRINT_NO_END | UCMPRINT_WAIT_END);
         }
         else
         {
            iLIBUCM_Print_Option(UCMC_PRINT, pucString_p, UCMPRINT_NO_END);
         }
      }
   }
#else
...
static void print_tail(void)
{
	...
	CUPRINT_String_XT(CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_1, g_language), ALIGN_LEFT, L0);
   CUPRINT_String_XT(CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_2, g_language), ALIGN_LEFT, L0);

   CUPRINT_String_XT(CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_3, g_language), ALIGN_LEFT, L0);
   CUPRINT_String_XT( CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_4, g_language), ALIGN_LEFT, L0);   
#else
	CUPRINT_String_XT(CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SAGEM_MONETEL, g_language), ALIGN_CENTER, L1);
	CUPRINT_Blanks_Lines(1);
