...
void EMVCUST_Process_Step_TlvTree(int function, TLV_TREE_NODE *phInputTLVTree, TLV_TREE_NODE *phOutputTLVTree)
{
   ...
   sprintf( cTemp, "EMVCUST_Process_Step_TlvTree %d", function);
...
#endif
			// Refashion of Pin Pad management file
 			trace(0x6E02,strlen("EMVDC_CARDHOLDER_VERIFICATION_FIRST_Function_Tlvtree"),"EMVDC_CARDHOLDER_VERIFICATION_FIRST_Function_Tlvtree");
...
#endif
			// Refashion of Pin Pad management file
 			trace(0x6E02,strlen("EMVDC_CARDHOLDER_VERIFICATION_OTHER_Function_Tlvtree"),"EMVDC_CARDHOLDER_VERIFICATION_OTHER_Function_Tlvtree");
	...
	if (isTrlog != TRUE)
	{
			...
			(*error_code == TAG_CUST_ICC_REDUNDANT_DATA) )
		{
			...
			for (i = 0; i < seg_number; i++)
			{
                  	...
                  	{                  		
      				if (info.application_type == I_EMVCUST_Type_Appli)
      				{
...
void EMVCUST_Choose_Account_Type_Menu_TlvTree (TLV_TREE_NODE *phOutputTLVTree)
{
	...
	trace(0x6E02,strlen("EMVCUST_Choose_Account_Type_Menu"), "EMVCUST_Choose_Account_Type_Menu");
...
static void Forced_Acceptance(int function, unsigned char *is_forced_acceptance)
{
	...
	if (found)
		if (elt.ptValue[0] == 1)
		{
			...
			if(UCM_Present)
			{
                   if( giDisplayType  == PARAM_TYPE_IUP250 )
                   {
                     pc_l2_DisplayTxt = CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_YES_NO, lg_code);
                   }
...
static int Is_COMM_COMPONENT_Loaded (unsigned char type_appli)
{
	...
	for (i = 0; i < seg_number; i++)
	{
		if(ObjectGetInfo(OBJECT_TYPE_APPLI,i,&info) ==0)
		{
		   if ((unsigned char)info.application_type == type_appli)
		   {
