...
typedef struct {
   ...
   unsigned char       ucOption ;            /* not used must be set 0 1=flash */
...
typedef struct {
   unsigned char    ucType ;      /* 0=None, 1=major Display = Display1,  2=Display2  3=All Display,  0x10=IUR  0x77=All BlackLight */
   unsigned char    ucColorDefined ;  /* use UCMCDISPLAY_BKL_COLOR_NO_DEFINED , UCMCDISPLAY_BKL_COLOR_RED ...*/
   unsigned char    ucOption ;            /* not used  */
   unsigned short   usBlue;                 /* color definition */
   unsigned short   usGreen;              /* color definition */ 
   unsigned short   usRed;                  /* color definition */
   unsigned short   usFlashTimeOn;     /* usFlashTimeOn and usFlashTimeOff must be set 0 if no flash*/
   unsigned short   usFlashTimeOff;    /* 10ms step */
   unsigned short   usRuf ;    /* Ruf */
 } T_UCMC_IAC_BACKLIGHT_COLOR;
...
#define  UCMC_BUZ_NORMAL_10MS 0x04
#define  UCMC_BUZ_HIGH_10MS      0x05
#define  UCMC_BUZ_BASS_10MS      0x06

typedef struct
{
	unsigned char ucCmd ;      /* UCMC_BUZ_OFF,  UCMC_BUZ_NORMAL,  UCMC_BUZ_HIGH, UCMC_BUZ_BASS, UCMC_BUZ_NORMAL_10MS ...*/
	unsigned short usDelay ;   /* second or 10ms if used UCMC_BUZ_xxx_10MS*/
} T_UCMC_IAC_BUZ ;
...
#define UCMDEVICE_CMD_GETDLLSECU 0xF006
...
typedef struct
{
    ...
    union
    {
        ...
        T_UCMC_IAC_BACKLIGHT_COLOR sBacklight;
...
#define UCMTTESTALL_CR_ABORD_STOP   ( 7 )		/* Abord by iLIBUCM_Device_Ttestall_Stop */
...
extern int iLIBUCM_Device_Backlight_Color( T_UCMC_IAC_BACKLIGHT_COLOR *ptBlkColor_p );
extern int iLIBUCM_Device_Ttestall_Stop( void );
