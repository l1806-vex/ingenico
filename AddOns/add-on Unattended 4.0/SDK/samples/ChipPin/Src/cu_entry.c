...
extern void InitStructList(StructListe *pMenu, int flag, int offset);
...
int iUcmIac_BckColorDevice( unsigned char ucType_p, unsigned char ucCmd_p, unsigned short usFlashon_p, unsigned short usFlashoff_p )
{
   int iRet;
   T_UCMC_IAC_BACKLIGHT_COLOR sBlkColor;

   sBlkColor.ucType = ucType_p;
   sBlkColor.ucColorDefined = ucCmd_p;
   sBlkColor.ucOption = 0;
   sBlkColor.usBlue = 0;
   sBlkColor.usGreen = 0;
   sBlkColor.usRed = 0;
   sBlkColor.usFlashTimeOn= usFlashon_p;  /* 0 if no flash */
   sBlkColor.usFlashTimeOff= usFlashon_p;

   iRet = iLIBUCM_Device_Backlight_Color( &sBlkColor );
...
void WaitingRemovedCard(void)
{
   ...
   {
      iUcmIac_BckColor(1, UCMCDISPLAY_BKL_COLOR_BLUE); /*  IUR250 */ 
...
/***********************************************/
int iSetSleepPowerManagement (int iPwr_p, int iDaily_p, int iWakeup_p)
{
   ...
   tPower.iDelay = 0 ;
   tPower.iWakeup = iWakeup_p ;   /* with light Type */
   tPower.iRuf = 0;
   ...
   /* Ko if power is set by manager menu -16972 */
   iRet = iLIBUCM_Device_Cmd( UCMDEVICE_CMD_SET_POWER_MNG, &tPower2, &iLg, &iRet2 ) ;
   /* iRet2 gives error parameters number of tpower structure */
   
   sprintf ( cTemp,  "SET_POWER_MNG= %d\niLg=%d iRet=%d",
      iRet, iLg, iRet2) ;
   trace( 0x6E01, strlen( cTemp), cTemp ) ;
...
char cTemp[ 200 ];
  ...
  iLIBUCM_Display_Message( UCMC_DISPLAY, "TEST ATR", 100 ) ;
   if( ucWaiting_CardInsert( 800 ) == UCMC_STATUS_CARD_INSIDE)
   {
      ...
      sprintf( cTemp, "EMV POWER ON\n %d Lg=%d", iRet, Histo.length );
      iLIBUCM_Display_Message( UCMC_DISPLAY, cTemp, 100 ) ;
      ...
      sprintf( cTemp, "EMV POWER ATR\n %d Lg=%d", iRet, sAtr.length );
      iLIBUCM_Display_Message( UCMC_DISPLAY, cTemp, 100 ) ;
...
char cTemp[ 200 ];
   ...
   iLIBUCM_Display_Message( UCMC_DISPLAY, "TEST SYNCHRO", 100 ) ;
   if( ucWaiting_CardInsert( 800 ) == UCMC_STATUS_CARD_INSIDE)
   {
      iRet = iLIBUCM_Icc_Open( UCMC_ICC, "rw" ) ;
      ...
      /* HISTORIC BYTE */
      Histo.length = 0;
      Histo.historic[0] = T2G;
      iRet = iLIBUCM_Icc_PowerOn(UCMC_ICC , UCMCICC_POWER_ON_SYNC, &Histo ) ;
      trace( 0x6E01, Histo.length, &Histo.historic[0] ) ;

      sprintf( cTemp, "EMV POWER SYNC\n %d Lg=%d", iRet, Histo.length );
      iLIBUCM_Display_Message( UCMC_DISPLAY, cTemp, 200 ) ;
...
static   t_topstack     *gsStopTaskHdl;
Word sStopTtestall_Task(void)
{
char cTemp[200];
int iRet;
   ...
   while(1)
   {
      ...
      iRet = iLIBUCM_Device_Ttestall_Stop( );

      sprintf( cTemp, "sStopTtestall_Task %d", iRet );
      trace( 0x6E01, strlen( cTemp), cTemp);      
   }
return( 0 );
}
...
/***********************************************/
/*  Sample Stop ttestall                                               */
/*                                                                               */
/***********************************************/
int iTestStopTtestall( void )
{
int iRet;
char cTemp[ 200 ];
unsigned short usEvent;
int num_task_ext;
   ...
   /* task creation */
   gsStopTaskHdl = fork(sStopTtestall_Task, NULL, NULL);
   num_task_ext = GiveNoTask (gsStopTaskHdl);

   
   iLIBUCM_Display_Message( UCMC_DISPLAY, "STOP TTESTALL", 100 ) ;
   iRet = iLIBUCM_Device_Ttestall( &usEvent, 1600 );

   sprintf( cTemp, "Event %x\niRet=%d", usEvent, iRet );
...
//! - PAY_KO : otherwise.
//===========================================================================
void cu_debit_emv_Vending(NO_SEGMENT AppliNum, S_TRANSIN *param_in,   S_TRANSOUT *param_out)
{
   DataElement	AnElt;
	...
	do
	{
		switch(ucState)
		{
			...
			case STATE_DEBIT:
			{
					  ...
					  ( sDebit.u.tDaAskForDebit.ucCmd == UCMCPAY_SOLV ) )
				{
					...
					if (sDebit.u.tDaAskForDebit.ucSelectionNotDefined==0)
					{
						...
						strcpy ( sRDeb.tAppLibelle, (const char *)g_szAppName ) ;
			...
			case STATE_REMOVE_CARD:
			{
				...
				if( ( i == PARAM_TYPE_IUC180 ) || ( i == PARAM_TYPE_IUP250 ) || (i == PARAM_TYPE_IUR250) )
				{
					iUcmIac_BckColor( 1, UCMCDISPLAY_BKL_COLOR_GREEN);
...
*----------------------------------------------------------------------------
*/
void UTIL_InitStruct_ParamVmc ( T_UCMHOST_DA_PARAMV5 * pParamDa_p, int ibacklight_p )
{
int                    iIndx1 ;
	...
	pParamDa_p->ucNbSelection = 0;
      ...
      pParamDa_p->ucEVA= ibacklight_p;
      
      pParamDa_p->ucAppMaxRspTime = 255;  /* pour MDB */
...
*----------------------------------------------------------------------------
*/
static void UTIL_InitStruct_ParamVmcMsg ( T_UCMHOST_DA_PARAM_MSGV3 * pParamDaMsg_p )
{
   ...
   strcpy ((char*)pParamDaMsg_p->tucMsgProductSelected ,"\n    PRESENT\n   YOUR CARD ") ;
...
//	sprintf(g_szAppName, "EMV_%04X", CustApplicationNumber);
		...
		if( sStatus.ucUCM == 3 /* Maintenance */ )
		{
         giModeMainteance = 1;
			/* UCMC_EVENT_TYPE_KEY not available in mainteannce mode */
			 /* adding service USB event */
			 sEvent.usType = UCMC_EVENT_TYPE_USB;
		}
		else
		{
			 /* adding service function key:  Warning: UCMC_EVENT_TYPE_KEY service decrease perfomance */
  ...
  /**************************/
   ...
   iRet = iLIBUCM_Display_Exist(UCMC_DISPLAY);
	sprintf( cTemp, "after_reset chip pin display ICC %d", iRet);
	trace( 0x6E01, strlen(cTemp), cTemp);
  ...
  if( iRet == 1 ) /* no DLL Protocole  */
  {
   ...
   if( giDisplayCanal > 0 )   
   {
     ...
     {
         case 0: /* no protocole */
            iGraphic_message( "CHIP PIN\nNO PROTOCOL", 200, _MEDIUM_ );            
         break;

         case 1: /* banking DLL SES1042 French Domain */
            iGraphic_message( "CHIP PIN\nBANKING PROTOCOL", 200, _MEDIUM_ );            
         break;
         
         case 2: /* vending DLL EXE MDB */
            iGraphic_message( "CHIP PIN\nVENDING PROTOCOL", 200, _MEDIUM_ );               
         break;
      }    
  }
   ...
   {
      /* init parameters for EXE MDB DLL */
      ...
      {     
         ...
         if ( iLIBUCM_Pay_Host_Cmd ( iSize, &sHostCmd, &sHostCmdAnsw ) == FCT_OK )
         {
            ...
            iBackligthManagedByDLLVending = sParam.ucEVA; //sParam.ucEVA;
         	...
         	sprintf( cTemp, "Chip Pin Get Param DA %d %x ucEVA=%x", 
   ...
   giKeyboardType = iLIBUCM_Pinpad_Exist(UCMC_PPAD);
   ...
   else
   {
	...
	sprintf( cTemp, "after_reset Chip Pin display %x %d %d %d %d", iBackligthManagedByDLLVending, giDisplayType, giDisplayCanal, giDisplayBackColor, giIccBackColor);
	trace( 0x6E01, strlen(cTemp), cTemp);
   ...
   if ((sDevice.ucType == 1) && (sDevice.ucPilote == 0) && (memcmp(&sDevice.uctNomDriver[0], "0xFFFF", 6) == 0))
	...
	/* event idle message */
  if( (sStatus.ucUCM != 3 /* Not in Maintenance mode */  ) && ( giDisplayCanal > 0 ) )
  { 
   ...
   /* Deep Sleep for IUP250 and IUC180 */
   /* Light and Deep Sleep for UCR */
   /* examples: iRet = iSetSleepPowerManagement( 2,60, 0);  Deep sleep after 60s as soon as possible */
  /* examples: iRet = iSetSleepPowerManagement( 1,60, 10);  Light sleep after 60s as soon as possible: for CAD30UCR*/
#endif
...
//===========================================================================
//! \brief This function processes a non EMV transaction for vending kinematic.
//! \param[in] noappli : application number.
...
//! debit transaction.
//===========================================================================
int cu_debit_non_emv_Vending(NO_SEGMENT no, S_TRANSIN *param_in, S_TRANSOUT *param_out)
{
	char                   cMess [ 100+1 ] ;
	char                   cErrMess [ 100+1 ] ;
	...
	unsigned char		   bWaitReply;
	unsigned char          ucStatusIcc;
	unsigned char          ucPaymentWithTrack;
   ...
   sprintf( cTemp, "cu_debit_non_emv_Vending start");
   trace( 0x6E01, strlen(cTemp), cTemp);
	...
	if ( iLIBUCM_Icc_Open( UCMC_ICC, "rw" ) == FCT_OK  )			// open ICC media
	{
      ...
      sprintf( cTemp, "cu_debit_non_emv_Vending iWhat =%x",iWhat);
      trace( 0x6E01, strlen(cTemp), cTemp);
               ...
               else
               {
                  ...
                  usEvent = UCMMULTI_TTESTALL_ICC | UCMMULTI_TTESTALL_HOST ;
					...
					if( iLIBUCM_Device_Ttestall( &usEvent, 30000) == UCMTTESTALL_CR_OK_EVENT )
					{
                     ...
                     sprintf( cTemp, "iLIBUCM_Pay_Ready_For_Debit %d", iWhat );
                     trace( 0x6E01, strlen(cTemp), cTemp);
   ...
   {
      iLIBUCM_Display_Message ( UCMC_DISPLAY, "ICC KO\n", 100 ) ;
   }
...
int bStateHeader;
   ...
   sprintf( cTemp, "cu_debit_non_emv start");
   trace( 0x6E01, strlen(cTemp), cTemp);
...
//   param_out = &transaction_out;
         ...
         switch ( iWhat )
         {
            case UCMCPAY_SOLV :
               ...
               if ( (bIccOK) || (debitExport == FCT_OK) )
               {
                  sRDeb.ucCr = UCMHOST_CR_OK;
                  sRDeb.ulAmount=sDebit.ulAmount;
                  iGraphic_message( "DEBIT OK", 200, _MEDIUM_ );
   ...
   /*&END FT/07/2825/MA     inform CAd30 simu of paiement  */
	...
	trans_out->noappli     = noappli;
   trans_out->rc_payment  = PAY_OK;
...
int cu_fall_back (NO_SEGMENT noappli, ENTRY_TYPE AmountEntry, unsigned long amount)
{
   ...
   iUcmIac_BckColor(0, UCMCDISPLAY_BKL_COLOR_CYAN);
	...
	/* !! 2 ttestall are necessary (see system documentation)               */
...
//	char       *pcline2;
   ...
   char cTemp[100];
	...
	Menu.Fenetre.titre = (unsigned char *) CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_CHOOSE_AN_AID, lg_code);
	...
	for (i=0 ; i < aAIDCandidateList->count ; i++ )
	{
		...
		{
			Menu.tab[i] = (unsigned char *)aAIDCandidateList->list[i].preferredName;
		}
		else
		{
			Menu.tab[i] = (unsigned char*) aAIDCandidateList->list[i].applicationLabel;
		}
	...
	if(UCM_Present)
	{
		...
		{
			sprintf(ctMessage, "%.16s\n%s",Menu.tab[index], CUMESS_Read_Message(DEFAULT_TABLE, DFT_MESS_NB_YESV_NOA, lg_code));
...
int cu_give_your_specific_context(NO_SEGMENT No, S_SPECIFIC_CONTEXT *param_out)
{
	...
	strcpy(param_out->returned_state[param_out->response_number].appname, g_szAppName);
...
int Main(unsigned int size, StructPt *data)
{
	...
	case DEBIT_NON_EMV :
         if( gucCinematicType == 2) /* Vending payment cinematic EXE MDB */
         {
            cu_debit_non_emv_Vending((unsigned char)No, &data->Param.DebitNonEmv.param_in, &data->Param.DebitNonEmv.param_out);
         }
         else
         {
	...
	case FALL_BACK :
       ...
       if (!UCM_Present)
       {
         if( gucCinematicType == 0) /* No protocol */
         {       
	...
	case DEBIT_EMV :
         if( gucCinematicType == 2) /* Vending payment cinematic EXE MDB */
         {
            cu_debit_emv_Vending((unsigned char)No,&data->Param.DebitEmv.param_in,&data->Param.DebitEmv.param_out); 
         }
         else
         {
...
char cTemp[200];
int i;
    sprintf( cTemp, "idle_message_UCMC Chip pin");
    trace( 0x6E01, strlen(cTemp), cTemp);
    ...
    i = iUcmIac_BckColor(0, UCMCDISPLAY_BKL_COLOR_WHITE); 

    sprintf( cTemp, "idle_message_UCMC Chip Pin %d end", i);
    trace( 0x6E01, strlen(cTemp), cTemp);
 ...
 pEvent = (void*)&pdata_p->Par.ucBuffer[0];
    ...
    switch( pEvent->usType )
   {
      case UCMC_EVENT_TYPE_KEY:   
         ...
         {
            case 0x17: /*  red key  */
                   iLIBUCM_Display_Message(UCMC_DISPLAY, "None ", 200);
                   iSetSleepPowerManagement(0, 0, 0);
            break;
            case 0x18: /*  yellow key */
//               Buzzer_Test();
                   iLIBUCM_Display_Message(UCMC_DISPLAY, "Profond 30s ", 200);
                   iSetSleepPowerManagement(2,30, 0);
            break;
            case 0x16: /*  green key */
                   iLIBUCM_Display_Message(UCMC_DISPLAY, "leger 20s 10 ", 200);
                   iSetSleepPowerManagement(1,20, 10);
           break;
           ...
           default:
                 ...
                 //   iTestPowerOnSync();
                 //   iTestStopTtestall();
//                  iLIBUCM_Display_Message(UCMC_DISPLAY, "Power 30s", 200);
//                 iSetSleepPowerManagement(2,30, 0);
                 
//                  iUcmIac_BckColorDevice( 0x77,  UCMCDISPLAY_BKL_COLOR_YELLOW, 0, 0 ); /* for all devices */           
//                  iLIBUCM_Display_Message(UCMC_DISPLAY, "Fixe 4s", 400);

//                  iUcmIac_BckColorDevice( 0x13,  UCMCDISPLAY_BKL_COLOR_CYAN, 100, 100 ); /* display1 and 2 + IUR */           
//                  iLIBUCM_Display_Message(UCMC_DISPLAY, "Flash 8s...", 800);

//                  iUcmIac_BckColorDevice( 0x10,  UCMCDISPLAY_BKL_COLOR_GREEN, 0, 0 ); /* IUR */           
//                  iUcmIac_BckColorDevice( 0x03,  UCMCDISPLAY_BKL_COLOR_GREEN, 0, 0 ); /* All DISPLAY */                             
           break;
