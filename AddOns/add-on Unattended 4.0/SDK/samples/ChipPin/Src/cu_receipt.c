...
//! \param[in] pucString_p : a string .
//! \param[in] iWait_p : Waiting end of print en 10ms unit.
//===========================================================================
void CUPRINT_PrintString(unsigned char *pucString_p, int iWait_p )
{
   ...
   if(UCM_Present)
   {
      if( iLIBUCM_Print_Exist(UCMC_PRINT ) > 0 )
      {
         if( iWait_p != 0 )
         {
            iLIBUCM_Print_Option(UCMC_PRINT, (char*)pucString_p, UCMPRINT_NO_END | UCMPRINT_WAIT_END);
         }
         else
         {
            iLIBUCM_Print_Option(UCMC_PRINT, (char*)pucString_p, UCMPRINT_NO_END);
         }
      }
   }
#else
...
//===========================================================================
//! \brief This function prints a characters severals time.
//! \param[in] ucCara_p : character .
//! \param[in] iNbCara_p : number of printing same caracter.
//===========================================================================
void CUPRINT_PrintLoop(unsigned char ucCara_p, int iNbCara_p )
{
char cTemp[ 401 ];
int i, iNb;
...
while( iNbCara_p > 0 )
{
   ...
   if(UCM_Present)
   {
      if( iLIBUCM_Print_Exist(UCMC_PRINT ) > 0 )
      {
          iLIBUCM_Print_Option(UCMC_PRINT, cTemp,  UCMPRINT_NO_END);
      }
   }
   else
#endif 
...
//===========================================================================
//! \brief This function prints characters and control.
//! \param[in] pString : string.
//! \param[in] iNbCara_p : number caracter.
//===========================================================================
void CUPRINT_PrintMultiChar(unsigned char *pString, int iNbCara_p )
{
char cTemp[ 401 ];
int i, iNb, iNbFait;
...
static int CUPRINT_SetPrintLevel(const t_level level)
{
   ...
   char cTemp[ 200 ];
	...
	switch (level)
	{
	case L0 : // condensed mode
      sprintf( cTemp, "%c", 0x0F);
      CUPRINT_PrintString( (unsigned char*)cTemp, 0 );
	...
	case L2 : // double height mode
      sprintf( cTemp, "%cF", 0x1B);
      CUPRINT_PrintString( (unsigned char*)cTemp, 0 );
	...
	case L3 : // double width mode
      sprintf( cTemp, "%c", 0x0E);
      CUPRINT_PrintString( (unsigned char*)cTemp, 0 );
...
char cTemp[50];
	...
	switch (level)
	{
	case L0 : // uncondensed mode
      sprintf( cTemp, "%c", 0x12);
      CUPRINT_PrintString((unsigned char*) cTemp, 0 );
		break;
	case L1 :
		break;
	case L2 :
		break;
	case L3 : // undoublewidht mode
      sprintf( cTemp, "%c", 0x14);
      CUPRINT_PrintString( (unsigned char*)cTemp, 0 );
		break;
	case L4 : // unbold mode
      sprintf( cTemp, "%cF", 0x1B);
...
static void CUPRINT_String_XT(const unsigned char *string, const t_align align, const t_level level)
{
	...
	case ALIGN_LEFT :
      ...
      if(UCM_Present)
      {
         if( iLIBUCM_Print_Exist(UCMC_PRINT ) > 0 )
         {
            iLIBUCM_Print_Option(UCMC_PRINT, (char*)string, UCMPRINT_NO_END);
         }
      }
      else
 #endif          
		...
		else
		{
            ...
            CUPRINT_PrintMultiChar( (unsigned char *)string, i-j ) ;
         ...
         CUPRINT_PrintString( (unsigned char*) &string[j], 0 );
		}
		break;
	...
	case ALIGN_RIGHT :
		...
		if (lenght <= ticket_width)
		{
            ...
            CUPRINT_PrintLoop( ' ', spaces_nb );
			}

			// Print the string
         CUPRINT_PrintString( (unsigned char *)string, 0 );
		}
		// The string is too long to be printed on one line
		else
		{
...
static void CUPRINT_Double_String(const unsigned char *left_string, const unsigned char *right_string, const t_level level)
{
	...
	// Print left string
	lenght = strlen((char*)left_string);

   CUPRINT_PrintString( (unsigned char *)left_string, 0 );

	// Print blanks spaces between the two strings
   CUPRINT_PrintLoop( (unsigned char) ' ', spaces_nb );

	// Print the right string
	lenght = strlen((char*)right_string);
   CUPRINT_PrintString( (unsigned char *)right_string, 0 );   
...
static void CUPRINT_Data(const unsigned char *label, const unsigned char *value, const t_level level)
{
	...
	{
   	CUPRINT_PrintMultiChar( (unsigned char *)label, lg_libele );

      CUPRINT_PrintLoop( ' ', spaces_nb );

   	CUPRINT_PrintMultiChar( (unsigned char *)value, lg_valeur );

      CUPRINT_PrintString( (unsigned char *)"\n", 0 );
	}
	else
	{
      CUPRINT_PrintString( (unsigned char *)"Too many characters.", 50 );	
	}
...
void CUPRINT_Blanks_Lines(const unsigned char lines_nb)
{
      ...
      if( iLIBUCM_Print_Exist(UCMC_PRINT ) > 0 )
      {
         iLIBUCM_Print_NewLine(UCMC_PRINT, lines_nb);
      }
#else
      unsigned char i;
...
static void CUPRINT_Top_Frame(const short offset, const short lenght)
{
   ...
   char cTemp[ 200];
	...
	// Print the offset
   CUPRINT_PrintLoop( ' ', offset );

	border_nb = lenght - 2;

	// Print the right top corner
   sprintf( cTemp, "%c",LEFT_TOP_ROUNDED_CORNER);
   CUPRINT_PrintString( (unsigned char *)cTemp, 0 );	

	// Print the border
   CUPRINT_PrintLoop(HORIZONTAL_BAR, border_nb);
	
	// Print the left top corner
   sprintf( cTemp, "%c",RIGHT_TOP_ROUNDED_CORNER);
   CUPRINT_PrintString( (unsigned char *)cTemp, 0 );	
}
...
static void CUPRINT_Top_Frame_String(const short offset, const short lenght, const unsigned char *string, const t_align align)
{
   ...
   CUPRINT_PrintLoop( ' ', offset);
	...
	// Print the right top corner
   sprintf( cTemp, "%c",LEFT_TOP_ROUNDED_CORNER);
   CUPRINT_PrintString( (unsigned char *)cTemp, 50 );	
	...
	case ALIGN_LEFT :
      ...
      sprintf( cTemp, "%c%s%c",HORIZONTAL_END_BAR,string,HORIZONTAL_BEGIN_BAR);
      CUPRINT_PrintString( (unsigned char *)cTemp, 0 ); 
		
		// Print the blanks
      CUPRINT_PrintLoop(HORIZONTAL_BAR, spaces_nb);		
		break;
	...
	case ALIGN_CENTER :
      ...
      CUPRINT_PrintLoop(HORIZONTAL_BAR, _spaces_nb);		
   ...
   char cTemp[ 200];
	...
	case ALIGN_LEFT :
		...
		if (string_lenght <= max)
		{
         ...
         sprintf( cTemp,  "%c",VERTICAL_BAR );
         CUPRINT_PrintString( (unsigned char *)cTemp, 0 ); 
			
			// Print the string
			CUPRINT_PrintMultiChar( (unsigned char *)string, string_lenght ) ;

			// Print the blanks between the string and the right border
			spaces_nb_after = max - string_lenght;
         CUPRINT_PrintLoop( ' ', spaces_nb_after );
		...
		else
		{
            ...
            CUPRINT_PrintMultiChar( (unsigned char *)&string[0], ind_end-ind_begin ) ;
	...
	case ALIGN_CENTER :
		...
		if (string_lenght <= max)
		{
         ...
         CUPRINT_PrintLoop( ' ', spaces_nb_before);

			// Print the string
			CUPRINT_PrintMultiChar( (unsigned char *)&string[0], string_lenght) ;
...
static void CUPRINT_Double_String_Frame(const short offset, const short lenght, const unsigned char *left_string, const unsigned char *right_string)
{
   ...
   char cTemp[ 200];
	...
	else
	{
      ...
      sprintf( cTemp, "%c%s",VERTICAL_BAR,left_string);
      CUPRINT_PrintString( (unsigned char *)cTemp, 0 ); 		
...
static void CUPRINT_Bottom_Frame(const short offset, const short lenght)
{
   ...
   sprintf( cTemp, "%c",LEFT_BOTTOM_ROUNDED_CORNER);
   CUPRINT_PrintString( (unsigned char *)cTemp, 0 ); 	
...
void CUPRINT_Boundary_Line(const short offset, const short length)
{
   ...
   sprintf( cTemp, "%c", HORIZONTAL_BEGIN_BAR );
   CUPRINT_PrintString((unsigned char *)cTemp, 0 ); 
...
static void print_tail(void)
{
	...
	CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_0, g_language), ALIGN_CENTER, L1);

#ifdef _USE_UCM_
	CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SAGEM_MONETEL, g_language), ALIGN_CENTER, L1);
	CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_1, g_language), ALIGN_LEFT, L0);
   CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_2, g_language), ALIGN_LEFT, L0);

   CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_3, g_language), ALIGN_LEFT, L0);
   CUPRINT_String_XT( (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SMO_INFO_4, g_language), ALIGN_LEFT, L0);   
#else
...
void CURECEIPT_Print_Ticket_POS_Demo(TLV_TREE_NODE hInputTLVTree, unsigned char status_tr)
{
...
#ifdef _USE_UCM_
         /* No special cara for host printer */
         if( iLIBUCM_Print_Exist(UCMC_PRINT ) == 2 ) /* Host */
         {
            strcpy((char *) currency, "E");
         }
#endif
		}
	}
	...
	{
		CUPRINT_Top_Frame_String(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_APPROVED, g_language), ALIGN_CENTER);
	}
	else
	{
		CUPRINT_Top_Frame_String(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_DECLINED, g_language), ALIGN_CENTER);
	}
	...
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_AMOUNT, g_language), buffer);
	...
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_TRANSACTION_VERIFICATION_STATUS, g_language), tvr_ascii);
	CUPRINT_Bottom_Frame(0, 24);
	...
	{
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_SIGNATURE, g_language), ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
		CUPRINT_String_Frame(0, 24, (const unsigned char*)" ", ALIGN_LEFT);
	}
...
void CURECEIPT_Print_EMV_Config(void)
{
	...
	if(UCM_Present)
	{
		sprintf ((char*)buffer_imp, "%s\n",CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_EMV_CONFIG_IDENT, g_language));
      CUPRINT_PrintString( (unsigned char *)buffer_imp, 50 ); 
	}
	else
#endif
		CUPRINT_String_XT((const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_EMV_CONFIG_IDENT, g_language), ALIGN_CENTER, L2);
	CUPRINT_Blanks_Lines(1);
	...
	CUPRINT_Top_Frame(0, 24);
	CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_ICS, g_language), ALIGN_CENTER);

	cr = PAR_ReadParameter(TAG_CUST_IS_CONFIG_FILE, &data);
	if (cr)
		CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_NAME, g_language), data.elt.ptValue );
	// Checksums
	cr = PAR_ReadParameter(TAG_CUST_CRC_CONFIG_FILE, &data);
	if (cr)
		CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_CHECKSUM, g_language), data.elt.ptValue);
	cr = PAR_ReadParameter(TAG_CUST_FULL_CRC_CONFIG_FILE, &data);
	if (cr)
		CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_FULL_CHECKSUM, g_language), data.elt.ptValue);

	// Skip line
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)"", (const unsigned char*)"");

	// EMV Kernel Informations
	CUPRINT_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_EMV_KERNEL, g_language), ALIGN_CENTER);

	// Kernel Version
	sprintf((char*)buffer, "%ld", ct_KernelVersion);
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_VERSION, g_language), (const unsigned char*)buffer);
	// Kernel CRC
	/*&BEGIN FT/08/4075/MA print of EMVDC CRC*/
	sprintf((char*)buffer, "%04lx", ct_KernelCRC);
	/*&END FT/08/4075/MA print of EMVDC CRC*/
	CUPRINT_Double_String_Frame(0, 24, (const unsigned char*)CUMESS_Read_Message(RECEIPT_TABLE, CU_MESS_CHECKSUM, g_language), (const unsigned char*)buffer);
