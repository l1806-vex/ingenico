...
char cTemp[ 100 ];
    ...
    if( iLIBUCM_Pinpad_Exist(UCMC_PPAD) > 0 )
    {
        ...
        if ( j == FCT_OK )
        {
            ...
            sPPad.ucPhase = 0;
            sPPad.ucRuf[0] = 0;
            sprintf( (char*)sPPad.uctDisplay, "INPUT 4 DIGIT" );
            sPPad.ucLgDisplay = strlen( (char*)sPPad.uctDisplay );
        ...
        if ( j == FCT_OK )
        {
            ...
            sPPad.ucPhase = 0;
            sPPad.ucRuf[0] = 0;
            sprintf( (char*)sPPad.uctDisplay, "INPUT 2 DIGIT" );
            sPPad.ucLgDisplay = strlen( (char*)sPPad.uctDisplay );
...
extern int iGraphic_message( char *pMessage_p, int iTo_p, int iFontSize );
	...
	do
	{
		...
		if (Cr==CR_ENTRY_OK)
		{
			...
			case 2:
				...
				break;
			case 3:
                            iGraphic_message( "Graphic test", 200, _MEDIUM_ );
				break;
	...
	if ((result == T_VAL) && (entered_amount!=0))
	{
		...
		if(hInputTree != NULL)
		{
			...
			hOutputTree = NULL;
					...
					pc_line1 = CUMESS_Read_Message(TERMINAL_TABLE, STD_MESS_REMOVE_CARD, merchLang);
					CUTERMdisplayLine(0, pc_line1, LEFT, (char *)"w");
					uiEvent = ttestall(KEYBOARD,500);
					if (keyb != NULL)
					{
						fclose(keyb);
						keyb = NULL;
...
static void CUMORE_TransactionLoopMode(void)
{
	...
	char	msg[100];
	char	*pc_line1, *pc_line2;
	TransactionLoopMode = FALSE;
	CUTERMclearDisplay();
	merchLang = PSQ_Give_Language();
				CUTERMclearDisplay();
				pc_line1 = CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_LOOP_MODE, merchLang);
				pc_line2 = CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_SELECTED, merchLang);
				memset(msg, 0, sizeof(msg));
				sprintf(msg, "%s\n%s", pc_line1, pc_line2);
...
static void CUMORE_BatchCapt_Iso8583 (void)
{
	...
	hTreeDis = NULL;
   hTree = NULL; /* bug fuite memoire */
   hOutTree = NULL; /* bug fuite memoire */
	...
	if (bConnected)
	{
		...
		if(hOutTree != NULL)
		{
         ...
         {
            TlvTree_ReleaseEx(&hTree); /* bug mem */
         }
...
static void CUMORE_Enter_Terminal_ID (void)
{
	...
	while (i>0)
	{
		...
		Menu.Fenetre.titre = (unsigned char *) CUMESS_Read_Message(TERMINAL_TABLE, CMN_MESS_MERCHANT_TERMINAL_ID2, lg_code);
		InitMSGinfos_message((MSGinfos *)&(Menu.tab[0]), NULL);
		InitMSGinfos_message((MSGinfos *)&(Menu.tab[1]), card_acceptor_terminal_id);
		InitMSGinfos_message((MSGinfos *)&(Menu.tab[2]), NULL);
		...
		t_term_id.nb_max = 8;
		t_term_id.nb_min = 0;
		G_Numerical_Entry(&Menu, (TAB_ENTRY_STRING *)&t_term_id);
		ttestall(ENTRY,0);
...
static void CUMORE_Load_Param(void)
{
  ...
  ix= FS_mount(new_szFileName, &uiAccessMode);
  ix = FS_unlink(new_szFileName);
  ix = FS_unlink(szFileName);
}
