...
{ 
   ...
   y= 0;
	...
	strcpy((char*)ac_l_CurrentScreen, xs_text);
...
//		iLIBUCM_Display_Message(UCMC_DISPLAY," ", 0);
...
#endif
	{
   	...
   	lg = strlen((char*)ac_l_CurrentScreen);
	...
	if(UCM_Present)
	{
		iLIBUCM_Display_Message(UCMC_DISPLAY, (char*)ac_l_CurrentScreen, 0);
	}
	else
#endif
	...
	{
		// For test
		CurrentMSG.message = (char*)ac_l_CurrentScreen;  

		// Header management / display on X07
		// ERROR_Deprecation_For_Module_Trap_SeeDocumentation_In_CHM_File gotoxy(xi_line, y);
				...
				if(cr>=0)
				{
					...
					*xp_key = (T_CUTERMkey)iLIBUCM_Pinpad_GetChar(UCMC_PPAD);
      				...
      				if( giKeyboardType == PARAM_TYPE_IUC180B )
      				{
      				   ...
      				   {
      				      case T_SKHAUT:
      				         *xp_key = (T_CUTERMkey)T_ANN;
      				      break;

      				      case T_SKBAS:
      				         *xp_key = (T_CUTERMkey)T_VAL;
      				      break;
