...
int PAR_InitParam(unsigned char uc_x_InitType, char *pc_x_DirFile)
{
	...
	switch (uc_x_InitType)
	{
		case C_PARAM_INIT_APLI :
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_INIT_APLI"), "PAR_InitParam() : C_PARAM_INIT_APLI");
			CUPAR_InitTree(&pTreeAppli);
			break;

		case C_PARAM_INIT_DEFAULT :
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_INIT_DEFAULT"), "PAR_InitParam() : C_PARAM_INIT_DEFAULT");
		...
		case C_PARAM_INIT:
			...
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_INIT"), "PAR_InitParam() : C_PARAM_INIT");
			...
			{
				trace(0x6E02, strlen("Pb on PARAM.PAR file load"), "Pb on PARAM.PAR file load");
				upload_diagnostic_txt ((unsigned char*)"\n Problem on CUPAR_FileRead() Load PARAM.PAR file");
				i_l_result = 1;
			}
			else
				trace(0x6E02, strlen("PARAM.PAR file loaded"), "PARAM.PAR file loaded");

			break;

		case C_PARAM_DEFAULT:
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_DEFAULT"), "PAR_InitParam() : C_PARAM_DEFAULT");
		...
		case C_PARAM_DEFAULT_ICS: 
			...
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_DEFAULT_ICS"), "PAR_InitParam() : C_PARAM_DEFAULT_ICS");
			TlvTree_Release(pTreeDefault);
		...
		case C_PARAM_ONLY_DEFAULT_ICS:		// Delete ICS values in pTreeParam and load default ICS values in pTreeParam and save new TLVTree Struct in param.par file
			...
			{
				trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_ONLY_DEFAULT_ICS"), "PAR_InitParam() : C_PARAM_ONLY_DEFAULT_ICS");
			...
			{
				trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID"), "PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID");
			...
			{
				trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID_EVAL"), "PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID_EVAL");
				ui_l_NumMark = C_TAG_AID;
			...
			{
				trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID_VISA"), "PAR_InitParam() : C_PARAM_ONLY_DEFAULT_AID_VISA");
				ui_l_NumMark = C_TAG_AID;
				pc_l_ParamArray = (char*)DefaultParamVisa;
				i_l_Size = sizeof(DefaultParamVisa);
			}
			else
				trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_???"), "PAR_InitParam() : C_PARAM_???");
		...
		case C_PARAM_FILE_TLVTSERIAL:
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_FILE_TLVTSERIAL"), "PAR_InitParam() : C_PARAM_FILE_TLVTSERIAL");
		...
		case C_PARAM_FILE_XML:
			trace(0x6E02, strlen("PAR_InitParam() : C_PARAM_FILE_XML"), "PAR_InitParam() : C_PARAM_FILE_XML");
		...
		default:
			trace(0x6E02, strlen("PAR_InitParam() : ??"), "PAR_InitParam() : ??");
			break;
	}

	if (i_l_ControlCaKeys)
		if (CUPAR_ControlCAKEYS() != TRUE)
			trace(0x6E02, strlen("PAR_InitParam():CUPAR_ControlCAKEYS pb\n"), "PAR_InitParam():CUPAR_ControlCAKEYS pb\n");
...
{
	trace(0x6E02, strlen("CUPAR_Update_Param_File()"), "CUPAR_Update_Param_File()");
...
static int CUPAR_GiveCAKEYS(CA_PUB_KEY * ptr_x_key, unsigned char * ptr_x_AID, unsigned char uc_x_Index)
{
	...
	do{
		...
		{
			if (cr == 0)
				trace(0x6E02, strlen("Bad CR PAR_ReadParameter TAG_AID_TERMINAL\n"), "Bad CR PAR_ReadParameter TAG_AID_TERMINAL\n");
			else
				trace(0x6E02, strlen("AID bad length \n"), "AID bad length \n");
			// Next NumMark, this one isn't good
			continue;
		}
		...
		{
			if (cr == 0)
				trace(0x6E02, strlen("Bad CR PAR_ReadParameter KEY_INDEX_TERM\n"), "Bad CR PAR_ReadParameter KEY_INDEX_TERM\n");
			else
				trace(0x6E02, strlen("TAG_CERT_AUTH bad length \n"), "TAG_CERT_AUTH bad length \n");
			// Next NumMark, this one isn't good
			continue;
		}
		...
		{
			if (cr == 0)
				trace(0x6E02, strlen("Bad CR PAR_ReadParameter KEY_MODULUS\n"), "Bad CR PAR_ReadParameter KEY_MODULUS\n");
			else
				trace(0x6E02, strlen("256 \n"), "256 \n");
			// Next NumMark, this one isn't good
			continue;
		}
...
int CUPAR_ControlCAKEYS (void)
{
	...
	do{
		...
		if ((cr) && (data.elt.length == 20) && (pShaDigest != NULL))
		{
			...
			{
				trace(0x6E02, strlen(" SHA KO !!\n"), " SHA KO !!\n");

				trace(0x6E02, 31, (unsigned char *)"Invalid key SHA (RID/Index/SHA)");
				trace(0x6E02, sizeof(key.rid), key.rid);
				trace(0x6E02, sizeof(key.index), key.index);
				trace(0x6E02, 20, (unsigned char *)pShaDigest);
		...
		{
			if (cr == 0)
				trace(0x6E02, strlen(" bad cr KEY_CHECKSUM \n"), " bad cr KEY_CHECKSUM \n");
			else if (pShaDigest == NULL)
				trace(0x6E02, strlen(" pSHA null \n"), " pSHA null \n");
			else 
				trace(0x6E02, strlen(" bad KEY_CHECKSUM length \n"), " bad KEY_CHECKSUM length \n");
...
void CUPAR_RestoreParam (unsigned char uc_x_FileMgtType)
{
		...
		(uc_x_FileMgtType == C_MGT_TXT_PAR_FILES))
	{
		...
		for (FileIndex=0; FileIndex<C_NB_OLD_FILES; FileIndex++)
		{
			...
			if (ret == FS_OK)
			{
				...
				trace(0x6E02, strlen(strbuf), strbuf);
			}
		}
	...
	{
		uiMode = FS_READMOD;
		rc   = FS_mount (FilePath_L, &uiMode);
		if (rc != FS_OK)
		{
			trace(0x6E02, strlen("PB Mount COM Filen"), "PB Mount COM Filen");
			upload_diagnostic_txt((unsigned char *)"PB Mount COM Filen\n");
		}
		else
		{
			trace(0x6E02, strlen("Mount COM File ok"), "Mount COM File ok");
			trace(0x6E02, strlen("Load PARAM.PAR file"), "Load PARAM.PAR file");
