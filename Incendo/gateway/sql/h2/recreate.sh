...
function recreateDB () {
  ...
  java -cp $CLASSPATH $CLASS $uname $passwd $JDBC_URL -f oegw/crebas.dml.sql  || return
	...
	if [ -f oegw/testdata.sql ]; then
    java -cp $CLASSPATH $CLASS $uname $passwd $JDBC_URL -f oegw/testdata.sql  || return
    fi
  java -cp $CLASSPATH $CLASS $uname $passwd $JDBC_URL -f cfgsrv/crebas.ddl.sql  || return
}
