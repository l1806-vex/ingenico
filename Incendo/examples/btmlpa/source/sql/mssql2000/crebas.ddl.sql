...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'btmlpa')
...
if not exists (select * from master.dbo.syslogins where loginname = N'btmlpa')
BEGIN
exec sp_addlogin N'btmlpa', 'ingenico', 'btmlpa'
END
GO
...
if not exists (select * from dbo.sysusers where name = N'btmlpa' and uid < 16382)
	EXEC sp_grantdbaccess N'btmlpa', N'btmlpa'
GO
