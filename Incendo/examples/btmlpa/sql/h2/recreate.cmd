...
set JDBC_URL=jdbc:h2:~/btmlpa
set uname=-user btmlpa
...
rem -----------------------------------------------------------------
java -cp %CLASSPATH% %CLASS% -url %JDBC_URL% %uname% %passwd% -script cred.ddl.sql || goto error
java -cp %CLASSPATH% %CLASS% -url %JDBC_URL% %uname% %passwd% -script crebas.ddl.sql || goto error
java -cp %CLASSPATH% %CLASS% -url %JDBC_URL% %uname% %passwd% -script crebas.dml.sql || goto error
